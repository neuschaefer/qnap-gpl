...
static unsigned long long g_last_member_write_ios[SM_MAX_RAID_NUMBER];
...
static int io_is_changed()
{
    int i, raid_id, raid_count = 0, io_changed = 0;
    int *raid_id_aryP = NULL;
    char raid_name[DEVICE_NAME_LEN];
    char *nameP;
    BLK_DEV_STAT stat_info;

    if (!(raid_id_aryP = calloc(SM_MAX_RAID_NUMBER, sizeof(int)))) {
        return 1;
    }

    raid_count = RAID_Enumerate(raid_id_aryP, SM_MAX_RAID_NUMBER, NULL, NULL);

    for(i = 0; i < raid_count; i++) {
        raid_id = raid_id_aryP[i];

        if (NAS_RAID_Is_Version2(raid_id) &&
           !RAID_Get_Name(raid_id, raid_name, sizeof(raid_name))) {

            Generic_Get_File_Name(raid_name, nameP);
            if (!Blk_Dev_Get_IO_Statistics(nameP, &stat_info)) {
                if (stat_info.write_count != g_last_member_write_ios[raid_id]) {

                    //UTIL_Debug(TRACE_INFO, "%s(%d): IO CHANGED! stat_info.write_count: %llu, g_last_member_write_ios[%d]: %llu\n", __func__, __LINE__, stat_info.write_count, raid_id, g_last_member_write_ios[raid_id]);              
                    io_changed = 1;
                    break;
                }
            }
...
static int save_last_io()
{
    int i, raid_id, raid_count = 0;
    int *raid_id_aryP = NULL;
    char raid_name[DEVICE_NAME_LEN];
    char *nameP;
    BLK_DEV_STAT stat_info;
    ...
    for(i = 0; i < raid_count; i++) {
        ...
        if (NAS_RAID_Is_Version2(raid_id) &&
           !RAID_Get_Name(raid_id, raid_name, sizeof(raid_name))) {
            ...
            Generic_Get_File_Name(raid_name, nameP);
            if (!Blk_Dev_Get_IO_Statistics(nameP, &stat_info)) {
                // g_last_member_read_ios[raid_id] = stat_info.read_count;
                g_last_member_write_ios[raid_id] = stat_info.write_count;
            }
        }
    }
...
{
#if DMEVENTD_DBG_IOSATUS
	int io_status = 0;
	FILE *fp;
	fp = fopen("/root/DMEVENTD_iostatus", "a");
#endif
...
#if DMEVENTD_DBG_IOSATUS
	if (io_status)
		fprintf(fp, "DMEVENTD _do_process_event, io changed!\n");
	else
		fprintf(fp, "DMEVENTD _do_process_event, io UN-changed!\n");
...
static void *_monitor_thread(void *arg)
{
	...
	while (1) {
		...
		// If io statistics is changed, do the rutine, else continue.
		if (io_is_changed()) {
                        save_last_io();
                } else {
                        continue;
...
int main(int argc, char *argv[])
{
	...
	//printf("DMEVENTD > main > Volume_Are_Ready_To_Shut_Down %d\n", Volume_Are_Ready_To_Shut_Down());
