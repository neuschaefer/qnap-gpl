...
int lv_thin_pool_percent(const struct logical_volume *lv, int metadata,
			 percent_t *percent, uint64_t *qnap_lv_used_data)
{
	return 0;
}
			 ...
			 percent_t *percent, uint64_t *qnap_lv_used_data)
{
	...
	if (!(r = dev_manager_thin_pool_percent(dm, lv, metadata, percent, qnap_lv_used_data)))
		stack;
		    ...
		    int mapped, percent_t *percent, uint64_t *qnap_lv_used_data)
{
	...
	if (!(r = dev_manager_thin_percent(dm, lv, mapped, percent, qnap_lv_used_data)))
		stack;
