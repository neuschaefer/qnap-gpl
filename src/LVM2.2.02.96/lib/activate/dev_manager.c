...
static int _percent_run(struct dev_manager *dm, const char *name,
			...
			uint32_t *event_nr, int fail_if_percent_unsupported, uint64_t *qnap_lv_used_data)
{
	int r = 0;
	struct dm_task *dmt;
	struct dm_info info;
	...
	uint64_t total_numerator = 0, total_denominator = 0, qnap_used_data = 0;
	...
	*overall_percent = PERCENT_INVALID;
	...
	do {
		...
		*qnap_lv_used_data = qnap_used_data;
		    ...
		    uint32_t *event_nr, int fail_if_percent_unsupported, uint64_t *qnap_lv_used_data)
{
	if (dlid && *dlid) {
		if (_percent_run(dm, NULL, dlid, target_type, wait, lv, percent,
				 event_nr, fail_if_percent_unsupported, qnap_lv_used_data))
			return 1;
				 ...
				 percent_t *percent)
{
	...
	if (!(_percent(dm, name, dlid, "snapshot", 0, NULL, percent,
		       NULL, fail_if_percent_unsupported, &qnap_lv_used_data)))
		return_0;
...
int dev_manager_thin_pool_percent(struct dev_manager *dm,
				  const struct logical_volume *lv,
				  int metadata, percent_t *percent, uint64_t *qnap_lv_used_data)
{
	char *name;
	const char *dlid;
	...
	if (!(_percent(dm, name, dlid, "thin-pool", 0,
		       (metadata) ? lv : NULL, percent, NULL, 1, qnap_lv_used_data)))
		return_0;
...
int dev_manager_thin_percent(struct dev_manager *dm,
			     const struct logical_volume *lv,
			     int mapped, percent_t *percent, uint64_t *qnap_lv_used_data)
{
	char *name;
	const char *dlid;
	const char *layer = lv_is_origin(lv) ? "real" : NULL;
	...
	if (!(_percent(dm, name, dlid, "thin", 0,
		       (mapped) ? NULL : lv, percent, NULL, 1, qnap_lv_used_data))) {
		
	    if (!(_percent(dm, name, dlid, "thick", 0,
		       (mapped) ? NULL : lv, percent, NULL, 1, qnap_lv_used_data)))
		return_0;
	}
			       ...
			       dm_node_callback_t type, void *cb_data)
{
	...
	// TBD
	// If dm-thin metadata block size is 8192, thin_check, thin_dump, ...
	// must been modified for supporting parsing the bigger metadata block
#ifdef QTS_SNAPSHOT
#else
