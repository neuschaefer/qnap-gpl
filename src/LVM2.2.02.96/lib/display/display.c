		   ...
		   void *handle __attribute__((unused)))
{
	...
	uint64_t qnap_lv_used_data = 0;	// 2013/09/16, add by Burton. For getting extra information for lvdispaly
	...
	if (lv_is_thin_volume(lv)) {
		...
		if (seg->origin)
			log_print("LV Thin origin name    %s",
				  seg->origin->name);
		if (inkernel) {
			// 2013/09/16, add by Burton. For getting extra information for lvdispaly
			thin_active = lv_thin_percent(lv, 0, &thin_percent, &qnap_lv_used_data);
		}
	} else if (lv_is_thin_pool(lv)) {
		if (inkernel) {
			// 2013/09/16, add by Burton. For getting extra information for lvdispaly
			thin_data_active = lv_thin_pool_percent(lv, 0, &thin_data_percent, &qnap_lv_used_data);
			thin_metadata_active = lv_thin_pool_percent(lv, 1, &thin_metadata_percent, &qnap_lv_used_data);
		}
...
************/
	...
	// 2013/09/16, add by Burton. For getting extra information for lvdispaly
	if (lv_is_thin_pool(lv)) {
		log_print("Allocated pool chunks  %llu", qnap_lv_used_data);
	} else if (lv_is_thin_volume(lv)) {
		log_print("Mapped sectors         %llu", qnap_lv_used_data);
	}
