				 ...
				 uint32_t stripes, uint32_t stripe_size)
{
	...
	if (clear_metadata) {
		...
		for (s = 0; s < seg->area_count; s++) {
			 ...
			 */
			if (!set_lv(meta_lv->vg->cmd, meta_lv, 0, 1, 0, LV_NOT_THIN_POOL_METADATA)) {
 ...
 * @param[in] offset_sector  The sector to start writing.
 * @param[in] sectors        The number of sectors to write the value.
 */
int set_lv(struct cmd_context *cmd, struct logical_volume *lv,
	   uint64_t offset_sector, uint64_t sectors, int value, int is_thin_metadata)
{
	struct device *dev;
	char *name;
	...
	if (!sectors) {
		if (is_thin_metadata == LV_IS_THIN_POOL_METADATA)
			sectors = UINT64_C(8192) >> SECTOR_SHIFT;
		else
	...
	if (!dev_set(dev, offset_sector << SECTOR_SHIFT, (size_t) sectors << SECTOR_SHIFT, value))
		stack;
...
// 20130430, Modified by Burton, for lvcreate --id
#if 1
		if (lp->thin_volumnid > 0) { 
			// If the passed id is smaller than any existed ID
			// Stop creating lv!
			if (validate_the_volumnid(first_seg(pool_lv), lp->thin_volumnid)) {
				printf("\n---===Using id %llu===---\n\n", lp->thin_volumnid);
				first_seg(lv)->device_id = lp->thin_volumnid;
			}
			else {
				printf("\n---===Snap_id %llu Error. Stop creating device===---\n", lp->thin_volumnid);
				goto revert_new_lv;
			}
		}
		else {
	...
	if (!seg_is_thin(lp) && !lp->zero && !lp->snapshot)
		log_warn("WARNING: \"%s\" not zeroed", lv->name);
		 ...
		 !set_lv(cmd, lv, UINT64_C(0), UINT64_C(0), 0, LV_NOT_THIN_POOL_METADATA)) {
