...
int pool_below_threshold(const struct lv_segment *pool_seg)
{
	...
	uint64_t qnap_lv_used_data = 0;

	/* Data */
	if (!lv_thin_pool_percent(pool_seg->lv, 0, &percent, &qnap_lv_used_data))
		return_0;
	...
	/* Metadata */
	if (!lv_thin_pool_percent(pool_seg->lv, 1, &percent, &qnap_lv_used_data))
		return_0;
...
// 20130430, Modified by Burton, for lvcreate --id
#if 0
int validate_the_volumnid(struct lv_segment *thin_pool_seg, int64_t volumn_id)
{
	uint32_t max_id = 0;
        struct seg_list *sl;
	...
	if (!seg_is_thin_pool(thin_pool_seg)) {
        ...
        if (++max_id > DM_THIN_MAX_DEVICE_ID) {
                ...
                printf("get_free_pool_device_id!\t\t 0\n");
                return 0;
        }
		...
		struct alloc_handle *ah, uint32_t stripes, uint32_t stripe_size)
{
	...
	char name[len];
	uint64_t superblock_backup_offset = 0; // in sector
	const uint64_t superblock_backup_size = METADATA_BLOCK_SIZE * NUM_BACKUP_METADATA_BLOCKS >> SECTOR_SHIFT; //in sectors
	...
	if (activation()) {
		 ...
		 */
		if (pool_lv->size > DM_THIN_MAX_METADATA_SIZE)
			superblock_backup_offset = DM_THIN_MAX_METADATA_SIZE - superblock_backup_size;
		else if (pool_lv->size > superblock_backup_size)
			superblock_backup_offset = pool_lv->size - superblock_backup_size;
		/* pool_lv is a new LV so the VG lock protects us */
		if (!activate_lv_local(pool_lv->vg->cmd, pool_lv) ||
		    /* Clear 8KB of metadata device for new thin-pool. */
		    !set_lv(pool_lv->vg->cmd, pool_lv, UINT64_C(0), UINT64_C(0), 0, LV_IS_THIN_POOL_METADATA) ||
		    /* Clear the backup blocks at the end of the metadata device */
		    (superblock_backup_offset > 0 && !set_lv(pool_lv->vg->cmd, pool_lv, superblock_backup_offset,
		                                             superblock_backup_size, 0,
		                                             LV_IS_THIN_POOL_METADATA))) {
