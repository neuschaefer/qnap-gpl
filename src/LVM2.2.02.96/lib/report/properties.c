...
static percent_t _data_percent(const struct logical_volume *lv)
{
	...
	return lv_thin_pool_percent(lv, 0, &perc, &qnap_lv_used_data) ? perc : PERCENT_INVALID;
}
...
static percent_t _metadata_percent(const struct logical_volume *lv)
{
	...
	return lv_thin_pool_percent(lv, 1, &perc, &qnap_lv_used_data) ? perc : PERCENT_INVALID;
}
