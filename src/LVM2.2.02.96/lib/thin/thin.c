				     ...
				     uint64_t *qnap_used_data)
{
	...
	//printf("_thin_pool_target_percent USED %llu, TOTAL %llu==--\n", s->used_data_blocks, s->total_data_blocks);
	*qnap_used_data = s->used_data_blocks;
...
static int _thick_add_target_line(struct dev_manager *dm,
				 struct dm_pool *mem,
				 struct cmd_context *cmd __attribute__((unused)),
				 void **target_state __attribute__((unused)),
				 struct lv_segment *seg,
				 ...
				 uint32_t *pvmove_mirror_count __attribute__((unused)))
{
	...
	if (!dm_tree_node_add_thick_target(node, len, pool_dlid, device_id))
		return_0;
				...
				uint64_t *qnap_used_data)
{
	...
	if (seg) {
		//printf("_thin_target_percent USED SECTORS %llu\n", s->mapped_sectors);
		*qnap_used_data = s->mapped_sectors;
...
static struct segtype_handler _thick_ops = {
	.name = _thin_name,
	.text_import = _thin_text_import,
	.text_export = _thin_text_export,
#ifdef DEVMAPPER_SUPPORT
	.add_target_line = _thick_add_target_line,
	.target_percent = _thin_target_percent,
	.target_present = _thin_target_present,
#endif
	.modules_needed = _thin_modules_needed,
...
#endif
{
	static const struct {
		...
		{ &_thick_ops, "thick", SEG_THIN_VOLUME | SEG_VIRTUAL }
	};
