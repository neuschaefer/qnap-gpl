...
// 20130430, Modified by Burton, for lvcreate --snapid
	if (arg_int64_value(cmd, volumnid_ARG, 0) > 0) {
		lp->thin_volumnid = arg_int64_value(cmd, volumnid_ARG, 0);
		printf("lvcreate > _lvcreate_params, id for snapshot! %llu\n", lp->thin_volumnid);
	}
	else {
		lp->thin_volumnid = 0;
		printf("lvcreate > _lvcreate_params, id generated by lvcreate!\n");
	}
