...
void avahi_interface_address_update_rrs(AvahiInterfaceAddress *a, int remove_rrs) {
    ...
    if (m->list_complete &&
        ...
        if (avahi_s_entry_group_is_empty(a->entry_group)) {
			...
			*/
			if((strstr(m->server->host_name, "tbt.local") && strstr(a->interface->hardware->name, "tbtbr")) ||
				(!strstr(m->server->host_name, "tbt.local") && !strstr(a->interface->hardware->name, "tbtbr") && !strstr(a->interface->hardware->name, "tbtnet")))
			#endif
