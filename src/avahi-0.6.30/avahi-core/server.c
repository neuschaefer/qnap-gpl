...
static void register_localhost(AvahiServer *s) {
	...
	expect client connect to bonjour server by thunderbolt interface. As avahi daemon starts, it
	add all interface except for thunderbolt to NAS system host name, and we write ip-hostname pair
	of thunderbolt into /etc/avahi/hosts to add thunderbolt ip to special hostname.
	*/
	AvahiHwInterface *hw = s->monitor->hw_interfaces;
	for(; hw; hw=hw->hardware_next)
	{
		if((!strstr(s->host_name, "tbt.local") && !strstr(hw->name, "tbtbr") && !strstr(hw->name, "tbtnet")) ||
			(strstr(s->host_name, "tbt.local") && strstr(hw->name, "tbtbr")))
			avahi_server_add_address(s, NULL, hw->index, AVAHI_PROTO_UNSPEC, AVAHI_PUBLISH_NO_PROBE|AVAHI_PUBLISH_NO_ANNOUNCE, "localhost", &a);
	}
	#else
	...
	#ifdef QNAPNAS
	for(hw = s->monitor->hw_interfaces; hw; hw=hw->hardware_next)
	{
		if((!strstr(s->host_name, "tbt.local") && !strstr(hw->name, "tbtbr") && !strstr(hw->name, "tbtnet")) ||
			(strstr(s->host_name, "tbt.local") && strstr(hw->name, "tbtbr")))
			avahi_server_add_address(s, NULL, hw->index, AVAHI_PROTO_UNSPEC, AVAHI_PUBLISH_NO_PROBE|AVAHI_PUBLISH_NO_ANNOUNCE, "ip6-localhost", &a);
	}
	#else
