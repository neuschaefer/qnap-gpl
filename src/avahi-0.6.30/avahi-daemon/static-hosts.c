...
static void add_static_host_to_server(StaticHost *h)
{
    ...
    if (avahi_s_entry_group_is_empty(h->group)) {
		...
		*/
		AvahiHwInterface *hw = avahi_server->monitor->hw_interfaces;
		for(; hw; hw=hw->hardware_next)
		{
			if((!strstr(h->host, "tbt.local") && !strstr(hw->name, "tbtbr") && !strstr(hw->name, "tbtnet")) ||
				(strstr(h->host, "tbt.local") && strstr(hw->name, "tbtbr")))
			{
				if ((err = avahi_server_add_address(avahi_server, h->group, hw->index, p, 0, h->host, &h->address)) < 0) {
