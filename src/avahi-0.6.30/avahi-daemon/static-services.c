...
static void add_static_service_group_to_server(StaticServiceGroup *g) {
    ...
    for (s = g->services; s; s = s->services_next) {
        AvahiStringList *i;
		/*
		QNAP jeffchang modify for spec#4355, we give the thunderbolt interface a special hostname, and
		expect client connect to bonjour server by thunderbolt interface. we write ip-hostname pair of 
		thunderbolt into /etc/avahi/hosts to make avahi broadcast all service through thunderbolt. 
		Also, we duplicate all service file under /etc/avahi/services and rename
		the duplicated service files. At last, we write <host-name> NAS-HOSTNAME </host-name> to the 
		original service files and write <host-name> HOSTNAME-FOR-THUNDERBOLT </host-name>, modify 
		<name replace-wildcards="yes"> SERVICE-NAME-FOR-THUNDERBOLT </name> to the duplicated service files.
		*/
		#ifdef QNAPNAS
		AvahiHwInterface *hw;
		for(hw=g->entry_group->server->monitor->hw_interfaces; hw; hw=hw->hardware_next)
		{
			if(strstr(hw->name, "tbtbr"))
			{
				if(!strstr(g->chosen_name, "(Thunderbolt)"))
					continue;
			}
			else if(strstr(hw->name, "tbtnet"))
				continue;
			else
			{
				if(strstr(g->chosen_name, "(Thunderbolt)"))
			...
			//avahi_log_info("add service '%s' of type '%s', interface %s, host_name: %s",
	                            //g->chosen_name, s->type, hw->name, s->host_name);
		#endif
