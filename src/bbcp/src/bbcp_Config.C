   ...
   while (c=arglist.getopt())
     { switch(c)
       ...
       case 'T': if (SnkXeq) free(SnkXeq);
		 ...
		 if(_c = strstr(SnkXeq, "ServerAliveInterval"))
		    sscanf(_c,"%*[A-Za-z=]%d", &_t1);
		 if(_c = strstr(SnkXeq, "ServerAliveCountMax"))
		    sscanf(_c,"%*[A-Za-z=]%d", &_t2);
		 TimeOutVal = _t1;
		 //fprintf(stderr, "%s %d\n", SnkXeq, _t1);
		 if(TimeOutVal) {
		    TimeOutVal = _t2 ? TimeOutVal * _t2 : TimeOutVal;
		    TimeOutVal += 120;
		 }
                 break;
...
//
   ...
   if (TimeOutVal)               {Add_Opt('T'); sprintf(_bbuf, "ServerAliveInterval=%d", (TimeOutVal>120) ? TimeOutVal-120: TimeOutVal); Add_Str(_bbuf); }
