...
static unsigned long waited = 0;
static unsigned long unwaited = 0;
static unsigned long long wait_sec = 0;
static unsigned long long delay_sec = 0;
static unsigned long long dump_sec = 0;
...
// cerr <<"PIPE   READ SIZE=" <<rdsz <<endl;
  ...
  fprintf(stderr,"Wait for %lu/%llu and Not wait for %lu/%llu Dump %llu\n", waited, wait_sec, unwaited, delay_sec, dump_sec);
// All done
//
   return 0;
}
...
//
   Ticker.Stop(); Ticker.Report(Etime); Ticker.Reset();
   minus = 0;
   if (Etime < PaceTime) {
    
    if(delay_sec && delay_sec > (PaceTime - Etime)){
        delay_sec -= (PaceTime - Etime);
    }
    else{
      waited++;
      minus = PaceTime - Etime - delay_sec;
      assert((PaceTime - Etime - delay_sec)>=0);
      wait_sec += (PaceTime - Etime - delay_sec);
      Ticker.Wait((long long)((PaceTime - Etime - delay_sec)));
      delay_sec = 0;
    }
  }
  else{
    unwaited++;
    delay_sec += Etime - PaceTime;
  }
  Ticker.Stop(); Ticker.Report(Dtime); Ticker.Reset();
  delay_sec += (Dtime - minus);
  dump_sec += (Dtime-minus);
   Ticker.Reset();
}
