...
inline int  CondWait(int _TO)
       {int rc;
	struct timespec ts;
	clock_gettime(CLOCK_REALTIME, &ts);
	ts.tv_sec += _TO;
	ts.tv_nsec = 0;
        do {rc=sem_timedwait( &h_semaphore, &ts);} while ((rc == -1) && (errno==EINTR));
	if (rc)
        {   //if (errno == EBUSY) return 0;
                  {throw std::runtime_error("sem_CondWait() failed"), errno;}
        }
        return rc;
       }

inline void Post() {if (sem_post(&h_semaphore))
                       {throw std::runtime_error("sem_post() failed"), errno;}
                   }

inline void Wait() {int rc;
                    do {rc=sem_wait(&h_semaphore);} while (rc && errno==EINTR);
                    if (rc) {throw std::runtime_error("sem_wait() failed"), errno;}
                   }

  bbcp_Semaphore(int semval=1) {if (sem_init(&h_semaphore, 0, semval))
                                   {throw std::runtime_error("sem_init() failed"), errno;}
                               }
 ~bbcp_Semaphore() {if (sem_destroy(&h_semaphore))
                       {throw std::runtime_error("sem_destroy() failed"), errno;}
                   }
