...
{
    char cmd[256], buff[16];
    int grandpa = -1;
    FILE *fp;
    sprintf(cmd, "/bin/cat /proc/%d/status | /bin/grep PPid | /bin/awk '{print $2}'", getppid());
    if ( (fp = popen(cmd, "r")) == NULL)
        return grandpa;
    fgets(buff, sizeof(buff), fp);
    pclose(fp);
    grandpa = strtol(buff, (char **)NULL, 10);
    if (errno) grandpa = -2;

    return (!kill(grandpa, 0)) ? grandpa : -3 ;
