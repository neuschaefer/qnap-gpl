...
{
#ifdef QNAPNAS
	long long blocks_used;
	long long blocks_percent_used;
#else
	long blocks_used;
	...
	opt = bb_getopt_ulflags(argc, argv, "hmkc");
	if(opt & 1) {
				df_disp_hr = 0;
				disp_units_hdr = "     Size";
	}
...
#ifdef CONFIG_FEATURE_HUMAN_READABLE
			if( opt & 8 ) {
			        bb_printf("%-21s %9s ", device, make_human_readable_str(s.f_blocks, s.f_bsize, df_disp_hr));
			} else {
