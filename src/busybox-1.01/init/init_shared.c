...
{
#ifdef QNAPNAS
	#define SYS_UPDATE_LOCK_FILE    "/tmp/nas_system.lock"
	{
	int lock_fd = open(SYS_UPDATE_LOCK_FILE, O_EXCL|O_CREAT, S_IRWXU|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH);
	if (lock_fd == -1) {
		//update F/W or other reboot, poweroff, shutdown, halt is running
		return -1;
	}
	close(lock_fd);
	}
