...
static const char default_home_prefix[] = "/share/homes";
...
/* putpwent(3) remix */
//static int adduser(const char *filename, struct passwd *p, int makehome, int setpass)//modified by jeffrey 2005/09/22
static int adduser(const char *filename, struct passwd *p, int makehome, int setpass, const char *userpwd)
{
	FILE *passwd;
	int r;
#ifdef CONFIG_FEATURE_SHADOWPASSWDS
	...
	if (shadow_enabled) {
		...
		sp->sp_warn = 7;
		//modified by jeffrey 2005/09/22
		if (userpwd) {
			sp->sp_pwdp=userpwd;//add
			fprintf(shadow, "%s:%s:%ld:%ld:%ld:%ld:::\n",
				sp->sp_namp, crypt(sp->sp_pwdp, "$1$"), sp->sp_lstchg, sp->sp_min, sp->sp_max,
				sp->sp_warn);
		}
		else {
	...
	//if (setpass) {	//modified by jeffrey 2005/09/22
	if (setpass && userpwd==NULL) {
...
//modified by jeffrey 2005/09/21
//#define SETPASS				(1 << 4)
//#define MAKEHOME			(1 << 6)
#define SETPASS				(1 << 6)
#define MAKEHOME			(1 << 8)
...
int adduser_main(int argc, char **argv)
{
	...
	/* get args */
	//flags = bb_getopt_ulflags(argc, argv, "h:g:s:G:DSH", &home, &gecos, &shell, &usegroup);	//modified by jeffrey 2005/09/20
	flags = bb_getopt_ulflags(argc, argv, "h:g:s:u:p:G:DSH", &home, &gecos, &shell, &uid, &pwd, &usegroup);
	...
	/* grand finale */
	//return adduser(bb_path_passwd_file, &pw, makehome, setpass);	//modified by jeffrey 2005/09/20
	return adduser(bb_path_passwd_file, &pw, makehome, setpass, pwd);
}
