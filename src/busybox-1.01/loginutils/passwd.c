		...
		if (!continued) {		// Check to see if we're updating this line.
			if (strncmp(username, buffer, strlen(username)) == 0) {	// we have a match.
				...
				*pw_rest++ = '\0';
				...
				if(update_chage_time){
					pw_rest++;
					pwlct_rest = strchr(pw_rest, ':');
					fprintf(out_fp, "%s:%s:%ld%s", buffer, crypt_pw, (long)time((time_t *)0)/SCALE, pwlct_rest);
				}else{
...
extern int update_gecos(const struct passwd *pw, char *gecos)
{
	char filename[1024];
	char buf[1025];
	char buffer[256];
		...
		if (!continued) {		// Check to see if we're updating this line.
			if (strncmp(username, buffer, strlen(username)) == 0) {	// we have a match.
				...
				*pw_rest++ = '\0';
				...
				pw_rest = strchr(pw_rest, ':');
				//:
				fprintf(out_fp, "%s:%s%s", buffer, gecos, pw_rest);
			} else {
				fputs(buffer, out_fp);
			}
		} else {
...
extern int passwd_main(int argc, char **argv)
{
	...
	char *name;
	char *myname;
	//add by jeffrey 2005/09/22
	char tmp[256]="";
 	char *pwd = NULL;
	int pflg = 0;				/* -p - change password */
	//end
	int flag;
	...
	//while ((flag = getopt(argc, argv, "a:dlu")) != EOF) {//modified by jeffrey 2005/09/22
	while ((flag = getopt(argc, argv, "a:p:dlug:S")) != EOF) {
		switch (flag) {
		case 'a':
	...
	//add by jeffrey 2005/09/22
	if (pflg) {
		pwd = pw_encrypt(tmp, "$1$");
		safe_strncpy(crypt_passwd, pwd, sizeof(crypt_passwd));
		update_passwd(pw, crypt_passwd);
		return (0);
	}
	if (gflg) {
		update_gecos(pw, tmp);
