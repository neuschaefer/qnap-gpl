 ...
 *
 * $Id: ifconfig.c,v 1.5 2014/02/15 06:58:41 justinchen Exp $
 *
 */
...
int ifconfig_main(int argc, char **argv)
{
	...
	while (*++argv != (char *) NULL) {
	  ...
	  FOUND_ARG:
        ...
        {
            // command success
            if (op->name && strcmp(op->name, "up") == 0)
            {
                if (strncmp(ifr.ifr_name, "eth", strlen("eth")) == 0)
                {
                    strcpy(file_name, "/tmp/.");
                    strcat(file_name, ifr.ifr_name);
            ...
            else if (op->name && strcmp(op->name, "down") == 0)
            {
                if (strncmp(ifr.ifr_name, "eth", strlen("eth")) == 0)
                {
                    // get parent process cmd

                    ppid = getppid();
                    sprintf(file_name, "/proc/%d/cmdline", ppid);
