...
#define QNAP_WIDTH_OPTIONS                 /* Benjamin 20121226 for BUG 29611: terminal width issue. */
...
#endif
//added by KenChen
	if(argv[1] && !strcmp(argv[1], "--showup") )
		showup = 1;
//end here
#ifdef QNAP_WIDTH_OPTIONS
	if(argc == 3 && argv[1] && argv[2] && !strcmp(argv[1], "--columns") ) {
		columns = strtol(argv[2], NULL, 0);
	...
	while ((p = procps_scan(1)) != 0) {
		...
		i = 0;
			if(strlen(namecmd) > i)
				namecmd[i] = 0;
//Modified by KenChen for hidding cryptsetup smd (ENCRYPTFS)
		if (!showup){
			if (strstr(namecmd, "cryptsetup"))
