...
int dd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	static const char keywords[] ALIGN1 =
		"bs\0""count\0""seek\0""skip\0""if\0""of\0""progress\0"
...
#ifdef QNAPNAS
#define progressfile (Z.progressfile)
#endif
	memset(&Z, 0, sizeof(Z));
	INIT_G();
...
#ifdef QNAPNAS
		if(progressfile && count>0){
			dd_output_progress(progressfile, (long)(G.in_full + G.in_part),(long)count, QSTAT_ONGOING);
		}
#endif
	}
...
#ifdef QNAPNAS
		if(progressfile && count>0){
			dd_output_progress(progressfile, (long)(G.in_full + G.in_part),(long)count, QIF_ERR);
		}
#endif
...
#ifdef QNAPNAS
		if(progressfile && count>0){
			dd_output_progress(progressfile, (long)(G.in_full + G.in_part),(long)count, QOF_ERR);
		}
#endif
...
#ifdef QNAPNAS
		if(progressfile && count>0){
			dd_output_progress(progressfile, (long)(G.in_full + G.in_part),(long)count, QSTAT_FIN);
		}
#endif
	dd_output_status(0);
