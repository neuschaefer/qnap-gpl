...
* $Id: centerim.cc,v 1.4 2011/05/02 02:11:45 ken Exp $
*
...
void centerim::handlesignal(int signum) {
	...
	case SIGUSR2:
	    clist.load();
           //deleteall and then reload modelist, by QNAP
            lst.deleteall();
            lst.load();
	    break;

	case SIGALRM:
		cicq.getCurrentConnectionState(msn);
	    break;
...
/*
    ...
    imstatus_size
*/
void centerim::getCurrentConnectionState(protocolname pname){
    imstatus stcurrent;
	string cmd = "/sbin/setcfg msn status %d -f /etc/config/centerim/default.conf";
	abstracthook &hook = gethook(pname);
	if(hook.isconnecting())
		system("/sbin/setcfg msn status 99 -f /etc/config/centerim/default.conf");
	else if(hook.logged())
		system("/sbin/setcfg msn status 1 -f /etc/config/centerim/default.conf");
	else
		system("/sbin/setcfg msn status 0 -f /etc/config/centerim/default.conf");
}
