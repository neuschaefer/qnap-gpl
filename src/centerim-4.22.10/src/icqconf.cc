...
* $Id: icqconf.cc,v 1.3 2011/05/02 02:10:40 ken Exp $
*
...
void icqconf::queryState(char st, const string &proto) const {
    protocolname pname;
	imstatus imst = offline;
	for(pname = icq; pname != protocolname_size; pname++)
	    if(getprotocolname(pname) == proto)
		break;
...
void icqconf::usage() const {
    ...
    cout << _("  --state update current connection status (99-connecting, 1-connected, 0-offline)") << endl;
