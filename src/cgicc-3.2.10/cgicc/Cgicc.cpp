...
/*
 *  $Id: Cgicc.cpp,v 1.1.1.1 2014/03/25 08:01:38 aquavitwu Exp $
 *
...
cgicc::Cgicc::parseFormInput(const std::string& data, const std::string &content_type)
{
  ...
  // File upload type = multipart/form-data
	...
	std::string boundary = extractBetween(data, "--", "\r\n");
	
    // generate the separators
    std::string sep1;
	sep1 = boundary.empty() ? content_type.substr(pos + bType.length()) : boundary;
    sep1.append("\r\n");
    sep1.insert(0, "--");

    std::string sep2;
	sep2 = boundary.empty() ? content_type.substr(pos + bType.length()) : boundary;
    sep2.append("--\r\n");
    sep2.insert(0, "--");
