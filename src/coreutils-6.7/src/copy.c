  ...
  if (! (S_ISREG (src_open_sb.st_mode) && src_open_sb.st_size == 0))
      ...
      if (! make_holes)
	{
	  /* These days there's no point ever messing with buffers smaller
		...
		/*
		 * QNAP-Spec#9692: We change the buf_size from 8KB to 128KB
		 * to improve the cp performance.
		 */
		enum { SMALL_BUF_SIZE = 128 * 1024 };
  ...
  if (x->preserve_timestamps)
    {
      ...
      if (futimens_ex (dest_desc, dst_name, timespec) != 0)
	{
