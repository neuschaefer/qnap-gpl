			   ...
			   const struct volume_key *vk)
{
	...
	log_dbg("[HK]magic: %s", hdr->magic);
	log_dbg("[HK]version: %d", hdr->version);
	log_dbg("[HK]cipherName: %s", hdr->cipherName);
	log_dbg("[HK]cipherMode: %s", hdr->cipherMode);
	log_dbg("[HK]hashSpec: %s", hdr->hashSpec);
	log_dbg("[HK]payloadOffset: %d", hdr->payloadOffset);
	log_dbg("[HK]keyBytes: %d", hdr->keyBytes);
	log_dbg("[HK]mkDigest: %s", hdr->mkDigest);
	log_dbg("[HK]mkDigestSalt: %s", hdr->mkDigestSalt);
	log_dbg("[HK]mkDigestIterations: %d", hdr->mkDigestIterations);
	log_dbg("[HK]uuid: %s", hdr->uuid);
	log_dbg("[HK]_padding: %s", hdr->_padding);

	log_dbg("[HK]vk->key: %s", vk->key);
	log_dbg("[HK]vk->keylength: %d", vk->keylength);

	log_dbg("[HK]checkHashBuf: %s", checkHashBuf);	
		  ...
		  struct crypt_device *ctx)
{
	...
	log_dbg("[HK]LUKS_decrypt_from_storage() return %d", r);
	...
	log_dbg("[HK]AF_merge() return %d", r);
	...
	log_dbg("[HK]LUKS_verify_volume_key() return %d", r);
