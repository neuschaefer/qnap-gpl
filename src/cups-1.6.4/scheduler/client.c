/*
 * "$Id: client.c,v 1.2 2013/10/15 07:30:10 aixchou Exp $"
 *
...
static int Get_Binding_Address(PT_BIND_ADDR ptBindAddr)
{
	...
	if (NULL == (pfAddr = popen(SZP_GET_BIND_ADDR " PRINTER 2>/dev/null", "r")))  return 0;
...
static int Check_Binding_Address(int fdAccept, PT_BIND_ADDR ptBindAddr, char *pszAddrBuf)
{
	...
	if (ptBindAddr->bEnabled)
	{
		...
		{
			sprintf(pszAddrBuf, "Family %d", tsaLocal.ss_family);
			iRet = 0;
		}
	}
...
cupsdAcceptClient(cupsd_listener_t *lis)/* I - Listener socket */
{
  ...
  */
                             ...
                             &addrlen)) < 0)
  {
	...
	{
		char  szAddrBuf[INET_ADDRSTRLEN + INET6_ADDRSTRLEN]="1234";
		if (g_bHup)
		{
			Get_Binding_Address(&g_tBindAddr);
			g_bHup = FALSE;
		}
		if (g_tBindAddr.bEnabled && (0 != Check_Binding_Address(con->http.fd, &g_tBindAddr, szAddrBuf)))
		{
			close(con->http.fd);
			free(con);
			return;
...
/*
 * End of "$Id: client.c,v 1.2 2013/10/15 07:30:10 aixchou Exp $".
 */
