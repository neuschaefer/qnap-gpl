...
#ifdef QNAPNAS
  // jeff modify 2010.7.9, we change folder by quote but not here.
  // jeff modify 2010.7.27, using a flag to prevent from changing original flow
  if((QNAP_IDAGENT_QSYNC == conn->data->set.idQnapAgent) && conn->data->set.bFTPNotChdir)
  	ftpc->cwddone = TRUE;
#endif //QNAPNAS
...
#ifdef QNAPNAS
  if(QNAP_IDAGENT_QSYNC == conn->data->set.idQnapAgent)
  {
  	cmd = aprintf( "%s%s%s",
                 data->set.str[STRING_CUSTOMREQUEST]?
                 data->set.str[STRING_CUSTOMREQUEST]:
                 (data->set.bMLSDSupp? "MLSD" : (data->set.ftp_list_only?"NLST":"LIST")),
                 lstArg? " ": "",
                 lstArg? lstArg: "" );
  }
  else
                                    ...
                                    ftpstate instate)
{
  ...
  if((QNAP_IDAGENT_QSYNC == conn->data->set.idQnapAgent) && conn->data->set.bFTPQuoteOnly)
    return CURLE_OK; 
#endif //QNAPNAS
...
#ifdef QNAPNAS
		// jeff modify 2010.7.10, for query one file state reading "426 connection closed"	 
		// to prevent from recieving this message at next command.  
		if(QNAP_IDAGENT_QSYNC == conn->data->set.idQnapAgent)
...
static CURLcode ftp_statemach_act(struct connectdata *conn)
{
  ...
  if(ftpcode) {
	...
	if(QNAP_IDAGENT_QSYNC == conn->data->set.idQnapAgent)
	{
		// jeff modify 2010.6.26, for after quote error we can't close the command socket
		if((ftpcode >= 400) && !ftpc->count2) {
			/* failure reponse code, and not allowed to fail */
			failf(conn->data, "QUOT command failed with %03d", ftpcode);
			return CURLE_REMOTE_ACCESS_DENIED;
	      }
		// jeff modify 2010.7.14, if quote, we don't go into passive mode.
		if(conn->data->set.bFTPQuoteOnly)
		{
			state(conn, FTP_STOP);
			return CURLE_OK;
		}
                              ...
                              bool premature)
{
  ...
  default:       /* by default, an error means the control connection is
	...
	#ifdef QNAPNAS
		if(QNAP_IDAGENT_QSYNC == conn->data->set.idQnapAgent)
			if(data->set.pbReconn)
				*data->set.pbReconn = TRUE;
	#endif //QNAPNAS
  ...
  if((ftp->transfer == FTPTRANSFER_BODY) && ftpc->ctl_valid &&
    ...
    if(!ftpc->dont_check) {
		// jeff modify 2010.7.10, for NOOP command and server response 200 should be ok!	
      /* 226 Transfer complete, 250 Requested file action okay, completed. */
#ifdef QNAPNAS
	if(QNAP_IDAGENT_QSYNC == conn->data->set.idQnapAgent)
	{
		 if((ftpcode != 226) && (ftpcode != 250) && (ftpcode != 200)) {
...
#ifdef QNAPNAS
  // jeff modify 2010.7.14, if quote, we receive response but do nothing others  
  if(QNAP_IDAGENT_QSYNC == conn->data->set.idQnapAgent)
  {
    if(connected && !conn->data->set.bFTPQuoteOnly)
...
#ifdef QNAPNAS
	// jeff modify 2010.6.28, for receiving message when close data socket, if we don't
	// do this, we will receive "close connect" at next request.
    curl_socket_t sock = conn->sock[FIRSTSOCKET];
    int ftpcode;
    struct pingpong *pp = &ftpc->pp;
    size_t nread = 0;
    if(QNAP_IDAGENT_QSYNC == conn->data->set.idQnapAgent)
      ftp_readresp(sock, pp, &ftpcode, &nread);
#endif //QNAPNAS
