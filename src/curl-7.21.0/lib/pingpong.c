...
#ifdef QNAPNAS
	// jeff modify 2011.1.7, for calculating command response lag time out
	if(-1 != rc)
	{
		if (QNAP_IDAGENT_QSYNC == data->set.idQnapAgent)
		{
			...
			{
				result = data->set.ptTransLagTime->pfParserCallback(NULL, rc, data->set.ptTransLagTime->pParam1, data->set.ptTransLagTime->pParam2);
...
#ifdef QNAPNAS
	// jeff modify 12/10, for clone command response data from server.
	if ((0 < gotbytes) && (QNAP_IDAGENT_QSYNC == pp->conn->data->set.idQnapAgent) && (NULL != pp->conn->data->set.ptParserCmd))
	{
		pp->conn->data->set.ptParserCmd->pfParserCallback(ptr, gotbytes, pp->conn->data->set.ptParserCmd->pParam1, pp->conn->data->set.ptParserCmd->pParam2);
	}
#endif
