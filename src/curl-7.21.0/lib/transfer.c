  ...
  *done = FALSE;
  ...
  do {
    ...
    *didwhat |= KEEP_RECV;
	...
	if ((QNAP_IDAGENT_QSYNC == data->set.idQnapAgent) && (data->state.proto.ftp->transfer == FTPTRANSFER_BODY) && 
		(NULL != data->set.ptParserData))
	{
		if(FALSE == is_empty_data)
		{
			if(0 < nread)
			{
				data->set.ptParserData->pfParserCallback(k->str, nread, data->set.ptParserData->pParam1, data->set.ptParserData->pParam2);
			}
		}
		// jeff modify 2010.8.29, if we set ptParserData NULL and data don't have received yet, the second receive parse will not be processed.
		else
			data->set.ptParserData = NULL;
	}
#endif //QNAPNAS
	...
	if((QNAP_IDAGENT_QSYNC == data->set.idQnapAgent) && data->set.pcbDownloadByte)
		*data->set.pcbDownloadByte += nread;
#endif //QNAPNAS
...
#ifdef QNAPNAS
	// jeff modify 2010.8.23, for qsync progress statistic
	if((QNAP_IDAGENT_QSYNC == data->set.idQnapAgent) && data->set.pcbUploadByte)
		*data->set.pcbUploadByte += bytes_written;
#endif //QNAPNAS
                        ...
                        bool *done)
{
  ...
  if(select_res == CURL_CSELECT_ERR) {
	...
	if ((QNAP_IDAGENT_QSYNC == data->set.idQnapAgent) && (data->state.proto.ftp->transfer == FTPTRANSFER_BODY)
		&& data->set.ptvTransTime && (FALSE == *done))
	{
		if((0 == k->writebytecount) && (0 == k->bytecount))
			gettimeofday(data->set.ptvTransTime, NULL);
	}
#endif //QNAPNAS
  ...
  *done = (bool)(0 == (k->keepon&(KEEP_RECV|KEEP_SEND|
	...
	// jeff modify 2010.9.1, for net speed
	if ((QNAP_IDAGENT_QSYNC == data->set.idQnapAgent) && (data->state.proto.ftp->transfer == FTPTRANSFER_BODY)
		&& data->set.ptvTransTime && (TRUE == *done))
	{
		struct timeval tvEnd;
		gettimeofday(&tvEnd, NULL);

		if((0 < tvEnd.tv_sec - data->set.ptvTransTime->tv_sec) && (0 > tvEnd.tv_usec - data->set.ptvTransTime->tv_usec))
		{
			data->set.ptvTransTime->tv_sec = tvEnd.tv_sec - data->set.ptvTransTime->tv_sec - 1;
			data->set.ptvTransTime->tv_usec = 1000000 - data->set.ptvTransTime->tv_usec + tvEnd.tv_usec;
		}
		else
		{
			data->set.ptvTransTime->tv_sec = tvEnd.tv_sec - data->set.ptvTransTime->tv_sec;
			data->set.ptvTransTime->tv_usec = tvEnd.tv_usec - data->set.ptvTransTime->tv_usec;
		}
	}
#endif //QNAPNAS
  return CURLE_OK;
...
Transfer(struct connectdata *conn)
{
  ...
  while(!done) {
...
#ifdef QNAPNAS
    iRet = Curl_socket_ready(fd_read, fd_write, timeout_ms);
    switch(iRet) {
#else
	...
	{
		if(data->set.ptTransLagTime)
		{
			iRet = data->set.ptTransLagTime->pfParserCallback(NULL, iRet, data->set.ptTransLagTime->pParam1, data->set.ptTransLagTime->pParam2);
			if(CURLE_OPERATION_TIMEDOUT == iRet)
				return iRet;
		}
	}
#endif //QNAPNAS
  ...
  for(;;) {
	...
	// curl_do ok
      ...
      if(res == CURLE_OK) {
		...
		if((QNAP_IDAGENT_QSYNC == data->set.idQnapAgent) && data->set.bFTPQuoteOnly)
		{
			Curl_done(&conn, res, FALSE);
			break;
		}
#endif //QNAPNAS
#ifdef QNAPNAS
