...
#ifdef QNAPNAS
  // jeff modify 2010.7.27, initial data value  
  set->bFTPQuoteOnly = FALSE;  
  set->ptParserCmd = NULL;
  set->ptParserData = NULL;
  set->ptTransLagTime = NULL;
  // we can't change dir inside libcurl, cause's we should record working dir in qsync  
  set->bFTPNotChdir = TRUE; 
  set->pcbDownloadByte = NULL;
  set->pcbUploadByte = NULL;
  // jeff modify 2010.9.1, for net speed
  set->ptvTransTime = NULL;
  // jeff modify 2010.12.8, for SIGINT/SIGTERM
  set->pbAbort = NULL;
  // jeff modify 2010.7.29, control libcurl program flow  
  // default curl flow  
  set->idQnapAgent = 0;
  // jeff modify 2012.6.18, Let qsync know that curl reconnects and need to change working dir.
  set->pbReconn = NULL;
#endif //QNAPNAS
  return res;
}
...
#ifdef QNAPNAS
  // jeff modify 2010.7.9, for quote only, cause' we don't recieve data from data socket, and don't have to move to passive mode
  case CURLOPT_FTPQUOTEONLY:
	data->set.bFTPQuoteOnly = va_arg(param, long);
	break;
  // jeff modify 2011.3.9, flag of MLSD command supporting
  case CURLOPT_FTPMLSDSUPP:
	data->set.bMLSDSupp = va_arg(param, long);
	break;
   // jeff modify 2010.8.6, we prevent from always allocating memory when dump socket data, but use callback function and call it 
   // in ftp library. it's not only that there isn't any api added to ftp library, but also save memory space. and then also, we save 
   // much time cause's sync each file would change modify time once!
  case CURLOPT_FTPSETDATASOCKET:
	data->set.ptParserData =  va_arg(param, PT_PARSER_OBJECT);
	break;
  case CURLOPT_FTPSETCOMSOCKET:
  	data->set.ptParserCmd =  va_arg(param, PT_PARSER_OBJECT);
	break;
  // jeff modify 2010.10.7, for calculating data translation lag time out
  case CURLOPT_FTPSETTRANSLAG:
  	data->set.ptTransLagTime =  va_arg(param, PT_PARSER_OBJECT);
	break;
  // jeff modify 2010.8.23, for qsync progress statistic
  case CURLOPT_FTPUPLOADBYTE:
  	data->set.pcbUploadByte = va_arg(param, uint64_t*);
	break;
  case CURLOPT_FTPDOWNLOADBYTE:
  	data->set.pcbDownloadByte = va_arg(param, uint64_t*);
	break;
  // jeff modify 2010.9.1, for net speed
  case CURLOPT_FTPTRANSTIME:
  	data->set.ptvTransTime= va_arg(param, struct timeval*);
	break;
  // jeff modify 2010.12.8, for SIGINT/SIGTERM
  case CURLOPT_FTPABORT:
    data->set.pbAbort= va_arg(param, long*);
    break;
  // jeff modify 2010.7.29, control libcurl program flow
  case CURLOPT_QNAPAGENT:
  	data->set.idQnapAgent = va_arg(param, long);
	break;
  case CURLOPT_FTPRECONN:
  	data->set.pbReconn = va_arg(param, long*);
	break;
#endif //QNAPNAS
...
#ifdef QNAPNAS
  // jeff modify 2010.7.12, we don't use http, and nonreal time qsync will write a temp file ~$%#[000-999] before overwrite remote file.
  if(QNAP_IDAGENT_QSYNC != data->set.idQnapAgent)
#endif //QNAPNAS  	
{
                   ...
                   bool premature)
{
  ...
  struct SessionHandle *data;
#ifdef QNAPNAS	
//  int task_stat = HTTP_FTP_TASK_STATE_FINISHED;
#endif
...
#ifdef QNAPNAS
//  if (QNAP_IDAGENT_BT == data->set.idQnapAgent) {
//	http_ftp_db_update_task_field_by_pid(getpid(), HTTP_FTP_TASK_TBL_FIELD_STATE, &task_stat);
//	system("/bin/echo Curl_done >> /cdebug");
//  }
#endif	
  return result;				
}
