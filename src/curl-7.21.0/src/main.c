...
#ifdef QNAPNAS
  int idQnapAgent; /* control libcurl program flow */
  curl_off_t upload_filesize;
#endif
};
...
#ifdef QNAPNAS
    "    --upload-file-size <bytes> Maximum file size to upload",
#endif
  ...
  do {
    ...
    *usedarg = FALSE; /* default is that we don't use the arg */
...
#ifdef QNAPNAS
		/* control libcurl program flow */
		switch(subletter)
		{
		  case 'a':
			if(str2num(&config->idQnapAgent, nextarg))
				return PARAM_BAD_NUMERIC;
			//config->idQnapAgent = atoi(nextarg);
			break;
		  default:
			break;
		}
    ...
    case 'T':
      ...
      switch(subletter) {
#ifdef QNAPNAS
        case 's': /* upload file size */
          if(str2offset(&config->upload_filesize, nextarg))
            return PARAM_BAD_NUMERIC;
          break;
#endif
        default:
...
#ifdef QNAPNAS
    //if is downlaod station overwrite existing files
    if ((config->content_disposition) && (QNAP_IDAGENT_BT != config->idQnapAgent)) {
#else
...
#ifdef QNAPNAS
    if (QNAP_IDAGENT_BT == config->idQnapAgent) {
	pid_t curpid = getpid();
	if (get_write_db_fork()) {
		if (write_db_lock(curpid) == 0) {
			if (fork() == 0) {
				http_ftp_db_update_task_field_by_pid(curpid, HTTP_FTP_TASK_TBL_FIELD_SAVENAME, out->filename);
				write_db_unlock(curpid);
				exit(0);
			}
		}
	}
...
#ifdef QNAPNAS
int _getcfg_str(char* section, char* field, char* defVal, char* retVal, int size, char* file)
{
        FILE *fp = NULL;
        char cmd[2048], buffer[1024];
        if (!section || !field || !defVal || !size || !file)
                return -1;
        sprintf(cmd, "/sbin/getcfg \"%s\" \"%s\" -d \"%s\" -f \"%s\"", section, field, defVal, file);
        fp = popen(cmd, "r");
        if (!fp)
                return -1;
        else{
                if (fgets(buffer, sizeof(buffer), fp) && strlen(buffer)>0 && strlen(buffer)<size){
                        int len=strlen(buffer);
                        buffer[len-1] = '\0';
                        strncpy(retVal,buffer,size);
                }
                else {
                        strncpy(retVal,defVal,size-1);
                        retVal[size] = '\0';
                }
                pclose(fp);
        }
...
#ifdef QNAPNAS
  config->idQnapAgent = 0;

  // default proxy 
  char tmpval[1024],tmpuser[128],tmppassword[128];
  _getcfg_str("System", "proxy server", "", tmpval, 1023, "/etc/config/uLinux.conf");
  if(strcmp(tmpval, "")){
      GetStr(&config->proxy, tmpval);
      config->proxyver = CURLPROXY_HTTP;

      _getcfg_str("System", "proxy username", "", tmpuser, 127, "/etc/config/uLinux.conf");
      _getcfg_str("System", "proxy password", "", tmppassword, 127, "/etc/config/uLinux.conf");

      if(strcmp(tmpuser, "")!= 0){
	snprintf(tmpval, sizeof(tmpval), "%s:%s", tmpuser, tmppassword);
	GetStr(&config->proxyuserpwd, tmpval);
	checkpasswd("proxy", &config->proxyuserpwd);
      }
  }
  _getcfg_str("System", "ForkWriteDB", "0", tmpval, 1023, "/etc/config/uLinux.conf");
  if (atoi(tmpval) == 1) set_write_db_fork(1);
#endif
    ...
    for(up = 0;
      ...
      for(i = 0;
...
#ifdef QNAPNAS
          if(config->upload_filesize && config->upload_filesize < uploadfilesize)
            uploadfilesize = config->upload_filesize;
#endif
...
#ifdef QNAPNAS
		if (config->idQnapAgent)
          my_setopt(curl, CURLOPT_QNAPAGENT, config->idQnapAgent);
#endif
		...
		if (get_write_db_fork()) {
			if (write_db_lock(curpid) == 0) {
				if (fork() == 0) {
					http_ftp_db_update_task_field_by_pid(curpid, HTTP_FTP_TASK_TBL_FIELD_SAVENAME, outs.filename);
					write_db_unlock(curpid);
					exit(0);
				}
			}
          ...
          if(retry_numretries &&
            ...
            if(retry) {
              ...
              if(outs.bytes && outs.filename) {
                ...
                /* QNAP: don't throw away downloaded data */
                struct_stat fileinfo;
                if(0 == fstat(fileno(outs.stream), &fileinfo)) {
                  my_setopt(curl, CURLOPT_RESUME_FROM_LARGE, fileinfo.st_size);
...
#ifdef QNAPNAS
  if (QNAP_IDAGENT_BT == config->idQnapAgent) {
    http_ftp_db_update_task_field_by_pid(getpid(), HTTP_FTP_TASK_TBL_FIELD_ERRCODE, &res);
  }
#endif
  return res;
}
