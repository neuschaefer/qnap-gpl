...
"$Id: db.c,v 1.1.1.1 2008/01/15 08:14:51 jimmychang Exp $ Copyright (c) 2004-2005 Internet Systems Consortium.  All rights reserved.\n";
#endif /* not lint */
...
#define         DHCP_LOG                "/etc/config/dhcp_log"
        ...
        if (fp)
        {
                ...
                if (tmpfp_bak)
                {
                        ...
                        if (tmpfp)
                        {
                                ...
                                {
                                        if (!strstr(s, print_hw_addr (c_lease->hardware_addr.hbuf[0],
                                                c_lease->hardware_addr.hlen-1,
                                                &c_lease->hardware_addr.hbuf[1])))
                                                fputs (s, tmpfp);
...
{
	struct tm *t;
	char tbuf [64], starts_time[64], end_time[64];
	int errors = 0;
	int i;
	struct binding *b;
	char *s;
	...
	if (lease -> starts) {
		...
		//  added by Tiger
		sprintf (starts_time, "%d/%02d/%02d-%02d:%02d:%02d",
				t->tm_year + 1900,
				t->tm_mon + 1, t->tm_mday,
				t->tm_hour, t->tm_min, t->tm_sec);
		//  until here
...
//  added by Tiger
	{
		char mac_addr[32];
//		note ("lease->flags = %d", lease->flags);
//		note ("lease->packet_type = %d", lease->packet_type);
//		note ("MAC addres = %s", print_hw_addr (lease->hardware_addr.hbuf[0],
//					lease->hardware_addr.hlen-1,
//					&lease->hardware_addr.hbuf[1]));
		if (lease->ends != 0)
		{
			sprintf (mac_addr, "%s", print_hw_addr (lease->hardware_addr.hbuf[0],
						lease->hardware_addr.hlen-1,
						&lease->hardware_addr.hbuf[1]));
			if (strlen (mac_addr) == 17)
			{
				add_lease_to_log (lease, starts_time, end_time);
			}
		}
	}
// until here
