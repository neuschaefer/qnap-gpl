...
//#fix QNAP bug #37131
  ...
  int buf_size;
  int size;
  unsigned int pkt_busrt_cnt; 
//END of #fix QNAP bug #37131
...
//#fix QNAP bug #37131
	  pkt_busrt_cnt += len;
	  if(pkt_busrt_cnt > MAX_PKT_BURST_COUNT)
	    {
      	      int tmp_cnt;
      	      tmp_cnt = 0;
      	      size = sizeof(buf_size);
      	      getsockopt(dhcpSocket, SOL_SOCKET, SO_RCVBUF, &buf_size, &size); 
      	      buf_bak = buf_size;
      	      buf_size = 0x0;  
      	      setsockopt(dhcpSocket, SOL_SOCKET, SO_RCVBUF, &buf_size, sizeof(buf_size));
      	      while ( peekfd(dhcpSocket, 1) == 0 )
      	        {
		  ...
		  if ( ++tmp_cnt > 2000)
      	            { 
		      syslog(LOG_ERR,"QNAP dhcpcd error #1 recvfrom: error\n");
      	              break;
		    }
      	        } 
      	      usleep(timeout);  
      	      setsockopt(dhcpSocket, SOL_SOCKET, SO_RCVBUF, &buf_bak, sizeof(buf_bak));
      	      break;
	    }
//END of #fix QNAP bug #37131	  
...
#endif
  /* not to bring interface down, fix for eth0 disppear
