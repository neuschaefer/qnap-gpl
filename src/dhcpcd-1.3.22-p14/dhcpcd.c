...
struct rfc2132_param_list  rfc2132;
...
prgs: switch ( argc[i][s] )
	{
	  ...
	  case 'o': {
		...
		if(buf) {
			char *p = NULL, *delim = ", \t";
			for(p = strtok(buf, delim);
				p; 
				p = strtok(NULL, delim)) {
				rfc2132.options[rfc2132.num++] = atoi(p);
			}
			free(buf);
		}
		s = 1;
...
"Usage: dhcpcd [-dknrBCDHNRSTY] [-l leasetime] [-h hostname] [-t timeout]\n\
       ...
       [-o {[rfc2132 opt],[rfc2132 opt],[150] ....}]\n"
#else /* ! RFC2132_OPTIONS */
"Usage: dhcpcd [-dknrBCDHNRSTY] [-l leasetime] [-h hostname] [-t timeout]\n\
       [-i vendorClassID] [-I ClientID] [-c filename] [-s [ipaddr]]\n\
       [-w windowsize] [-L ConfigDir] [-G [gateway]] [interface]\n"
#endif /* RFC2132_OPTIONS */
  ...
  if ( TestCase ) alarm(TimeOut); //modify by richardchang is not test to disable timeout
  do
