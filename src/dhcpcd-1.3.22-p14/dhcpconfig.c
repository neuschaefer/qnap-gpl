...
int dhcpConfig()
{
  ...
  arpInform();
  // add by Kent 2008/02/27
  // After get IP from DHCP server, maybe need to run something...
  // modify by richardchang is not test to do ipchange_notify.sh
  
  if ( ! TestCase )
  {
    char cmd_buf[128];
    snprintf(cmd_buf, sizeof(cmd_buf), "/etc/init.d/ipchange_notify.sh %s >&/dev/null &", IfNameExt);
    system(cmd_buf);
  }
  // end
  if ( DebugFlag )
  ...
  if ( ReplResolvConf )
	...
	int i;
	char dns_type[30];
	FILE *pipe = popen("/sbin/getcfg  Network  'DNS type'  -f /etc/config/uLinux.conf -d 'auto'", "r");
	char default_gw[30], ifname_chk[30];
	FILE *pipe2; 
	char *Ifname_tmp=NULL, delim[3] = " .", *tmp = NULL;
	int Ifname_tmp_len;
	if(pipe == NULL)
		syslog(LOG_ERR," '/etc/config/uLinux.conf' open error.\n");
	else{
		fgets(dns_type,sizeof(dns_type),pipe);
		pclose(pipe); // close old pipe then open a new pipe
		if(!strcmp(dns_type, "auto\n")){ //If dns_type was auto.
			pipe2 = popen("/sbin/getcfg  Network  'Default GW Device' -f /etc/config/uLinux.conf -d 'eth0'", "r");
			fgets(default_gw,sizeof(default_gw),pipe2);
			pclose(pipe2);
			strcpy(ifname_chk, IfName);
			Ifname_tmp = strtok_r(ifname_chk, delim, &tmp);
			Ifname_tmp_len = strlen(ifname_chk);
			if(!strncmp(default_gw, Ifname_tmp,Ifname_tmp_len))
			{
		...
		else{ // If dns_type was manual.
			if ( ! TestCase ) system("/etc/init.d/network.sh setdns");
		}
		//pclose(pipe);
	}
...
/* Richard Chen 20090611, rfc2132 supports */
int len = 0;
if ( (len = DhcpOptions.len[tftpServerAddress])>=4 ) {
  if(len == 4) {
    fprintf(f,"\nTFTPSRVADDR=%u.%u.%u.%u",
    ((unsigned char *)DhcpOptions.val[tftpServerAddress])[0],
    ((unsigned char *)DhcpOptions.val[tftpServerAddress])[1],
    ((unsigned char *)DhcpOptions.val[tftpServerAddress])[2],
    ((unsigned char *)DhcpOptions.val[tftpServerAddress])[3]);
  } else {
	((unsigned char *)DhcpOptions.val[tftpServerAddress])[len-1] = 0;
	fprintf(f,"\nTFTPSRVADDR=%s", 
      (unsigned char *)DhcpOptions.val[tftpServerAddress]);
  }
}

if ( DhcpOptions.len[tftpServerName] )
	fprintf(f,"\nTFTPSRVNAME=\'%s\'",cleanmetas((char *)DhcpOptions.val[tftpServerName]));

if ( DhcpOptions.len[bootFileName] )
	fprintf(f,"\nBOOTFILENAME=\'%s\'",cleanmetas((char *)DhcpOptions.val[bootFileName]));
