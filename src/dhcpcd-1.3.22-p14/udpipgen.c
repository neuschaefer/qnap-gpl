...
/*****************************************************************************/
// QNAP_PATCH - addr will have alignment issue in linux 3.4.6
...
unsigned short sport,dport,msglen;
{
  ...
  udp->uh_sum = 0;
	// QNAP_PATCH - addr will have alignment issue in linux 3.4.6
	udp->uh_sum=in_cksum((unsigned char *)udpip,msglen+sizeof(udpiphdr)); // fix alignment problem
  ...
  // QNAP_PATCH - addr will have alignment issue in linux 3.4.6
  ip->ip_sum = in_cksum((unsigned char *)&ip_local,sizeof(struct ip)); // fix alignment problem
#else
  ip->ip_csum = 0;
  memcpy(udpip->ip,ip,sizeof(ip_local));
  // QNAP_PATCH - addr will have alignment issue in linux 3.4.6
  ip->ip_csum = in_cksum((unsigned char *)&ip_local,sizeof(struct ip)); // fix alignment problem
#endif
  memcpy(udpip->ip,ip,sizeof(ip_local));
}
/*****************************************************************************/
...
udpiphdr *udpip;
{
  ...
  hl = ip->ip_hl<<2;
	// QNAP_PATCH - addr will have alignment issue in linux 3.4.6
	if ( in_cksum((unsigned char *)udpip,hl) ) return -1; // fix alignment problem
  ...
  // QNAP_PATCH - addr will have alignment issue in linux 3.4.6
  if ( in_cksum((unsigned char *)nudpip,hl) ) return -2; // fix alignment problem
