...
#endif
#ifdef QNAP
extern char    blkdevice[MAXPATHLEN];
#endif
/*
 * make_hfs
 *	
...
ClearDisk(const DriveInfo *driveInfo, UInt64 startingSector, UInt32 numberOfSectors)
{
...
#ifdef QNAP
	UInt32 totalSectors = numberOfSectors;
	int log_progress = 0;
	int rate = 0;
	char cmd[512];
	char log_dev[32];

	if (startingSector == (UInt64)0) {
		log_progress = 1;
		char* tmp_ptr = strrchr(blkdevice,'/');
		if( tmp_ptr != NULL)
			snprintf(log_dev, sizeof(log_dev), "/dev%s", tmp_ptr);
		else
			snprintf(log_dev, sizeof(log_dev), "%s", blkdevice);
	}
#endif
...
#ifdef QNAP
		if (log_progress) {
			int cur_rate = 100 - (100*numberOfSectors)/totalSectors;
			if (cur_rate != rate) {
				rate = cur_rate;
				sprintf(cmd, "/sbin/setcfg Disk \"Format %s\" %d -f /tmp/format.status", log_dev,rate);
				system(cmd);
			}
		}
#endif
	...
	if (tempBuffer)
		free(tempBuffer);
#ifdef QNAP
	if (log_progress && rate != 100) {
		sprintf(cmd, "/sbin/setcfg Disk \"Format %s\" 100 -f /tmp/format.status", log_dev);
		system(cmd);
	}
#endif
}
