...
/* $Id: content_dir.c,v 1.2 2011/09/27 07:15:48 charley Exp $
 *
...
{
Log_Printf (LOG_WARNING, "%s(%p, %s, %s)\n", __FUNCTION__, cds, objectId, criteria);
	...
	*result = (ContentDir_Children) {
...
#ifdef QNAP
	int count = 0;
	int upper_limit = (g_max_item > 0) ? ((nb_matched < g_max_item) ? nb_matched:g_max_item) : nb_matched;
	while (PtrArray_GetSize (objects) < upper_limit) {
		Log_Printf (LOG_WARNING, 
			    "ContentDir_BrowseId ObjectId=%s : "
			    "got %d results, expected %d. count %d ...",
			    objectId, (int) PtrArray_GetSize (objects), 
			    (int) nb_matched, count++);
#else
	int nb_retry = 0;
	while (PtrArray_GetSize (objects) < nb_matched && nb_retry++ < 2) {
		...
		rc = BrowseOrSearchAction 
			(cds,
			 ...
			 /* requested_count => */ 
#ifdef QNAP
			 upper_limit - PtrArray_GetSize (objects),
#else
	...
	Log_Printf (LOG_WARNING, "The final return %d, nb_returned=%d", rc, nb_returned);
