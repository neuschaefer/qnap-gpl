...
/* $Id: device_list.c,v 1.3 2012/08/31 12:14:46 charley Exp $
 *
...
int g_max_error_url = MAX_ERROR_URL;
FATAL_URL *g_error_url_list = NULL;
int g_error_url_count = 0;
int g_download_count = 0;
int g_all_download_count = 0;
int g_error_list = 0;
int g_test_conn = 0;
int g_sync_add_device = 1;

static int __find_in_error_url_list(const char *url)
{
	int i;
	if (!g_error_url_list) {
		return -1;
	}
	for (i = 0; i < g_max_error_url; i ++) {
		if (!g_error_url_list[i].url) continue;
		if (strcmp(url, g_error_url_list[i].url) == 0) {
			return i;
		}
	}
	return -1;
...
static int __find_empty_in_error_url_list()
{
	int i;
	if (!g_error_url_list || g_error_url_count >= g_max_error_url) {
		return -1;
	}
	for (i = 0; i < g_max_error_url; i ++) {
		if (!g_error_url_list[i].url) return i;
	}
	return -1;
}
static int add_error_url_list(const char *url)
{
	int i;
	if (!g_error_url_list) {
		g_error_url_list = calloc(g_max_error_url, sizeof(FATAL_URL));
		if (!g_error_url_list) {
			return -1;
		}
		g_error_url_count = 0;
	}
	i = __find_in_error_url_list(url);
	if (i < 0) {
		i = __find_empty_in_error_url_list();
		if (i < 0) {
			printf("Can't add %s into error url list (over %d)\n", url, MAX_ERROR_URL);
			return -1;
		}
		g_error_url_list[i].url = strdup(url);
		g_error_url_list[i].err_count = 1;
		g_error_url_list[i].err_time = time(NULL);
		g_error_url_count ++;
	}
	else {
		g_error_url_list[i].err_count ++;
		g_error_url_list[i].err_time = time(NULL);
	}
	printf("Add %s into error url list (%d)\n", url, g_error_url_count);
	return 0;
}
static int in_error_url_list(const char *url)
{
	int i, in_list = 1;
	i = __find_in_error_url_list(url);
	if (i < 0) {
	...
	if (time(NULL)-g_error_url_list[i].err_time > 5*60) {
		free(g_error_url_list[i].url);
		g_error_url_list[i].url = NULL;
		g_error_url_list[i].err_count = 0;
		g_error_url_list[i].err_time = 0;
		in_list = 0;
	}
	return in_list;
}
static int connect_timeout(int sock, struct sockaddr *addr, int size_addr, int timeout)
{
        int error = 0;
        int     flags;
        fd_set  rset, wset;
...
static int is_ip_connectable(char *external_ip, int external_port)
{
        int fd, err;
        ...
        addr.sin_addr.s_addr = inet_addr(external_ip);
        addr.sin_port = htons(external_port);

        err = connect_timeout(fd, &addr, sizeof(addr), 1);
	close(fd);
        return err==0;
}
static int is_url_connectable(char *_url)
{
	if (g_error_list && in_error_url_list(_url)) return 0;	//FAIL
	   ...
	   int expires)
{
	...
	if (devnode) {
...
#ifdef QNAP
		if (g_error_list) ithread_mutex_lock (&DeviceListMutex);
		if (g_test_conn && !is_url_connectable(descLocation)) {
			if (g_error_list) add_error_url_list(descLocation);
			if (g_error_list) ithread_mutex_unlock (&DeviceListMutex);
			printf("test url failed[%s]\n", descLocation);
			return;
		}
		if (g_error_list) ithread_mutex_unlock (&DeviceListMutex);
#endif
		...
		if (rc != UPNP_E_SUCCESS) {
			...
			if (g_error_list) {
				ithread_mutex_lock (&DeviceListMutex);
				add_error_url_list(descLocation);
				ithread_mutex_unlock (&DeviceListMutex);
			}
#endif //QNAP
			return; // ---------->
		...
		*devnode = (struct _DeviceNode) { }; 
		...
		if (devnode->d == NULL) {
			...
			if (node) {
...
#ifdef QNAP
				char *addr = NULL;
				const char* presURL = Device_GetDescDocItem 
					(devnode->d, "presentationURL", true);
				const char* friendlyName = Device_GetDescDocItem 
					(devnode->d, "friendlyName", true);
				Log_Printf (LOG_DEBUG, "AddDevice([%s]%s)", NN(presURL), NN(friendlyName));
				if (presURL) {
					char *temp_addr = strdup(presURL);
					char *ptr = strstr(temp_addr, "http://");
					if (ptr) {
						ptr += 7;
						char *end = strchr(ptr, '/');
						if (end) *end = 0;
				...
				talloc_set_name (devnode->d, "[%s]%s", (addr)?addr:"", friendlyName);
				if (addr) free(addr);
#else
...
#ifdef QNAP  
void AddDeviceSync(void *data)
{
	if (!data) return ;
	SYNC_DATA *sdata = (SYNC_DATA *)data;

	AddDevice(sdata->did, sdata->loc, sdata->exp);
...
#ifdef QNAP  
			if (g_sync_add_device) {
				pthread_t th;
				int rc;
				SYNC_DATA *data = malloc(sizeof(SYNC_DATA));
				if (!data) break;
				data->did = strdup(e->DeviceId);
				data->loc = strdup(e->Location);
				data->exp = e->Expires;
				rc = pthread_create(&th, NULL, AddDeviceSync, (void *)data);
				if (rc != 0) {
					free(data);
					break;
				}
