...
/* $Id: fuse_main.c,v 1.3 2012/08/31 12:14:46 charley Exp $
 *
 * main FUSE interface.
...
static void smb_panic(const char *why)
{
	...
	if (backtrace_strings) {
		...
		for (i = 0; i < backtrace_size; i++)
			PRINTF(" #%u %s", i, backtrace_strings[i]);
...
static void fault_report(int sig)
{
	...
	PRINTF("===============================================================");
	PRINTF("INTERNAL ERROR: Signal %d in pid %d (%s)",sig,(int)getpid(),VERSION);
	PRINTF("===============================================================");
...
#ifdef QNAP
extern  int g_max_item;
extern  int g_error_list;
extern  int g_test_conn;
extern  int g_sync_add_device;
#endif
...
#endif
#if QNAP
     "    max_item=<number>    max number of items in one container (default: 1000)(0 is no limit)\n"
#endif
...
#ifdef QNAP
				} else if (strncmp(s, "max_item=", 9)
					   == 0) {
					g_max_item = atoi (s+9);
				} else if (strncmp(s, "error_list", 10)
					   == 0) {
					g_error_list = 1;
				} else if (strncmp(s, "test_conn", 9)
					   == 0) {
					g_test_conn = 1;
				} else if (strncmp(s, "sync_add", 8)
					   == 0) {
					g_sync_add_device = 1;
#endif
