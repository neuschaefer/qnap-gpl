...
#ifdef QNAP
char qnap_log_dev[32]="";
int qnap_progress = -1;
void init_qnap_progress(const char *dev)
{
	char* tmp_ptr = strrchr(dev,'/');
	if( tmp_ptr != NULL)
		snprintf(qnap_log_dev, sizeof(qnap_log_dev), "/dev%s", tmp_ptr);
	else
		snprintf(qnap_log_dev, sizeof(qnap_log_dev), "%s", dev);
}
int set_qnap_progress(int progress)
{
	char cmd[512];
	if (qnap_progress == progress)
		return progress;
	if (*qnap_log_dev == 0)
		return -1;
	qnap_progress = progress;
	sprintf(cmd, "/sbin/setcfg Disk \"Format %s\" %d -f /tmp/format.status", qnap_log_dev, progress);
	system(cmd);
	return progress;
}
#endif
...
#ifdef QNAP
	total_sectors = reserved_sectors+nr_fats*fat_length;
	for( x = 0; x < reserved_sectors; ++x ) {
		writebuf( blank_sector, sector_size, "reserved sector" );
		qx ++;
		set_qnap_progress(1+99*qx/total_sectors);
	}
#else
  for( x = 0; x < reserved_sectors; ++x )
    writebuf( blank_sector, sector_size, "reserved sector" );
...
#ifdef QNAP
	init_qnap_progress(device_name);
	set_qnap_progress(0);
#endif
