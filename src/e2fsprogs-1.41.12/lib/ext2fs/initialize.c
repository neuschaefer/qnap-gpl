...
#ifdef QNAP_SUPPORT_BIGGER_THAN_16TB
	/* 
	 * Benjamin 20110526: Reserve for the current filesystem size up to 128 TB anyway.
	 */
	struct ext2_super_block *sb = fs->super;
	unsigned long long bpg = sb->s_blocks_per_group;
	unsigned int gdpb = EXT2_DESC_PER_BLOCK(sb);
	unsigned long long max_blocks = BLK_CNT_128TB; // 128 TB has 34359738368 (4-KByte) blocks
	unsigned long long rsv_groups;
	unsigned long long rsv_gdb;

	/*
	 * ext2fs_div64_ceil() is necessary because max_blocks could be larger than 32 bits.
	 */
	rsv_groups = ext2fs_div_ceil(max_blocks - sb->s_first_data_block, bpg);
	rsv_gdb = ext2fs_div64_ceil(rsv_groups, gdpb) - fs->desc_blocks;

#ifdef RES_GDT_DEBUG
	printf("max_blocks %llu, rsv_groups = %llu, rsv_gdb = %llu, rsv_gdb(32bit)=%u\n",
	       max_blocks, rsv_groups, rsv_gdb, (unsigned int)rsv_gdb);
#endif
...
//Patch by QNAP:Search filename by case insensitive
    super->s_hash_magic = param->s_hash_magic;
////////////
...
#ifdef QNAP_SUPPORT_BIGGER_THAN_16TB    
    set_field(s_feature_qnap_compat, 0);
#endif     
...
retry:
	...
	if (super->s_feature_incompat & EXT4_FEATURE_INCOMPAT_64BIT &&
		...
		set_field(s_inodes_count, ext2fs_blocks_count(super) / i);  // Benjamin 20110526: TODO: check s_inodes_count
...
ipg_retry:
	 ...
	 */
#ifdef QNAP_SUPPORT_BIGGER_THAN_16TB    
	if (super->s_feature_qnap_compat & EXT2_FEATURE_COMPAT_RESIZE_INODE)
		rsv_gdt = calc_reserved_gdt_blocks(fs);
	else
		rsv_gdt = 0;
	set_field(s_reserved_gdt_blocks, rsv_gdt);
  #ifdef RES_GDT_DEBUG    
    printf("QNAP: super->s_reserved_gdt_blocks=%u\n", super->s_reserved_gdt_blocks);
  #endif /*  #ifdef RES_GDT_DEBUG */  

  if (super->s_reserved_gdt_blocks > super->s_blocks_per_group) {
      retval = EXT2_ET_RES_GDT_BLOCKS;
      printf("QNAP: ERROR: Too many reserved GDT blocks (%u).\n", super->s_reserved_gdt_blocks);      
      goto cleanup;
  }  
#else 
	...
	for (i = 0; i < fs->group_desc_count; i++) {
		...
		if (fs->super->s_log_groups_per_flex)
			numblocks += 2 + fs->inode_blocks_per_group; // Benjamin 20110523: Add back (Already decrease in ext2fs_reserve_super_and_bgd())
