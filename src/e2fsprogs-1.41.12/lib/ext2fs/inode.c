...
errcode_t ext2fs_read_inode_large(ext2_filsys fs, ext2_ino_t ino,
			    struct ext2_inode_large* inode)
{
	return ext2fs_read_inode_full(fs, ino, inode,
...
errcode_t ext2fs_write_inode_large(ext2_filsys fs, ext2_ino_t ino,
			     struct ext2_inode_large *inode)
{
...
/*
 * This function should be called when writing a new large inode.  
 * It makes sure the left extra part of large inodes is initialized properly.
 */
errcode_t ext2fs_write_new_inode_large(ext2_filsys fs, ext2_ino_t ino,
				 struct ext2_inode_large *inode)
{
	int 			size = EXT2_INODE_SIZE(fs->super);
	struct ext2_inode_large	*large_inode;
	...
	large_inode = malloc(size);
	if (!large_inode)
		return ENOMEM;

	memset(large_inode, 0, size);
	*large_inode = *inode;
	...
	retval = ext2fs_write_inode_full(fs, ino, large_inode, size);
	free(large_inode);
	return retval;
}
