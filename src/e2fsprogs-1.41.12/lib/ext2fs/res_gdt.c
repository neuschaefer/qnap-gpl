 ...
 * during ext2fs_reserve_super_and_bgd() in ext2fs_initialize(), so that they are 
 * not allocated for other uses before we can add them to the resize inode 
 * (which has to come after the creation of the inode table).
 */
