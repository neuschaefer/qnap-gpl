...
/* Richard Chen 20090304, QNAP's 1.40.4 diff port */
#include <storage.h>
#include <eventlog.h>
#include "../libnaslog-2.0.0/naslog.h"
...
#if defined(TS209) || defined(TS309) || defined(TS409) || defined(TS509)
			VOLUME_CONF vol_data;
			int ret;

			ret = Get_One_Volume_Conf(ctx->qnap_vol_no,&vol_data);
			Storage_Write_Log(vol_data, VOL_UNCLEAN, 0);
#else
			char msg_log[256];
			strcpy(msg_log,"[SATA Disk] The file system is not cleaned. It is suggested that you to run \"check disk\".");
			write_log(msg_log, EVENT_TYPE_WARN);
#endif
		}
...
//	sprintf(cmd, "/sbin/setcfg \"Disk\" \"Check %s\" %4.1f -f /tmp/check.status", label, percent);
//end here
		system(cmd);
	}
//End
