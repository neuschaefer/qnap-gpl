...
// 2012.03.19: fix bug.24111. fork() is failed when kernel allocate mem. so use Conf_Set_Field to fix it.
extern int Conf_Set_Field(char *conf_file, char *section_name, char *field_name, char *field_data); // from "NasLib\include\Util.h"
...
{
	char    cmd[512];
//2012.03.19: added by Frank for fixing bug.24111
	char progress[8]={0};
	char field_name[20]={0};
//added by KenChen for EncryptFS (device name might be /dev/mapper/sda3)
	char tmp_dev[128];
	...
	if (total==0){
		//sprintf(cmd, "/sbin/setcfg Disk \"Format %s\" -1 -f /tmp/format.status", tmp_dev);				
		sprintf(progress,"%d",(int)(-1));
		sprintf(field_name, "Format %s", tmp_dev);
	}
	else{
		//sprintf(cmd, "/sbin/setcfg Disk \"Format %s\" %d -f /tmp/format.status", tmp_dev, (int)count*100/total);
		sprintf(progress,"%d",(int)count*100/total);
		sprintf(field_name, "Format %s", tmp_dev);
	}
...
/*
	if (total==0)
		sprintf(cmd, "/sbin/setcfg Disk \"Format %s\" -1 -f /tmp/format.status");
	else
		sprintf(cmd, "/sbin/setcfg Disk \"Format %s\" %d -f /tmp/format.status", device_name, (int)count*100/total);
*/
	//system(cmd);
	Conf_Set_Field("/tmp/format.status", "Disk",field_name, progress);
}
//End
...
static void write_inode_tables(ext2_filsys fs, int lazy_flag)
{
    ...
    /* Richard Chen 20090305, QNAP 1.40.4 diff port */
    //Ricky added QNAP
    // save_progress(1, 1);    // write progress to 100%
    //End
}
...
#ifdef SEARCH_FILENAME_CASE_INSENSITIVE
...
//        save_progress(1, 1);    // write progress to 100%
        ...
        // update to 100%, in quiet mode or std mode
