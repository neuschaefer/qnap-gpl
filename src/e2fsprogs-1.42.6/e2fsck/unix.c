...
/// Patch by QNAP
//#define _BSD_SOURCE /* for inclusion of DT_DIR */ // Patched by Benjamin 20111129 for HAL support
//////////////////////////////////////////////////////////
...
//Patch by QNAP
	{
		if (ctx->options & E2F_OPT_QONLYJOURNAL)
			qnap_event = 1;
///////////////////////////////////////////////////
...
//////////////////////////////////////////////////////
	...
	// Kevin Liao 20130912: It seems that the newer version of e2fsck does not think 
	// the below condition as error. However, for backward compatibility, we still
	// need to report the event log here...
	pctx.ino = fs->super->s_free_inodes_count;
	pctx.ino2 = ctx->free_inodes;
	if (pctx.ino != pctx.ino2) {
		if (ctx->options & E2F_OPT_READONLY) {
			// Kevin Liao 20131126: Report log only if the volume is not mounted
			if ((ctx->options & E2F_OPT_QONLYJOURNAL) && !(ctx->mount_flags & EXT2_MF_MOUNTED))
				qnap_event = 1;
		}
		else if (fix_problem(ctx, PR_0_FREE_INODE_COUNT, &pctx)) {
	...
	if (pctx.blk != pctx.blk2) {
		...
		else if (fix_problem(ctx, PR_0_FREE_BLOCK_COUNT, &pctx)) {
	...
	if (qnap_event == 1) {
		char s[64];
		sprintf(s,"/sbin/qe2fsprog_event 1 %d 1",ctx->qnap_vol_no);
		system(s);
	}
...
//Patch by QNAP
/**
 * Patched by Benjamin 20111129 for HAL support
 */
static inline void transfer_device_name(const char *input_name, char *output_name, int buf_size)
{
	FILE *fp, *fp2;
	...
	strncpy(output_name, input_name, buf_size);
	// Patched by Hungkai
	// Change "/dev/mapper/ce_cachedev1" to "/dev/mapper/cachedev1"
	if ((p = strstr(input_name, "/dev/mapper/ce_cachedev"))) {
		p_name = p + strlen("/dev/mapper/ce_");
		snprintf(output_name, buf_size, "/dev/mapper/%s", p_name);
	}
	else 
	if ((p = strstr(input_name, "/dev/mapper/cachedev"))) {
		// Do nothing.
	}
	else
	if ((p = strstr(input_name, "/dev/mapper/md_ag"))) {
		p_name = p + strlen("/dev/mapper/md_");
		snprintf(output_name, buf_size, "/dev/mapper/%s", p_name);
	}
	else if (!strstr(input_name, "/dev/mapper/ag") && strstr(input_name, "/dev/mapper/")) {
		p_name = strrchr(input_name, '/');
		p_name = p_name ? p_name + 1 : (char *)input_name;
		if ((dirP = opendir("/sys/block"))) {
			while ((direntP = readdir(dirP)) != NULL) {
//				if (DT_DIR == direntP->d_type) {
					if ((p = strstr(direntP->d_name, "dm-"))) {
						printf("Path:/sys/block/%s\n", direntP->d_name);
						sprintf(cmd, "/bin/cat /sys/block/%s/dm/name", direntP->d_name);
						if ((fp = popen(cmd, "r")) != NULL) {
							found = (fgets(buffer, sizeof(buffer), fp) != NULL) ? strstr(buffer, p_name) != NULL : 0;
							pclose(fp);
							if (found) {
								sprintf(cmd, "/bin/ls /sys/block/%s/slaves", direntP->d_name);
								if ((fp2 = popen(cmd, "r")) != NULL) {
									if (fgets(buffer, sizeof(buffer), fp2) != NULL)
										snprintf(output_name, buf_size, "/dev/%s", buffer);
									pclose(fp2);
								}
							}
						}
...
//				}
	...
	output_name[buf_size - 1] = '\0';
	p = p_name = output_name + strlen(output_name) - 1;
	while (p != output_name && (*p == ' ' || *p == '\t' || *p == '\n'))
		p--;
	if (p != p_name)
		*(p + 1) = '\0';
}
...
//Patch by QNAP:dded by KenChen for EncryptFS (device name might be /dev/mapper/sda3)
#if 0 //Fix bug#57896. Report more percent values.
	if ((fixed_percent % 40) == 0) {
#else
	if ((percent - old_percent) >= 1.3) {
#endif
		char tmp_dev[32];
#if 0  // Patched by Benjamin 20111129 for HAL support
		char *tmp_ptr = strrchr(label, '/');
		if (tmp_ptr != NULL)
			sprintf(tmp_dev, "/dev%s", tmp_ptr);
		else
			sprintf(tmp_dev, "%s", label);
#else
		transfer_device_name(label, tmp_dev, sizeof(tmp_dev));
#endif
		sprintf(cmd, "/sbin/setcfg \"Disk\" \"Check %s\" %4.1f -f /tmp/check.status", tmp_dev, percent);
		system(cmd);
		old_percent = percent;
	}
/////////////////////////////////////////////////////////////////////////
...
//Patch by QNAP
		case 'q':
			ctx->options |= E2F_OPT_QONLYJOURNAL;
			ctx->qnap_vol_no = atoi(optarg);
			break;
////////////////////////////////////////////////////            
		default:
			usage(ctx);
