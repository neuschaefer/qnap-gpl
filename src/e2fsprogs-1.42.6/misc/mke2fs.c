...
#endif
//Patch by QNAP
#ifdef HAVE_DIRENT_H  /* for inclusion of DT_DIR */ // Patched by Benjamin 20111129 for HAL support
#include <dirent.h>
#endif
////////////////////////////////////////
#include <sys/ioctl.h>
...
void save_progress(blk64_t count, blk64_t total, int start_percent)
{
	static int rate = -1;
	int send_message = 0;
...
// Patched by Benjamin 20111129 for HAL support
	...
	DIR *dirP = NULL;
	int found = 0, size = sizeof(tmp_dev);

	strncpy(tmp_dev, device_name, size);
	// Patched by Hungkai
	// Change "/dev/mapper/ce_cachedev1" to "/dev/mapper/cachedev1"
	if ((p = strstr(device_name, "/dev/mapper/ce_cachedev"))) {
		p_name = p + strlen("/dev/mapper/ce_");
		snprintf(tmp_dev, size, "/dev/mapper/%s", p_name);
	}
	else 
	if ((p = strstr(device_name, "/dev/mapper/cachedev"))) {
		// Do nothing.
	}
	else
	if ((p = strstr(device_name, "/dev/mapper/md_ag"))) {
		p_name = p + strlen("/dev/mapper/md_");
		snprintf(tmp_dev, size, "/dev/mapper/%s", p_name);
	}
	else if (!strstr(device_name, "/dev/mapper/ag") && strstr(device_name, "/dev/mapper/")) {
		p_name = strrchr(device_name, '/');
		p_name = p_name ? p_name + 1 : (char *)device_name;
							...
							if (found) {
								...
								if ((fp2 = popen(cmd, "r")) != NULL) {
									if (fgets(buffer, sizeof(buffer), fp2) != NULL)
										snprintf(tmp_dev, size, "/dev/%s", buffer);
									pclose(fp2);
								}
							}
						}
...
//				}
	...
	tmp_dev[size - 1] = '\0';
	p = p_name = tmp_dev + strlen(tmp_dev) - 1;
	while (p != tmp_dev && (*p == ' ' || *p == '\t' || *p == '\n'))
		p--;
	if (p != p_name)
		*(p + 1) = '\0';
	closedir(dirP);
...
//Patch by QNAP:Use child process to update format process
	...
	else if (rate != ((count*50)/total+start_percent)) {
		rate = ((count*50)/total+start_percent);
		send_message = 1;
	}
	if (send_message) {
		msgp.mtype = 1;
...
//Patch by QNAP
		if (i % 10 == 0)
			save_progress(i, fs->group_desc_count, 50);
////////////////////////////////////////////////
...
//Patch by QNAP
			save_progress(1, 0, 50);    // write error to progress
////////////////////////////////////////////////
			exit(1);
		}
		if (sync_kludge) {
...
#define _SC_PAGESIZE _SC_PAGE_SIZE
	...
	if (retval == ENOENT) {
		...
		if (retval)
			goto profile_error;
		retval = profile_set_default(profile, mke2fs_default_profile); // Benjamin: Default Profile is here...
		if (retval)
			goto profile_error;
	} else if (retval) {
profile_error:
...
//Patch by QNAP
#ifdef __linux__
	// 20130520 Kevin Liao: Enable 64bit feature only for ext4
	if ((strcmp(*fs_types, "ext4") == 0) && (strcasestr(ut.machine, "x86_64") != NULL) && !(fs_param.s_feature_incompat & EXT4_FEATURE_INCOMPAT_64BIT)) {
		fs_param.s_feature_incompat |= EXT4_FEATURE_INCOMPAT_64BIT;
		// 20121207 Kevin Liao: Do we need to disable resize_inode while enabling 64bit?
		//fs_param.s_feature_compat &= ~EXT2_FEATURE_COMPAT_RESIZE_INODE;
	}

#ifdef LFS_ON_32BIT_CPU
	// Enable 64bit feature for 32-bit ARM
	if ((strcmp(*fs_types, "ext4") == 0) && (strcasestr(ut.machine, "armv7l") != NULL)
		&& !(fs_param.s_feature_incompat & EXT4_FEATURE_INCOMPAT_64BIT)) {
			fs_param.s_feature_incompat |= EXT4_FEATURE_INCOMPAT_64BIT;
	}
#endif
...
// Patched by QNAP [start]
		if (cur % 10 == 0)
			save_progress(cur, blocks, 0);
// Patched by QNAP [end]
		...
		if (retval) {
// Patched by QNAP [start]			
			save_progress(1, 0, 0);    // write error to progress
// Patched by QNAP [end]
			break;
		}
		cur += count;
