...
//[S] Patch by QNAP Benjamin 20120414 for resize2fs progress
#include <sys/ipc.h>
#include <sys/msg.h>
#include <signal.h>
#include <sys/prctl.h>
...
// Note that signal_handler and save_progress is copied from mke2fs.c,
// So if needed, update both!
...
//	printf(_("save_progress: device_name=%s, (count,total,rate)=(%d,%d,%d).\n\n"), device_name, count, total, rate);
}
//[E] Patch by QNAP Benjamin 20120414 for resize2fs progress
...
//Patch by QNAP Benjamin 20120414 for resize2fs progress: change labelwidth from 30 to 100
#if 1
		retval = ext2fs_progress_init(&progress, label, 100,
					      40, max, 0);
#else
...
//[S]Patch by QNAP:Benjamin 20120414: Use child process to update format process
...
//[E]Patch by QNAP:Benjamin 20120414: Use child process to update format process
