...
//Patch by QNAP
#ifdef RES_GDT_DEBUG
	/* Benjamin 20110607: Debug only. */
	rfs->flags |= RESIZE_DEBUG_IO | RESIZE_DEBUG_BMOVE | RESIZE_DEBUG_INODEMAP | RESIZE_DEBUG_ITABLEMOVE;
#endif
//////////////////////////////////////////////////////////
	rfs->itable_buf	 = 0;
	...
	retval = adjust_superblock(rfs, *new_size); // Benjamin: Extending the inode table is doing here.
	if (retval)
		goto errout;
...
//[S] Benjamin 20110513 patch http://lists.openwall.net/linux-ext4/2010/08/20/2
		blk = ((blk64_t)g * (blk64_t)fs->super->s_blocks_per_group) +
			(blk64_t)fs->super->s_first_data_block;
//[E] Benjamin 20110513 patch http://lists.openwall.net/linux-ext4/2010/08/20/2
...
retry:
	...
	// Benjamin 20110603: TODO: not check yet...
...
//Patch by QNAP
#if 1
	group_block = (blk64_t)fs->super->s_first_data_block +
		(blk64_t)old_fs->group_desc_count * fs->super->s_blocks_per_group;
#else
...
//Patch by QNAP
#if 0
		/* QNAP: Benjamin 20110607 for solving unused inodes=0 of the new BGDs */
		// Kevin Liao 20121203: Check whether we really need this for kernel 3.4.6 later...
		ext2fs_bg_itable_unused_set(fs, i, fs->super->s_inodes_per_group);
#endif
//////////////////////////////////////////////////////////
...
//Patch by QNAP
	blk64_t     total_free = 0;
	blk64_t     group_free = 0;
#if 0
	int		total_free = 0;
	int		group_free = 0;
#endif
...
blk64_t calculate_minimum_resize_size(ext2_filsys fs)
{
	 ...
	 */
	// Benjamin 20110603: TODO: Below is not checked yet...Assume right.
	flex_groups = groups;
