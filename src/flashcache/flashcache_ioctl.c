 ...
 */
int
flashcache_uncacheable(struct disk_slot *slot, struct bio *bio)
{
        struct cache_c *dmc = (struct cache_c *) slot->dmc;
	int dontcache=0;
	
	if (dmc->sysctl_cache_all) {
#ifdef FLASHCACLE_PID_CONTROL
		 ...
		 */
		if (!dontcache) dontcache = skip_sequential_io(slot, bio);
 ...
 */
int 
skip_sequential_io(struct disk_slot *slot, struct bio *bio)
{
	struct cache_c *dmc = (struct cache_c *) slot->dmc;
	...
	for (seqio = dmc->seq_io_head; seqio != NULL && sequential == 0; seqio = seqio->next) { 
		...
		if (bio->bi_sector == seqio->most_recent_sector) {
			...
			seqio->recent_size = to_sector(bio->bi_size);
			seqio->sequential_count	= to_sector(bio->bi_size);
			sequential = 1;
			if (dmc->seq_io_head != seqio)
				seq_io_move_to_lruhead(dmc, seqio);
		}
		/* i/o to one block more than the previous i/o = sequential */	
		else if (bio->bi_sector == seqio->most_recent_sector + seqio->recent_size) {
			...
			if (seqio->sequential_count > dmc->sysctl_skip_seq_thresh_kb * 2) {
...
out:
	if (skip) {
		if (bio_data_dir(bio) == READ) {
	        	...
	        	slot->flashcache_stats.uncached_sequential_reads++;
		} else {
	        	dmc->flashcache_stats.uncached_sequential_writes++;
	        	slot->flashcache_stats.uncached_sequential_writes++;
		}
	}
...
#endif
{
        ...
        struct cache_c *dmc = (struct cache_c *) slot->dmc;
	struct block_device *bdev = slot->disk_dev->bdev;
	struct file fake_file = {};
	struct dentry fake_dentry = {};
	struct ssd_hotplug_cmd ssd_hotplug;
	int enable;

	switch(cmd) {
	case FLASHCACHEPLUGIN_SSD:
		if (copy_from_user(&ssd_hotplug, (struct ssd_hotplug_cmd *)arg, sizeof(struct ssd_hotplug_cmd)))
			return -EFAULT;
		return flashcache_plugin(dmc, &ssd_hotplug);
	case FLASHCACHEPLUGOUT_SSD:
		return flashcache_plugout(dmc);
	case FLASHCACHERECONFIG_SSD:
		if (copy_from_user(&ssd_hotplug, (struct ssd_hotplug_cmd *)arg, sizeof(struct ssd_hotplug_cmd)))
			return -EFAULT;
		return 0;
	case FLASHCACHEENABLE_SSD:
		if (copy_from_user(&enable, (int *)arg, sizeof(int)))
			return -EFAULT;
		return flashcache_enable(slot, enable);
	default:
		fake_file.f_mode = slot->disk_dev->mode;
...
#else
		return __blkdev_driver_ioctl(slot->disk_dev->bdev, slot->disk_dev->mode, cmd, arg);
#endif
	}
...
int 
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,27)
flashcache_ssd_ioctl(struct dm_target *ti, struct inode *inode,
		 struct file *filp, unsigned int cmd,
		 unsigned long arg)
#else
flashcache_ssd_ioctl(struct dm_target *ti, unsigned int cmd, unsigned long arg)
#endif
{
	...
	case FLASHCACHERECONFIG_SSD:
		...
		return 0;
	default:
		fake_file.f_mode = dmc->cache_dev->mode;
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,20)
		fake_file.f_dentry = &fake_dentry;
#else
		fake_file.f_path.dentry = &fake_dentry;
		...
		return blkdev_driver_ioctl(bdev->bd_inode, &fake_file, bdev->bd_disk, cmd, arg);
#else
		return __blkdev_driver_ioctl(dmc->cache_dev->bdev, dmc->cache_dev->mode, cmd, arg);
#endif
	}
}
...
int 
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,27)
flashcache_compat_ioctl(struct dm_target *ti, struct inode *inode,
		 struct file *filp, unsigned int cmd,
		 unsigned long arg)
#else
flashcache_compat_ioctl(struct dm_target *ti, unsigned int cmd, unsigned long arg)
#endif
{
        struct disk_slot *slot = (struct disk_slot *) ti->private;
        struct cache_c *dmc = (struct cache_c *) slot->dmc;
	...
	switch(cmd) {
	case FLASHCACHEPLUGIN_SSD:
		if (copy_from_user(&ssd_hotplug, (struct ssd_hotplug_cmd *)compat_ptr(arg), sizeof(struct ssd_hotplug_cmd)))
			return -EFAULT;
		return flashcache_plugin(dmc, &ssd_hotplug);
	case FLASHCACHEPLUGOUT_SSD:
		return flashcache_plugout(dmc);
...
int 
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,27)
flashcache_ssd_compat_ioctl(struct dm_target *ti, struct inode *inode,
		 struct file *filp, unsigned int cmd,
		 unsigned long arg)
#else
flashcache_ssd_compat_ioctl(struct dm_target *ti, unsigned int cmd, unsigned long arg)
#endif
{
        struct cache_c *dmc = NULL;
	struct block_device *bdev = NULL;
