 ...
 *
 * Version:	$Id: auth.c,v 1.3 2010/03/30 10:29:59 richardchang Exp $
 *
...
RCSID("$Id: auth.c,v 1.3 2010/03/30 10:29:59 richardchang Exp $")
...
//RichardChang@QNAP 20100325 add for log
void write_loginsucc_log(REQUEST *request)
{
	VALUE_PAIR	*uni;
	char remote_ip[64] = "127.0.0.1", user[64] = "guest", *comp = "---";
	int nas_address = 0;
	
	if ((uni = pairfind(request->packet->vps, PW_USER_NAME)) != NULL) {
		sprintf(user, "%.64s", (char *)uni->vp_strvalue);
	}
	
	if (request->packet->src_ipaddr.ipaddr.ip4addr.s_addr) {
		nas_address = request->packet->src_ipaddr.ipaddr.ip4addr.s_addr;
		ip_ntoa(remote_ip, nas_address);
	}
	
	SendConnToLogEngine(EVENT_TYPE_INFO, user, remote_ip, comp, CONN_SERV_RADIUS, CONN_ACTION_AUTH_OK, comp);
}

void write_loginfail_log(REQUEST *request)
{
	VALUE_PAIR	*uni;
	char remote_ip[64] = "127.0.0.1", user[64] = "guest", *comp = "---";
	int nas_address = 0;
	...
	SendConnToLogEngine(EVENT_TYPE_WARN, user, remote_ip, comp, CONN_SERV_RADIUS, CONN_ACTION_AUTH_FAIL, comp);
}
//end
#endif
/*
...
#ifdef QNAP
	if(goodpass){
		write_loginsucc_log(request);
	}
	else{
		write_loginfail_log(request);
	}
#endif
