...
int main(int argc, char **argv)
{
	...
	/*
	** Test gdImageCreateFromiPngSource'
	**/
	...
	**/
 	...
 	imgsnk.sink = fwriteWrapper;
	imgsnk.context = out;
	gdImageGifToSink(im, &imgsnk);
        fclose(out);
        in = fopen(of, "rb");
        if (!in) {
                fprintf(stderr, "GD Sink (GIF): ERROR - GD Sink Output file does not exist!\n");
        } else {
        	im2 = gdImageCreateFromGif(in);
		fclose(in);

        	CompareImages("GD Sink (GIF)", ref, im2);
		gdImageDestroy(im2);
	};
