...
int Is_Sata_91XX(void) //9125
{
        char buf[64];
	sprintf(buf, "/bin/cat /proc/bus/pci/devices | grep 1b4b91 1>/dev/null 2>&1");
	return system(buf); // if have 91XX=0, NULL=1
}
...
int Get_CPU_Model(void)
{
        FILE *fptr;
        int model_type = -1;
#if defined(TS419) || defined(TS219) || defined(TS119) // roylin add by 20100714, check cpu 6281 or 6282
        char buf[20];
        fptr = fopen("/proc/tsinfo/cpu_model", "r");
        if(fptr == NULL)
                return model_type;
        fgets(buf, sizeof(buf), fptr);
        fclose(fptr);
        if(!strncmp(buf,"88F6282",7))
                model_type = 6282;
        else
                model_type = 6281;
#endif
...
#endif
#if defined(TS859) || defined(TS859U)
	int dev_num = 0;
	dev_num = devname[7] - 'a';
#endif
...
#define WIN_FLUSHCACHE 0xe7
		...
		if(dev_num >= 4){	//use ATA
			...
			smart_command.cmd[0] = 0xEF; //SCSI_CMD_MARVELL_SPECIFIC
			smart_command.cmd[4] = 6;     //command length
			smart_command.cmd[1] = 0x01; //CDB_CORE_MODULE
			smart_command.cmd[2] = wcache ? 0x04 : 0x05;
			buff[0] = 0xEF;
			buff[1] = 0;
			buff[2] = smart_command.cmd[2];
			buff[3] = 0;
	
			if (ioctl(fd, SCSI_IOCTL_SEND_COMMAND, (void *)&smart_command))
				perror(" HDIO_DRIVE_CMD(setcache) failed");
			else
				printf(" HDIO_DRIVE_CMD(setcache) succeeded.\n");
		}
#elif defined(TS859)
		if(dev_num < 4){	//use ATA
			args[2] = wcache ? 0x02 : 0x82;
...
#elif defined(TS259) || defined(TS459) || defined(TS559) || defined(TS659) || defined(X86_SANDYBRIDGE) || defined(X86_CEDAVIEW) || defined(X86_EVANSPORT)
		args[2] = wcache ? 0x02 : 0x82;
		if (get_wcache) {
			printf(" setting drive write-caching to %ld", wcache);
			on_off(wcache);
...
#endif // DO_FLUSHCACHE 
#else
//Added & Modified by KenChen@QNAP  2008/Nov/03
		mvsata_scsi_cmd  smart_command;
		unsigned char *buff = (unsigned char *)&smart_command.cmd[6];
//End here
		args[2] = wcache ? 0x02 : 0x82;
...
#define WIN_STANDBYNOW2 0x94
		...
		if(dev_num >= 4){	//use ATA
			...
			if (get_standbynow) {
				printf(" setting standby immediately.\n");
			}
			buff[0] = WIN_STANDBYNOW1;
			buff[1] = 0;
			buff[2] = 0;
			buff[3] = 0;
			if (ioctl(fd, SCSI_IOCTL_SEND_COMMAND, (void *)&smart_command))
		...
		// roylin add by 20100714, check cpu 6281 or 6282
		if((Get_CPU_Model() == 6282) && (Is_Sata_91XX()== 0)){
			unsigned char args1[4] = {WIN_STANDBYNOW1,0,0,0};
			unsigned char args2[4] = {WIN_STANDBYNOW2,0,0,0};
			if (get_standbynow)
				printf(" issuing standby command\n");
			...
			//Added & Modified by KenChen 20071210
			if (get_standbynow)
				printf(" issuing standby command\n");
			...
			buff[3] = 0;
			if (ioctl(fd, SCSI_IOCTL_SEND_COMMAND, (void *)&smart_command)){
				// Fix Bug 36192, arm platform hd standby function is abnormal on kernel 3.4.6
				// try SCSI_IOCTL_SEND_COMMAND failed, then try HDIO_DRIVE_CMD for Linux-3.4.6 arm platform
				unsigned char args1[4] = {WIN_STANDBYNOW1,0,0,0};
				unsigned char args2[4] = {WIN_STANDBYNOW2,0,0,0};
				if (ioctl(fd, HDIO_DRIVE_CMD, &args1)
						&& ioctl(fd, HDIO_DRIVE_CMD, &args2)){
...
#define HDIO_DRIVE_TASKFILE 0x031d
	...
	if (set_standby) {
		...
		if(dev_num >= 4){	//use ATA
			...
			buff[0] = WIN_SETIDLE1;
			buff[3] = standby_requested;
			if (ioctl(fd, SCSI_IOCTL_SEND_COMMAND, (void *)&smart_command))
				perror(" SCSI_IOCTL_SEND_COMMAND(setidle1) failed ");
		}
#elif defined(TS859)
		if(dev_num < 4){	//use ATA
			unsigned char args[4] = {WIN_SETIDLE1,standby_requested,0,0};
		...
		if((Get_CPU_Model() == 6282) && (Is_Sata_91XX()== 0)){
			...
			buff[3] = standby_requested;
			if (ioctl(fd, SCSI_IOCTL_SEND_COMMAND, (void *)&smart_command)){
				// try SCSI_IOCTL_SEND_COMMAND, then try HDIO_DRIVE_CMD for Linux-3.4.6 arm platform
				unsigned char args[4] = {WIN_SETIDLE1,standby_requested,0,0};
				if (ioctl(fd, HDIO_DRIVE_CMD, &args)){
					perror(" HDIO_DRIVE_CMD(setidle1) failed ");
				}
...
#define WIN_CHECKPOWERMODE2 0x98
		...
		if(dev_num >= 4){	//use ATA
			...
			buff[0] = WIN_CHECKPOWERMODE1;
	
			if (ioctl(fd, SCSI_IOCTL_SEND_COMMAND, (void *)&smart_command) &&
				(buff[0] = WIN_CHECKPOWERMODE2) && 
				ioctl(fd, SCSI_IOCTL_SEND_COMMAND, (void *)&smart_command)){
				buff = (unsigned char *)&smart_command.cmd[0];
				if (errno != EIO || buff[0] != 0 || buff[1] != 0)
					state = "unknown";
				else
					state = "sleeping";
			}
				...
				buff = (unsigned char *)&smart_command.cmd[0];
				state = (buff[3] == 255) ? "active/idle" : "standby";
			}
		}
