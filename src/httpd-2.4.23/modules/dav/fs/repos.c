...
#ifdef QNAP
    if(-1 == chmod(dst, 0666))
	return dav_new_error(p, HTTP_INTERNAL_SERVER_ERROR, 0, errno,
				"Change mode fail");
#endif
...
#ifdef QNAP
    if(-1 == chmod(dstinfo->pathname, 0777))
	err = dav_new_error(ctx->pool, HTTP_INTERNAL_SERVER_ERROR, 0, errno,
			"Change mode fail");
#endif
    }
    else {
...
#ifdef QNAP
    if ((err = apr_file_rename(dstinfo->pathname, srcinfo->pathname,
...
static dav_error * dav_fs_walker(dav_fs_walker_context *fsctx, int depth)
{
    ...
    if ((status = apr_dir_open(&dirp, fsctx->path1.buf, pool)) != APR_SUCCESS) {
        /* ### need a better error */
#ifdef QNAP
	if ( status == 13 ) /* 13 = EACCES */
	    return dav_new_error(pool, HTTP_FORBIDDEN, 0, status, NULL);
	else
#endif //QNAP
...
#ifdef QNAP
    int veto_cnt = 0, i = 0;
    SYS_VETO_INFO *veto_files = NULL;
    veto_cnt = Sys_Get_Veto_Files(&veto_files);
#endif //QNAP
...
#ifdef QNAP
        if(params->func != dav_fs_delete_walker){
        for(i=0;i<veto_cnt;i++)
        {
            if(!strcmp(dirent.name, veto_files[i].file_name))
                break;
        }
        if(i != veto_cnt){
            continue;
...
#ifdef QNAP
	//[2013-10-17 by LIN_WC]: if symlink distinguish between file and directory
	if (fsctx->info1.finfo.filetype == APR_LNK){
	    DIR *dir;
	    if ((dir = opendir(fsctx->path1.buf)) == NULL){
		fsctx->info1.finfo.filetype = APR_REG;
	    }
	    else{
		fsctx->info1.finfo.filetype = APR_DIR;
		closedir(dir);
	    }
	}
#endif //QNAP
