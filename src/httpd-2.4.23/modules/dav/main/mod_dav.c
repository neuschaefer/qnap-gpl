...
static int dav_method_propfind(request_rec *r)
{
    ...
    if (err != NULL) {
        /* If an error occurred during the resource walk, there's
           ...
           response situations. */
#ifdef QNAP
	/* If permission denied return 13 from error, ignore this 
	   connection aborted, let move fuction to check permission*/
	if ( err->error_id != 13 ){
        err = dav_push_error(r->pool, err->status, 0,
...
#include <sys/stat.h>
#include <pwd.h>
#define	DAVLOCK_DIR "/tmp/DavLock.dir"
#define	DAVLOCK_PAG "/tmp/DavLock.pag"
static int change_user(char *user)
{
    struct passwd *pwuser;
    int ncgids = 100;//MAX_GROUPS
    gid_t groupList[100];
    pwuser = getpwnam(user);
    getgrouplist(user, pwuser->pw_gid, groupList, &ncgids);
    setgroups(ncgids, groupList);
    if (setgid(pwuser->pw_gid) != 0){
        return  DECLINED;
    }
...
static int qnap_dav_handler(request_rec *r)
{
    int fd,status;
    pid_t pid = 0;
    struct passwd *pwuser;
     ...
     */
    int ret = 0;
    if( access( DAVLOCK_DIR, F_OK ) != -1 ) {
	chmod( DAVLOCK_DIR, 0766 );
    }
    if( access( DAVLOCK_PAG, F_OK ) != -1 ) {
	chmod( DAVLOCK_PAG, 0766 );
    }
    ...
    if (r->method_number == M_GET) {
        ret = dav_method_get(r);
    }

    else if (r->method_number == M_PUT) {
        ret = dav_method_put(r);
    }

    else if (r->method_number == M_POST) {
        ret = dav_method_post(r);
    }

    else if (r->method_number == M_DELETE) {
        ret = dav_method_delete(r);
    }

    else if (r->method_number == M_OPTIONS) {
        ret = dav_method_options(r);
    }

    else if (r->method_number == M_PROPFIND) {
        ret = dav_method_propfind(r);
    }

    else if (r->method_number == M_PROPPATCH) {
        ret = dav_method_proppatch(r);
    }

    else if (r->method_number == M_MKCOL) {
        ret = dav_method_mkcol(r);
    }

    else if (r->method_number == M_COPY) {
        ret = dav_method_copymove(r, DAV_DO_COPY);
    }

    else if (r->method_number == M_MOVE) {
        ret = dav_method_copymove(r, DAV_DO_MOVE);
    }

    else if (r->method_number == M_LOCK) {
        ret = dav_method_lock(r);
    }

    else if (r->method_number == M_UNLOCK) {
        ret = dav_method_unlock(r);
    }

    else if (r->method_number == M_VERSION_CONTROL) {
        ret = dav_method_vsn_control(r);
    }

    else if (r->method_number == M_CHECKOUT) {
        ret = dav_method_checkout(r);
    }

    else if (r->method_number == M_UNCHECKOUT) {
        ret = dav_method_uncheckout(r);
    }

    else if (r->method_number == M_CHECKIN) {
        ret = dav_method_checkin(r);
    }

    else if (r->method_number == M_UPDATE) {
        ret = dav_method_update(r);
    }

    else if (r->method_number == M_LABEL) {
        ret = dav_method_label(r);
    }

    else if (r->method_number == M_REPORT) {
        ret = dav_method_report(r);
    }

    else if (r->method_number == M_MKWORKSPACE) {
        ret = dav_method_make_workspace(r);
    }

    else if (r->method_number == M_MKACTIVITY) {
        ret = dav_method_make_activity(r);
    }

    else if (r->method_number == M_BASELINE_CONTROL) {
        ret = dav_method_baseline_control(r);
    }

    else if (r->method_number == M_MERGE) {
        ret = dav_method_merge(r);
    }

    /* BIND method */
    else if (r->method_number == dav_methods[DAV_M_BIND]) {
        ret =  dav_method_bind(r);
    }

    /* DASL method */
    else if (r->method_number == dav_methods[DAV_M_SEARCH]) {
        ret =  dav_method_search(r);
    }
...
{
#ifdef QNAP
    if(Get_Private_Profile_Integer("QWEB", "Enable_User_Permission", 0, "/etc/config/uLinux.conf"))
       ap_hook_handler(qnap_dav_handler, NULL, NULL, APR_HOOK_MIDDLE);
    else
