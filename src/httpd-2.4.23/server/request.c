...
#ifdef QNAP_WEBDAV
    /* we need do this after login so that we can replace %H to home folder */
    int is_home_path = 0;
    if ((r && r->filename && strncmp(r->filename, "/%H", 3) == 0)){
	is_home_path = 1;
    }
...
#ifdef QNAP_WEBDAV
		if((r->user) && (r->filename) && (strncmp(r->filename, "/%H", 3) == 0)){
		    NAS_Create_User_Home_By_Name(r->user);
		}
#endif //QNAP_WEBDAV
            }
...
#ifdef QNAP_WEBDAV
static void qnap_replace_home(request_rec *r)
{
    ...
    if(r->filename){
        if(ptr = strstr(r->filename, "%H")){
            pw = getpwnam(r->user);
            if(pw){
		len = strlen(r->filename) + 1;
		
		if(strlen(ptr) > 2)
		    r->filename = apr_psprintf(r->pool, "%s%s", pw->pw_dir, ptr + 2);
		else
		    r->filename = apr_psprintf(r->pool, "%s", pw->pw_dir);
            }
        }
    }

    if(r->path_info){
        if(ptr = strstr(r->path_info, "%H")){
            pw = getpwnam(r->user);
            if(pw){
		len = strlen(r->path_info) + 1;

		if(strlen(ptr) > 2)
		    r->path_info = apr_psprintf(r->pool, "%s%s", pw->pw_dir, ptr + 2);
		else
		    r->path_info = apr_psprintf(r->pool, "%s", pw->pw_dir);
            }
        }
    }
}
