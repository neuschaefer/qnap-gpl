...
int Get_Binding_Address(PT_BIND_ADDR ptBindAddr)
{
	...
	if (g_qLisan == 0) { //apache
		if (NULL == (pfAddr = popen(SZP_GET_BIND_ADDR " WEBPAGE 2>/dev/null", "r")))  return 0;
	}
	else{ //apache_proxy
	...
	while (NULL != fgets(szLine, sizeof(szLine), pfAddr))
	{
		...
		if (NULL != strchr(szLine, ':'))
		{
			...
			//ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, ap_server_conf, "[Servbind] Binding IPv6 address: %s", szLine);
		...
		else if (NULL != strchr(szLine, '.'))
		{
			if (strstr(SZ_INADDR_ANY, szLine))
			{
				...
				//ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, ap_server_conf, "[Servbind] Binding IPv4 address: %s ==> Service binding is disabled!", szLine);
				ptBindAddr->bEnabled = FALSE;
				ptBindAddr->cnIpv4 = ptBindAddr->cnIpv6 = 0;
				break;
			}
			//DEBUG_PRINTF("Binding IPv4 address: %s\n", szLine);
			//ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, ap_server_conf, "[Servbind] Binding IPv4 address: %s", szLine);
...
static int Check_Binding_Address(int fdAccept, PT_BIND_ADDR ptBindAddr, char *pszAddrBuf)
{
	...
	if (ptBindAddr->bEnabled)
	{
		...
		if (-1 == getsockname(fdAccept, (struct sockaddr *)&tsaLocal, &cbSktAddr))
		{
			...
			//ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, ap_server_conf, "[Servbind] Can't get the socket name of an accepted socket!");
		...
		if ((AF_INET == tsaLocal.ss_family) && (0 < ptBindAddr->cnIpv4))
		{
			...
			//ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, ap_server_conf, "[Servbind] Check binding address: %s, %d", pszAddr, iRet);
		}
		...
		else
		{
			...
			//ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, ap_server_conf, "[Servbind] Unknown interface family number: %d!", tsaLocal.ss_family);
		}
	}
                               ...
                               apr_pool_t *connection_context)
{
	...
	if (0 != (iRetDbg = Check_Binding_Address(s, &g_tbaDaemon, szIpAddr)))
    	{
    		// This socket connection came from a unbound interface, we return a special error code to represent it.
    		close(s);
    		return E_UNBOUND_INTERFACE;
    	}
    }
#endif /* QNAP_SERVICE_BINDING */
