...
void get_number_of_cpus()
{
    ...
    // SWChen patch for arm and X31
    // X31 will show as following 
    // Processor name  : ARMv7 Processor rev 1 (v7l)
    // processor       : 0
    // processor       : 1
    // 
    // and MV6281,6282 will show
    ...
    // processor       : 1
    // ...
    // We count "processor" and "Processor name" and use the bigger as number of cpu.
    //
    int pcpu = 0, pncpu = 0;

	handle_error("Can't open /proc/cpuinfo", !ncpufp);
	while (fgets(buffer, sizeof(buffer), ncpufp)) {
		if (!strncasecmp(buffer, "processor\t:", 11)) 
			pcpu++;
        else if (!strncasecmp(buffer, "processor name\t:", 16))
            pncpu++;
	}

    ncpu = (pcpu > pncpu) ? pcpu : pncpu;
