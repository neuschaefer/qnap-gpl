...
int parseConfigFile(globals_p vars)
{
    ...
    regcomp(&re_desc_doc,"description_document_name[[:blank:]]*=[[:blank:]]*([[:alnum:].]{1,20})",REG_EXTENDED);//QNAP alpha => alnum
    regcomp(&re_xml_path,"xml_document_path[[:blank:]]*=[[:blank:]]*([[:alnum:]_/.]{1,50})",REG_EXTENDED);//QNAP alpha => alnum
