...
//				UpnpGetServerPort(), g_vars.descDocName);
	if ((fp=popen("/sbin/getcfg System \"Web Access Port\" -d 8080", "r"))){
		int _get_port=0;
		if((fscanf(fp, "%d", &_get_port) == 1) && _get_port>0)
			thttpd_port = _get_port;
		pclose(fp);
	}
	if(thttpd_port>0)
		sprintf(descDocUrl, "http://%s:%d/upnpd/%s", UpnpGetServerIpAddress(), thttpd_port, g_vars.descDocName);
	else
		sprintf(descDocUrl, "http://%s:%d/%s", UpnpGetServerIpAddress(),
				UpnpGetServerPort(), DESC_DOC_DEFAULT);
//======
