...
$Log: mDNSPosix.c,v $
Revision 1.2  2011/06/09 08:32:57  clko
[Service binding] Add service binding function into the iTunes service.

Revision 1.1.1.1  2009-01-25 03:16:46  charley
...
#define	CBMAX_NETIF_NAME		16			// The maximum length of the netif device name. (include null char)
 ...
 */
int Check_Binding_Address(struct ifi_info **pptIntfList)
{
	int  cbAddr, i, cnAllowIF = 0;
	char  szLine[256], szAllowIF[MAX_BINDING_ADDRESS][CBMAX_NETIF_NAME];
	FILE  *pfAddr = NULL;
	struct stat  tStat;
	struct ifi_info  **pptIntf = NULL, *ptIntfDel;

	// If there is nothing in the interface list, just return as it succeeds.
	if (!pptIntfList || !(*pptIntfList))  return 0;

	// Just report no binding address if the getbindaddr is not existed.
	if (-1 == stat(SZP_GET_BIND_ADDR, &tStat))  return 0;
	if (NULL == (pfAddr = popen(SZP_GET_BIND_ADDR " iTUNES -onlynetif 2>/dev/null", "r")))  return 0;

	while (NULL != fgets(szLine, sizeof(szLine), pfAddr) && (MAX_BINDING_ADDRESS > cnAllowIF))
	{
		szLine[sizeof(szLine)-1] = '\0';
		if (0 >= (cbAddr = strlen(szLine)))  continue;
		if ('\n' == szLine[cbAddr-1])  szLine[cbAddr-1] = '\0';
		// The service binding is disabled, just don't do anything.
		if (strstr(SZ_INADDR_ANY, szLine))
		{
			return 0;
		}
		// Record the allowed interface name in an array.
		strcpy(szAllowIF[cnAllowIF++], szLine);
	}
	pclose(pfAddr);

	// Check each interface.
	//for (pptIntf = pptIntfList; *pptIntf; pptIntf = &((*pptIntf)->ifi_next))
	pptIntf = pptIntfList;
	while (*pptIntf)
	{
		// Skip the loopback interface. (Always allow the loopback interface)
		if (0 == strcmp((*pptIntf)->ifi_name, "lo"))
		{
			pptIntf = &((*pptIntf)->ifi_next);
			continue;
		}
		// Check if this interface is in the allowed list.
		for (i = 0; (i < cnAllowIF) && (0 != strcmp((*pptIntf)->ifi_name, szAllowIF[i])); i++);
		// This interface is not bound to this service. Remove it.
		if (i >= cnAllowIF)
		{
			ptIntfDel = *pptIntf;
			*pptIntf = (*pptIntf)->ifi_next;
			ptIntfDel->ifi_next = NULL;
			free_ifi_info(ptIntfDel);
		}
		else
			pptIntf = &((*pptIntf)->ifi_next);
	}
...
#ifdef QNAP_SERVICE_BINDING
	if (g_bQnapBind)  Check_Binding_Address(&intfList);
#endif /* QNAP_SERVICE_BINDING */
