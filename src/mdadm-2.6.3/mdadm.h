...
#ifdef QNAPNAS
#define	MAX_QDEV	16
typedef struct QEXAMINE_INFO{
	int cnt;
	int g_raid_disks;
	int g_level;
//---member disk info---
	int total_devices[MAX_QDEV];
	int	raid_disks[MAX_QDEV];
	int level[MAX_QDEV];
	char	uuid[MAX_QDEV][64];
	char	devname[MAX_QDEV][20];
	int	number[MAX_QDEV];
	float	metadata[MAX_QDEV];
	unsigned long long events_hi[MAX_QDEV];
	unsigned long long events_lo[MAX_QDEV];
}QEXAMINE_INFO;

typedef	struct QEXAMINE_INFO_BUF{
	int total_devices;
	int	raid_disks;
	int level;
	char	uuid[64];
	...
	int	number;
	float	metadata;
	unsigned long long events_hi;
	unsigned long long events_lo;
}QEXAMINE_INFO_BUF;	
...
#ifdef QNAPNAS
	void (*qexamine_super)(void *sbv, QEXAMINE_INFO *qinfo);
#endif
...
#ifdef QNAPNAS
extern int QdevExamine(mddev_dev_t devlist, int brief, int scan, int SparcAdjust, struct supertype *forcest, char *homehost);
#endif
