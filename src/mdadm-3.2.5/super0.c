...
#ifdef QNAPNAS
		// Fixed bug 19463. Change to unsigned int to support up to 4TB.
		printf("  Used Dev Size : %u%s\n", sb->size,
...
#ifdef QNAPNAS
static void qexamine_super0(struct supertype *st, QEXAMINE_INFO *qinfo)
{
	mdp_super_t *sb = st->sb;
	char *c=map_num(pers, sb->level);
	mdp_disk_t *dp;
	...
	printf("Array = /dev/md%d\n", sb->md_minor);

//	printf("RaidLevel = %d\n", c?sb->level:(-2)); // uknown = -2
//	qinfo->level[qinfo->cnt] = (c?sb->level:(-2));
	if (c) {
		printf("RaidLevel = %d\n", sb->level);
		qinfo->level[qinfo->cnt] = sb->level;
	}
	else {
		printf("RaidLevel = -2\n"); // uknown = -2
	...
	qinfo->raid_disks[qinfo->cnt] = sb->raid_disks;
	printf("TotalDevices = %d\n", sb->nr_disks); //TotalDevices=current active+spare
	qinfo->total_devices[qinfo->cnt] = sb->nr_disks;
	printf("UUID = ");
	if (sb->minor_version >= 90){
		printf("%08x:%08x:%08x:%08x", sb->set_uuid0, sb->set_uuid1,
		       sb->set_uuid2, sb->set_uuid3);
		sprintf(qinfo->uuid[qinfo->cnt],"%08x:%08x:%08x:%08x", sb->set_uuid0, sb->set_uuid1,sb->set_uuid2, sb->set_uuid3);
	}
	else{
		printf("%08x", sb->set_uuid0);
		sprintf(qinfo->uuid[qinfo->cnt],"%08x", sb->set_uuid0);
	}
	printf("\n");

	if (sb->minor_version > 0)
		printf("Metadata = 0.%d\n", sb->minor_version);
	qinfo->metadata[qinfo->cnt] = 0.9;
	qinfo->events_hi[qinfo->cnt] = sb->events_hi;
	qinfo->events_lo[qinfo->cnt] = sb->events_lo;

	printf("Number = %d\n", dp->raid_disk); //device order
	qinfo->number[qinfo->cnt] = dp->raid_disk;
...
#ifdef QNAPNAS
	.qexamine_super = qexamine_super0,
#endif
#endif
