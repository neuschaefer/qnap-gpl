...
#ifdef QNAP_FLEX_RAID
    if(c->is_flexRaid){
        if(s->level!=4 && s->level!=5 && s->level!=6){
            printf("[ERROR]%s().%d, level:%d does not support flex raid mode currently \n",
                __func__, __LINE__, s->level);
            exit(-1);
        }

        qnap_super_set_feature(st, eQNAP_MD_SUPER1_FEATURE_FLEX_RAID);
    }
#endif
