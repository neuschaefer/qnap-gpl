	...
	if (c->brief) {
...
#ifdef QNAPNAS
			{
				//Patch by QNAP: Fixed bug 19463
				//We make version 0.9 can support up to about 4TB
				if (array.size == 0xFFFFFFFF)
					array.size = get_component_size(fd)/2;
		...
		if (array.raid_disks) {
			...
			printf("          State : %s%s%s%s%s%s%s \n",
			       ...
			       (array.state&(1<<MD_SB_QNAP_RESCUE))? ", Rescue": "");
		} else if (inactive) {
