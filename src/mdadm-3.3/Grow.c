...
//
#ifdef QNAPNAS
void clean_suspend(struct mdinfo *sra)
{
	sysfs_set_num(sra, NULL, "suspend_lo", 0x7FFFFFFFFFFFFFFFULL);
			      ...
			      int disks, unsigned long long blocks)
{
	...
	cache /= (pagesize/512); /* Covert from sectors to pages */
		  ...
		  int dests, int *destfd, unsigned long long *destoffsets)
{
    ...
    //  
#ifdef QNAPNAS
	clean_suspend(sra); /* remove any remaining suspension */
#else
...
#ifdef QNAPNAS
	//return 1;
	return 0; //Modified by KenChen 20071101
#else
	return 1;
#endif
}
