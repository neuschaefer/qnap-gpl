...
#if 0   // 20140627 nowadays this issue has been fixed using original code.
#ifdef QNAPNAS
//Patch by QNAP: array size overflow
		unsigned long long dsize;
		dsize = (unsigned long long)array->size << 1;

		if(array->size == 0xFFFFFFFF) { //Return from GET_ARRAY_INFO when overflow
...
#ifdef QNAPNAS
// Patch by QNAP: fail/remove non-existent device.
// i.e.: "--fail detached" or "--remove failed".
					if (dv->disposition == 'r' || 
						dv->disposition == 'f') {
						char *delimP, *dupnameP = NULL;
						int major, minor;

						dupnameP = strdup(dv->devname);
						if ((delimP = strstr(dupnameP, ":")) != 0) {
							*delimP = '\0';
							major = strtol(dupnameP, NULL, 10);
							minor = strtol(delimP + 1, NULL, 10);
