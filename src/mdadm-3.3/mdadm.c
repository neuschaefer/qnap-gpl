...
#ifdef QNAP_FLEX_RAID
		case O(CREATE, Flex):
			printf("[INFO] raid create with opt--flex \n");
			c.is_flexRaid = 1;
			continue;
#endif
		}
...
#ifdef QNAPNAS
	// If --homehost doesn't be specified by creation,
	// the hostname will be prefixed to the array name in mdadm-3.3.
	...
	case MISC:
		if (devmode == 'E') {
			...
			if (c.brief && c.verbose)
				c.brief = 2;
			rv = QdevExamine(devlist, c.scan?(c.verbose>1?0:c.verbose+1):c.brief, c.scan, c.SparcAdjust, ss, c.homehost);
#endif			
