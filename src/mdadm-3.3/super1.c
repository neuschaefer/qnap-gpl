...
#include "mdadm.h"
#ifdef QNAP_FLEX_RAID
#include "qnap_super1.h"
#endif
/*
...
#if 0
#ifdef QNAP_FLEX_RAID
	__u16	dev_roles[256];	/* role in array, or 0xffff for a spare, or 0xfffe for faulty */
	__u64	work_sectors[256];
#else
	__u16	dev_roles[0];	/* role in array, or 0xffff for a spare, or 0xfffe for faulty */
#endif
#endif
...
#ifdef QNAP_NAS
		case 0: ddsks = __le32_to_cpu(sb->raid_disks); break;
#endif
...
#ifdef QNAP_FLEX_RAID
    printf("   Qnap Feature : 0x%X \n", __le32_to_cpu(sb->qnap_feature));
#endif
...
#if QNAP_FLEX_RAID
    /* check if all the flags in superblock is supported or not */
    sbFeature = __le32_to_cpu(super->qnap_feature);
    for(featureId=0;featureId<32;featureId++){
        if((sbFeature & (1<<featureId))==0)
            continue;

        if(!qnap_super1_is_feature_supported(featureId)){
		    printf("Qnap Feature: 0x%X is not supported now\n", (1<<featureId));
            snprintf(strBuf, 128, "log_tool -a \"QNAP Feature: 0x%X is not supported now\" -t 1",(1<<featureId));
            system(strBuf);
		    exit(-1);
        }
    }
...
static struct supertype *match_metadata_desc1(char *arg)
{
	...
	if (strcmp(arg, "1.2") == 0 ||
		...
		/* 
		 * Task #7333: 
		 * Set default 1.0 if -e or --metadata is NOT specified, 
		 * due to data MD creation is specified to use 1.0 in NAS Library.
		 */
...
#ifdef QNAPNAS
//FIXME: metadata 1.x supported, but not verified yet, KenChen@QNAP
static void qexamine_super1(struct supertype *st, QEXAMINE_INFO *qinfo)
{
	struct mdp_superblock_1 *sb = st->sb;
	int i;
	unsigned long long sb_offset;
	...
	printf("Array = /dev/md%s\n", nm);

//	printf("RaidLevel = %d\n", c?__le32_to_cpu(sb->level):-2); // uknown = -2
//	qinfo->level[qinfo->cnt] = (c?__le32_to_cpu(sb->level):-2);
	if (c) {
		printf("RaidLevel = %d\n", __le32_to_cpu(sb->level));
		qinfo->level[qinfo->cnt] = __le32_to_cpu(sb->level);
	}
	else {
		printf("RaidLevel = -2\n"); // uknown = -2
		qinfo->level[qinfo->cnt] = -2;
	}

	printf("RaidDevices = %d\n", __le32_to_cpu(sb->raid_disks));
	qinfo->raid_disks[qinfo->cnt] = __le32_to_cpu(sb->raid_disks);
	//printf("TotalDevices = %d\n", __le32_to_cpu(sb->raid_disks));

	sb_offset = __le64_to_cpu(sb->super_offset);
	if (sb_offset <= 4){
		printf("Metadata = 1.1\n");
		qinfo->metadata[qinfo->cnt] = 1.1;
	}
	else if (sb_offset <= 8){
		printf("Metadata = 1.2\n");
		qinfo->metadata[qinfo->cnt] = 1.2;
	}
	else{
		printf("Metadata = 1.0\n");
		qinfo->metadata[qinfo->cnt] = 1.0;
	}
	...
	for (i=0; i<16; i++) {
		if ((i&3)==0 && i != 0){
			printf(":");
			strcat(qinfo->uuid[qinfo->cnt], ":");
		}
		printf("%02x", sb->set_uuid[i]);

		if(i==0)
			sprintf(qinfo->uuid[qinfo->cnt], "%02x", sb->set_uuid[i]);
		else
			sprintf(qinfo->uuid[qinfo->cnt], "%s%02x", qinfo->uuid[qinfo->cnt], sb->set_uuid[i]);
	}
	printf("\n");

	qinfo->events_hi[qinfo->cnt] = (unsigned long long)__le64_to_cpu(sb->events);
	qinfo->events_lo[qinfo->cnt] = 0;
	printf("Number = %d\n", __le32_to_cpu(sb->dev_number));
	qinfo->number[qinfo->cnt] = __le32_to_cpu(sb->raid_disks); // qinfo->number is employed by _qreorder for filter out spare disks
	{
		unsigned int d = 0;
		int got = -1;
		...
		for (i=0; i< __le32_to_cpu(sb->max_dev); i++) {
			...
			if (role == d) {
				...
				if (i == __le32_to_cpu(sb->dev_number))
					me = 1;
				if (cnt == 1 && me)
					got = __le16_to_cpu(sb->dev_roles[i]);
			}
		}
		if (cnt == 1 && me)
			qinfo->number[qinfo->cnt] = got;
		}
	}
	//qinfo->number[qinfo->cnt] = __le32_to_cpu(sb->dev_number);
	printf("RNumber = %d\n", qinfo->number[qinfo->cnt]);
	qinfo->cnt++;
...
int qnap_super1_is_feature_supported(eQNAP_MD_SUPER1_FEATURE feature)
{
    ...
    if(feature >= eQNAP_MD_SUPER1_FEATURE_MAX)
        return 0;
    
    if(feature == eQNAP_MD_SUPER1_FEATURE_FLEX_RAID){
        return 1;
    }
...
void qnap_super_set_feature(struct supertype *st, eQNAP_MD_SUPER1_FEATURE featureId){

    struct mdp_superblock_1 *sb;
    if(!st || !st->sb){
        printf("[ERROR] st or sb is null pointer \n");
        exit(-1);
    }


    if(strncmp("1.x", st->ss->name, sizeof(st->ss->name))!=0){
        printf("[ERROR]%s().%d featureId:%X is Not supported in this sb version\n",
            __func__, __LINE__, eQNAP_MD_SUPER1_FEATURE_FLEX_RAID);
        exit(-1);
    }


    if(!qnap_super1_is_feature_supported(featureId)){
        printf("[ERROR] md feature:%u does not supported now \n",featureId);
        exit(-1);
    }

    sb = st->sb;
    sb->qnap_feature = __cpu_to_le32(1 << featureId);
    sb->sb_csum = calc_sb_1_csum(sb);
...
#ifdef QNAPNAS
	.qexamine_super = qexamine_super1,
#endif	
#endif
