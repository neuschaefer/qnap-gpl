/*
 * $Id: conf.c,v 1.2 2012/04/16 12:54:54 ken Exp $
...
static char *conf_main_file_fixed = NULL; //QNAP
...
int conf_reload(void) {
	...
	return conf_read(conf_main_file_fixed); //QNAP
}
...
int conf_read(char *file) {
	...
	if(!conf_main_file_fixed && conf_file){ //QNAP
		 conf_main_file_fixed = strdup(conf_file);
	}
	DPRINTF(E_DBG,L_CONF,"Global config file %s\n",conf_main_file_fixed);
