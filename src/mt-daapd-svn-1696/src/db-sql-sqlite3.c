/*
 * $Id: db-sql-sqlite3.c,v 1.2 2012/04/02 07:46:43 ken Exp $
...
#include <sys/types.h> //QNAP
#include <sys/stat.h> //QNAP
...
#ifdef QNAPNAS
static char db_sqlite3_temp_directory[PATH_MAX + 1];
#endif
...
#ifdef QNAPNAS
        if (db_sqlite3_temp_directory[0] != '\0')
            sqlite3_temp_directory = sqlite3_mprintf("%s", db_sqlite3_temp_directory);
#endif
...
int db_sqlite3_open(char **pe, char *dsn) {
    ...
    if(err != DB_E_SUCCESS) {
		...
		if(db_file_existed){	//QNAP
			system("/bin/touch /tmp/.mt_daapd.err");
