...
EXPORT int pi_db_wait_update(WS_CONNINFO *pwsc) {
    ...
    int lastver=0;
	#ifdef QNAPNAS
	int timeout=60, tmp_lastver=0, waiting=0, interrupt=0;
	#endif
    IO_WAITHANDLE hwait;
    uint32_t ms;
	...
	#ifdef QNAPNAS
	tmp_lastver = lastver = db_revision();
	while(1)
	{
		// db version freeze
		if(db_revision() == lastver)
		{
			// client has the last version
			if(clientver == db_revision())
			{
		...
		// client don't have any song list, send it.
		if(clientver == 1)
			break;

		// need update to client
		// db version freeze & client has the old version
		// db version up & client has the last version
		// full scan
		if(config.reload)
		{
			tmp_lastver = db_revision();
			if(waiting && interrupt==0)
			{	
				// if the full-scan with interrupting makes version updating, reset timeout and take a version snapshot to lastver.
				// it means it's an effective full-scan
				if(tmp_lastver > lastver)
				{
					lastver = tmp_lastver;
					timeout = 60;
				}
		...
		// after full scan and wait for 60 sec
		else if(--timeout > 0)
		{
			// need update and 60sec-waiting is interrupted to full scan. 
			// after full scan, version not update.
			if(interrupt)
				interrupt = 0;
			waiting = 1;
			if(!io_wait(hwait,&ms) && (ms != 0))
		...
		// never met full scan in this function or after full scan and 60-sec timeout.
		else
			break;
	}
	#else
