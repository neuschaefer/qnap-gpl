...
#define QNAP_SERVICE_BINDING	// Define this to enable the service binding function. (Added by clko, 2011/05/19)
...
int Get_Binding_Address(PT_BIND_ADDR ptBindAddr)
{
	...
	if (NULL == (pfAddr = popen(SZP_GET_BIND_ADDR " MySQL 2>/dev/null", "r")))  return 0;
	while (NULL != fgets(szLine, sizeof(szLine), pfAddr))
	{
		...
		if (NULL != strchr(szLine, ':'))
		{
			...
			//sql_print_information("[ServBind] Binding IPv6 address: %s", szLine);
		...
		else if (NULL != strchr(szLine, '.'))
		{
			if (strstr(SZ_INADDR_ANY, szLine))
			{
				...
				//sql_print_information("[ServBind] Binding IPv4 address: %s ==> Service binding is disabled!", szLine);
				ptBindAddr->bEnabled = FALSE;
				ptBindAddr->cnIpv4 = ptBindAddr->cnIpv6 = 0;
				break;
			}
			//DEBUG_PRINTF("Binding IPv4 address: %s\n", szLine);
			//sql_print_information("[ServBind] Binding IPv4 address: %s", szLine);
...
static int Check_Binding_Address(int fdAccept, PT_BIND_ADDR ptBindAddr, char *pszAddrBuf)
{
	...
	if (ptBindAddr->bEnabled)
	{
		...
		if (-1 == getsockname(fdAccept, (struct sockaddr *)&tsaLocal, &cbSktAddr))
		{
			...
			//sql_print_information("[ServBind] Can't get the socket name of an accepted socket!");
		...
		if ((AF_INET == tsaLocal.ss_family) && (0 < ptBindAddr->cnIpv4))
		{
			...
			//DEBUG_PRINTF("Check binding address: %s, %d\n", pszAddr, iRet);
			//sql_print_information("[ServBind] Check binding address: %s, %d", pszAddr, iRet);
		}
		...
		else if(tsaLocal.ss_family == AF_LOCAL){
			iRet = 0;
			//sql_print_information("[ServBind] Check binding address: local");
		}
		else
		{
			iRet = -ENOTTY;
			//ERROR_PRINTF("Unknown interface family number: %d!\n", tsaLocal.ss_family);
			//sql_print_information("[ServBind] Unknown interface family number: %d!", tsaLocal.ss_family);
		}
	}
  ...
  */
  ...
  for (;;)
  {
	...
	reload_acl_and_cache((THD*) 0,
        ...
        {
          int  iRetDbg;
          // Get service binding settings here.
          iRetDbg = Get_Binding_Address(&g_tbaDaemon);
          //sql_print_information("[ServBind] Get_Binding_Address: %d, %d, %d, %d", iRetDbg, g_tbaDaemon.bEnabled, g_tbaDaemon.cnIpv4, g_tbaDaemon.cnIpv6);
        }
#endif /* QNAP_SERVICE_BINDING */
...
pthread_handler_t handle_connections_sockets(void *arg __attribute__((unused)))
{
  ...
  while (!abort_loop)
  {
...
#ifdef QNAP_SERVICE_BINDING
    {
    	...
    	if (0 != (iRetDbg = Check_Binding_Address(new_sock, &g_tbaDaemon, szIpAddr)))
    	{
    		// This socket connection came from a unbound interface, reject it.
    		sql_print_information("[ServBind] Reject a socket from the unbound interface: %s!", szIpAddr);
