 ...
 *
 * $Id: interface_linux.c,v 1.2 2014/04/11 12:05:41 charley Exp $
 */
...
#ifdef _QNAP_
static int qnap_is_bonding(const char *name)
{
	char fname[256];
	struct stat st;
	snprintf(fname, sizeof(fname), "/proc/net/bonding/%s", name);
	if (stat(fname, &st) == 0)
		return 1;
	return 0;
}
static unsigned long long qnap_get_if_speed(const char *name)
{
	char fname[256], buffer[512];
	unsigned long long slave_speed = 0;
	snprintf(fname, sizeof(fname), "/sys/class/net/%s/speed", name);
	FILE *fp = fopen(fname, "r");
	if (!fp) return 0;
	fscanf(fp, "%llu", &slave_speed);
	fclose(fp);
	if (4294967295 == slave_speed)
		return 0;
	slave_speed *= 1000000;	//Mbit to bit
	return slave_speed;
}

static int qnap_get_bond_all_slave_speed(const char *name, unsigned long long *speed)
{
	unsigned long long slave_speed = qnap_get_if_speed(name);
	*speed += slave_speed;
	return 0;
}
static int qnap_get_bond_max_slave_speed(const char *name, unsigned long long *speed)
{
	unsigned long long slave_speed = qnap_get_if_speed(name);
	if (slave_speed > *speed)
		*speed = slave_speed;
	return 0;
}
static unsigned long long qnap_get_bond_speed_by_slave(const char *name, int(*fn_get_slave_speed)(const char *ifname, unsigned long long *speed))
{
	unsigned long long llSpeed = 0;
	char fname[256], buffer[512];
	snprintf(fname, sizeof(fname), "/sys/class/net/%s/bonding/slaves", name);
	FILE *fp = fopen(fname, "r");
	if (!fp) return (unsigned long long)0;
	if (fgets(buffer, sizeof(buffer), fp)) {
		char *str = buffer, *slave_name = NULL, *ptr;
		while (1) {
			if ((slave_name = strtok_r(str, " \n\r\t", &ptr)) == NULL)
				break;
			str = NULL;
			fn_get_slave_speed(slave_name, &llSpeed);
		}
	}
	fclose(fp);
	return llSpeed;
}
static unsigned long long qnap_get_bond_speed(const char *name)
{
/*
Mode of operation; 
0 for balance-rr,
...
*/
	...
	int mode = -1;
	unsigned long long llSpeed=0;
	snprintf(fname, sizeof(fname), "/sys/class/net/%s/bonding/mode", name);
	FILE *fp = fopen(fname, "r");
	fscanf(fp, "%s %d", tmp, &mode);
	switch (mode) {
	case 0:
	case 2:
	case 4:
		llSpeed = qnap_get_bond_speed_by_slave(name, qnap_get_bond_all_slave_speed);
		break;
	case 1:
	case 3:
	case 5:
	case 6:
		llSpeed = qnap_get_bond_speed_by_slave(name, qnap_get_bond_max_slave_speed);
		break;
	}
	fclose(fp);
	return llSpeed;
...
netsnmp_linux_interface_get_if_speed(int fd, const char *name)
{
    ...
    if (ioctl(fd, SIOCETHTOOL, &ifr) == -1) {
        DEBUGMSGTL(("mibII/interfaces", "ETHTOOL_GSET on %s failed\n",
                    ifr.ifr_name));
#ifdef _QNAP_
		if (qnap_is_bonding(name)) {
			return qnap_get_bond_speed(name);
		}
#endif
