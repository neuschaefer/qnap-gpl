...
{
#if _QNAP_
	char szPID[128], szFWVer[64];
	Get_Model_Name(szPID, sizeof(szPID));
	Get_Profile_String("System", "Version", "", szFWVer, sizeof(szFWVer));
#if Athens
	snprintf(version_descr, sizeof(version_descr), "Linux Cisco Small Business %s %s", szPID, szFWVer);
#else
	if (Is_Generic_Version())
		snprintf(version_descr, sizeof(version_descr), "NAS");
	else
		snprintf(version_descr, sizeof(version_descr), "Linux %s %s", szPID, szFWVer);
#endif
#else	//_QNAP_
#ifdef HAVE_UNAME
    ...
    /* default sysObjectID */
    memcpy(sysObjectID, version_sysoid, version_sysoid_len * sizeof(oid));
    sysObjectIDLength = version_sysoid_len * sizeof(oid);
