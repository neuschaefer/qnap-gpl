...
#ifdef _QNAP_
#define DOS_ADDR_SIZE 30
typedef struct {
	char *addr;
	unsigned int sec;
	unsigned int count;
} DoS_REC;
DoS_REC dosAddr[DOS_ADDR_SIZE];
DoS_REC ddos_rec;
//unsigned int ddos_sec = 0;
//unsigned int ddos_count = 0;
int detect_ddos = 1;
#ifdef Athens
unsigned int ddos_max_count = 30;
#else
unsigned int ddos_max_count = 300;
#endif
unsigned int ddos_alert_interval = 60;
unsigned int ddos_prev_alert_time = 0;
#endif
...
#ifdef _QNAP_
static int is_Rec_DoS(DoS_REC *rec,  unsigned now_sec)
{
	if (rec->sec == now_sec) {
		if (++ rec->count >= ddos_max_count)
			return 1;
	}
	else {
		rec->sec = now_sec;
...
static int is_DoS(char *addr,  unsigned now_sec)
{
	int i, rec_index = -1;
	unsigned int oldest_sec = (unsigned int)-1;
	for (i = 0; i < DOS_ADDR_SIZE; i++) {
		if (dosAddr[i].addr && strcmp(addr, dosAddr[i].addr) == 0) {
			return is_Rec_DoS(dosAddr+i, now_sec);
		}
	}
	for (i = 0; i < DOS_ADDR_SIZE; i++) {
		if (!dosAddr[i].addr) {
		...
		if (dosAddr[i].sec < oldest_sec) {
			rec_index = i;
			oldest_sec = dosAddr[i].sec;
		}
	}
	if (rec_index >= 0) {
		if (dosAddr[rec_index].addr)
			free(dosAddr[rec_index].addr);
		dosAddr[rec_index].addr = strdup(addr);
		dosAddr[rec_index].sec = now_sec;
		dosAddr[rec_index].count = 1;
	}
	return 0;
}
static int is_DDoS(unsigned now_sec)
{
	return is_Rec_DoS(&ddos_rec, now_sec);
}
#endif
/*
...
if (detect_ddos) {
	char *ip, *end_ip = NULL;
	if ((ip = strchr(addr, '[')) != NULL) {
		ip ++;
		if ((end_ip = strchr(addr, ']')) != NULL)
			*end_ip = 0;
	}
	else
		ip = addr;
	if (is_DoS(ip, now.tv_sec) || is_DDoS(now.tv_sec)) {
		if (ddos_prev_alert_time < now.tv_sec && now.tv_sec - ddos_prev_alert_time > ddos_alert_interval) {
			char cmd[1024];
			ddos_prev_alert_time = now.tv_sec;
			snprintf(cmd, sizeof(cmd), "/sbin/log_tool -t 1 -u \"\" -p \"%s\" -m \"\" -a \"[SNMP] DoS detected\" >/dev/null 2>&1", ip);
			system(cmd);
		}
	}
	if (end_ip)
