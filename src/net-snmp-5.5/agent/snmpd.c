...
#ifdef _QNAP_
extern int snmpd_listen_port;
extern int detect_ddos;
extern unsigned int ddos_max_count;
#endif
#ifdef WIN32SERVICE
/*
 * SNMP Agent Status 
...
// Define this to enable QNAP service binding.
// Note: If QNAP service binding is enabled, the "agentaddress" field in the snmpd config file (/etc/config/snmpd.conf) will not work anymore.
// But ipv6 need the "agentaddress" field in the snmpd config file if QNAP service binding is disabled.
...
#define SZP_NETWORK_LOCK		"/tmp/touch_network"
#define	SZ_INADDR_ANY			"0.0.0.0"
//#define CBMAX_AGENT_ADDRESS		(MAX_BINDING_ADDRESS*(INET6_ADDRSTRLEN+16)+8)	// The maximum string size of the agent address for service binding.
#define CS_TIMEOUT				30
 ...
 */
int Get_Binding_Address(PT_BIND_ADDR ptBindAddr)
{
	...
	if (NULL == (pfAddr = popen(SZP_GET_BIND_ADDR " SNMP -netif 2>/dev/null", "r")))  return 0;
...
/** \brief To get the updated agent address for service binding.
 *
 * \param cbBuf The size of the buffer to get the agent address.
 * \param pszBuf The buffer to get the agent address.
 * \return	It returns 0 if successful, or ENODATA if the service-binding is disabled, or -XXX if error happened.
 *			-EOVERFLOW			If the buffer is too small.
 */
static int Get_Updated_Agent_Address(char **ppszBuf)
{
	int  iRet = 0, csTimeout=0, bNetRestart=1;
	T_BIND_ADDR  tbaDaemon;
	if (!ppszBuf)  return -EINVAL;

	// if network.sh restart, it'll call kill -HUP snmpd in hostname.sh. At this moment snmpd will get 169.254.100.100
	// in Get_Binding_Address. So, we should wait network.sh restart complete and all ip addresses are valid.
	while(csTimeout < CS_TIMEOUT)
	{
		struct stat tStat;
		if(0 != stat(SZP_NETWORK_LOCK, &tStat))
		{
			bNetRestart = 0;
			break;
		}
	...
	if(bNetRestart)
		return ENODATA;
	memset(&tbaDaemon, 0, sizeof(T_BIND_ADDR));
	// If the service-binding is enabled.
	if ((0 == (iRet = Get_Binding_Address(&tbaDaemon))) && tbaDaemon.bEnabled)
	{
		int  idx, cbAllAddr = 0;
		// ex: agentaddress udp6:[fe80::208:9bff:fe00:59%eth0]:161, udp6:[%eth0]:161 => 16 chars, and 7 ',' cause's max 8 addresses.
		//char  pszAllBindAddr[CBMAX_AGENT_ADDRESS];
		char *pszAllBindAddr=NULL;

		if(NULL == (pszAllBindAddr = (char*)malloc(((tbaDaemon.cnIpv4+tbaDaemon.cnIpv6)*(INET6_ADDRSTRLEN+16)+8))))
		{
			Release_Bind_Addr(&tbaDaemon);
			return ENOMEM;
		}
		// Format the agentaddress string.
		for(idx = 0; idx < tbaDaemon.cnIpv4; idx++)
		{
			sprintf(pszAllBindAddr+cbAllAddr, "udp:%s:%d", tbaDaemon.ryszIpv4[idx], snmpd_listen_port);
			if((idx != (tbaDaemon.cnIpv4 - 1)) || (tbaDaemon.cnIpv6))  strcat(pszAllBindAddr, ",");
			cbAllAddr = strlen(pszAllBindAddr);
		}
		for(idx = 0; idx < tbaDaemon.cnIpv6; idx++)
		{
			if(!strncmp(tbaDaemon.ryszIpv6[idx], "::ffff:", 7))
				continue;
			sprintf(pszAllBindAddr+cbAllAddr, "udp6:[%s]:%d", tbaDaemon.ryszIpv6[idx], snmpd_listen_port);
			if(idx != (tbaDaemon.cnIpv6 - 1))  strcat(pszAllBindAddr, ",");
			cbAllAddr = strlen(pszAllBindAddr);
		}
		// Update to buffer.
		//if (cbAllAddr >= cbBuf)  return -EOVERFLOW;
		*ppszBuf = pszAllBindAddr;
		Release_Bind_Addr(&tbaDaemon);
		return 0;
	}
	return ((0 == iRet) ? ENODATA : iRet);
}
...
#ifdef _QNAP_
    detect_ddos = Get_Profile_Boolean("SNMP", "EnableDetectDDoS", FALSE);
    ddos_max_count = Get_Profile_Integer("SNMP", "MaxPacketPerSecond", 300);
#endif
...
#ifdef QNAP_SERVICE_BINDING
	{
		char  *pszAgentAddr=NULL;
		// Update the agentaddress if the service-binding is enabled.
		if (0 == Get_Updated_Agent_Address(&pszAgentAddr))
		{
			netsnmp_ds_set_string(NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_PORTS, pszAgentAddr);
			//snmp_log(LOG_INFO, "[ServBind] The new agentaddress is: %s\n", szAgentAddr);
		}
		// Do nothings if the service-binding is disabled.
		//snmp_log(LOG_INFO, "%s.%d, agentaddress[%s]\n", __FUNCTION__, __LINE__, netsnmp_ds_get_string(NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_PORTS));
	}
#endif /* QNAP_SERVICE_BINDING */
...
receive(void)
{
    ...
    while (netsnmp_running) {
	    ...
	    snmp_log(LOG_INFO, "NET-SNMP version %s restarted\n",
		     netsnmp_get_version());
#ifdef QNAP_SERVICE_BINDING
			// First clear the old agentaddress field to avoid appending the agentaddress field of config
			//snmp_log(LOG_INFO, "%s.%d, agentaddress[%s]\n", __FUNCTION__, __LINE__, netsnmp_ds_get_string(NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_PORTS));
			netsnmp_ds_set_string(NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_PORTS, NULL);
#endif /* QNAP_SERVICE_BINDING */
            update_config();
...
#ifdef QNAP_SERVICE_BINDING
			{
				...
				// Re-register the agnet nsap.
				shutdown_master_agent();
				if (0 != (iRet = init_master_agent()))
				{
					snmp_log(LOG_INFO, "[ServBind] ERROR: Can't reinitialize the master agent. Program will exit. (%d)\n", iRet);
