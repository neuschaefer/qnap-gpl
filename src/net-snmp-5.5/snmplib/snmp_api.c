...
{
    char port_str[64];
    snprintf(port_str, sizeof(port_str), ":%d", snmpd_listen_port);
    netsnmp_register_default_target("snmp", "udp", port_str);
    netsnmp_register_default_target("snmp", "tcp", port_str);
    netsnmp_register_default_target("snmp", "udp6", port_str);
    netsnmp_register_default_target("snmp", "tcp6", port_str);

    snprintf(port_str, sizeof(port_str), ":%d", snmpd_listen_port+1);
    netsnmp_register_default_target("snmptrap", "udp", port_str);
    netsnmp_register_default_target("snmptrap", "tcp", port_str);
    netsnmp_register_default_target("snmptrap", "udp6", port_str);
    netsnmp_register_default_target("snmptrap", "tcp6", port_str);
}
#else
 ...
 *
 * @return -1 if name1 < name2, 0 if name1 = name2, 1 if name1 > name2
 * @return -2 if name1 < name2, 2 if name1 > name2, and 2 or -2 both mean these are not in the same branch
 */
int
Q_snmp_oid_compare(const oid * in_name1,
                 size_t len1, const oid * in_name2, size_t len2)
{
    register int    len;
    register const oid *name1 = in_name1;
