...
#ifdef _QNAP_NFS_
static exports		get_export_share_list (void);
#else
...
#ifdef _QNAP_NFS_
	NAS_SHARE_INFO  shinfo;
	int     ret, len;
	int     flg_slash=0;
	char *ptr, *subdir = NULL;

	len = strlen(*path);
	if ( len >= SHARE_NAME_LENGTH - 1 || len < 0 )
		goto exit_getshare ;

	if ((*path) && (!strncmp(*path, "/share/", 7))){
		strcpy (shinfo.share_name, strrchr( *path, '/')+1);
	}
	else if ( **path == '/' ) {
		flg_slash = 1 ;
		strcpy (shinfo.share_name, (*path+1)) ;
	} else
		strcpy (shinfo.share_name, *path) ;
	// maybe there is sub folder
	//	CharleyCheng 2010.01.22
	if ((ptr = strchr(shinfo.share_name, '/'))) {
		subdir = strdup(ptr);
		*ptr = 0;
	}

	ret = Get_NAS_Share_Info_Ex (&shinfo) ;
	if ( ret != 0 ) {
		goto exit_getshare ;
	}

	xfree (*path) ;
	*path = (char *) xmalloc (MAX_LINE_LENGTH*sizeof (char)) ;
	strncpy (*path, shinfo.path, MAX_LINE_LENGTH) ;
	//Append sub folder back.
	//	CharleyCheng 2010.01.22
	if (subdir) {
		strncat(*path, subdir, MAX_LINE_LENGTH);
		free(subdir);
	}
exit_getshare:
...
mount_umnt_1_svc(struct svc_req *rqstp, dirpath *argp, void *resp)
{
	...
	NAS_SHARE_INFO  shinfo ;
	char share_path[MAXPATHLEN+1];
	int     ret, len ;
	int     flg_slash=0 ;
	char *ptr, *subdir = NULL;

	len = strlen(*argp);
	if ( len >= MAXPATHLEN - 1 || len < 0 )
		goto umount_exit ;

	if ((*argp) && (!strncmp(*argp, "/share/", 7))){
		strcpy (share_path, strrchr( *argp, '/')+1);
	}
	else if ( **argp == '/' ) {
		flg_slash = 1 ;
		strcpy (share_path, (*argp+1)) ;
	} else
		strcpy (share_path, *argp) ;
	// maybe there is sub folder
	//	CharleyCheng 2010.01.22
	if ((ptr = strchr(share_path, '/'))) {
		subdir = strdup(ptr);
		*ptr = 0;
	}
	strncpy(shinfo.share_name, share_path, sizeof(shinfo.share_name)-1);
	shinfo.share_name[sizeof(shinfo.share_name)-1] = 0;

	ret = Get_NAS_Share_Info_Ex (&shinfo) ;
	if ( ret != 0 ) {   // The mount point is not exist anymore
		*p = '\0' ;
		goto umount_exit ;
	}
	xfree (*argp) ;
	*argp = (char *) xmalloc (MAX_LINE_LENGTH*sizeof (char)) ;
	strncpy (*argp, shinfo.path, MAX_LINE_LENGTH) ;
	//Append sub folder back.
	//	CharleyCheng 2010.01.22
	if (subdir) {
		strncat(*argp, subdir, MAX_LINE_LENGTH);
		free(subdir);
	}
	p = *argp;
umount_exit:

xlog(L_NOTICE, "umount [%s]",*argp);//KenChen e.g. argp=/share/Public
#endif
...
#ifdef _QNAP_NFS_
	if ((*resp = get_export_share_list()) == NULL)
...
mount_mnt_3_svc(struct svc_req *rqstp, dirpath *path, mountres3 *res)
{
	...
	xlog(L_ALL, "mount_mnt_3_svc(%s) begin", *path);
//To get the share information from the database.
//The "path" contains the share name information.
//We could both accept the share name format with the prefix slash symbol or not.

/*	
	if ( is_addr_accessible(rqstp->rq_xprt->xp_raddr.sin_addr.s_addr) == 0 ) {
...
*/
	...
	if ((*path) && (!strncmp(*path, "/share/", 7))){
		strcpy (share_path, strrchr( *path, '/')+1);
	}
	else if ( **path == '/' ) {
		flg_slash = 1 ;
		strcpy (share_path, (*path+1)) ;
	} else {
		strcpy (share_path, *path) ;
	}
	// maybe there is sub folder
	//	CharleyCheng 2010.01.22
	if ((ptr = strchr(share_path, '/'))) {
	...
	xlog(L_ALL, "share_path(%s)", share_path);
	strncpy(shinfo.share_name, share_path, sizeof(shinfo.share_name)-1);
	shinfo.share_name[sizeof(shinfo.share_name)-1] = 0;
	...
	xlog(L_ALL, "shinfo.path(%s)", shinfo.path);
...
#ifdef _QNAP_NFS_
static exports
get_export_share_list(void)
{
	static exports      elist = NULL ;
	struct exportnode   *e, *tmpe ;
	struct groupnode    *g ;
	int         i, ret ;
	char            *namebuf, *empty ;
...
xlog(L_NOTICE, "%s\n", __FUNCTION__);
	...
	elist = NULL ;
	ret = Get_NAS_Share_List_Ex(&share_list);
	if (ret<0) {
		return elist ;
	}
	for (i=0; i<ret; i++) {
		...
		empty = (char *) xmalloc (4*sizeof (char)) ;
		strcpy (empty, " ") ;
		e->ex_next = elist ;
		g->gr_name = empty ;
		g->gr_next = NULL ;
		e->ex_groups = g ;
		namebuf = (char *) xmalloc (sizeof(share_list[i].section_name) * sizeof (char)) ;
		sprintf(namebuf, "/%s", share_list[i].section_name);
		e->ex_dir = namebuf ;
		elist = e;
	}
