...
mount_mnt_1_svc(struct svc_req *rqstp, dirpath *path, fhstatus *res)
{
	...
	ret=Get_Private_Profile_String(shinfo.share_name, "path", "", spath, MAX_LINE_LENGTH, SAMBA_CONF_PATH);
	if ( ret <= 0 ) {
		goto exit_getshare ;
	}
	xfree (*path) ;
	*path = (char *) xmalloc (MAX_LINE_LENGTH*sizeof (char)) ;
	strncpy (*path, spath, MAX_LINE_LENGTH) ; 
...
mount_umnt_1_svc(struct svc_req *rqstp, dirpath *argp, void *UNUSED(resp))
{
	...
	*argp = (char *) xmalloc (MAX_LINE_LENGTH*sizeof (char)) ;
	strncpy (*argp, spath, MAX_LINE_LENGTH) ;
	// Append sub folder back.
	// CharleyCheng 2010.01.22
	if (subdir) {
