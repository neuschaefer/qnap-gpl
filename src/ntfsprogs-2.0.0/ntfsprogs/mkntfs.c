...
int qnap_progress = -1;
static void init_qnap_progress(const char *dev)
{
...
static int set_qnap_progress(int progress)
{
        char cmd[512];
...
static int mkntfs_redirect(struct mkntfs_options *opts2)
{
	...
	int i, err;
#ifdef QNAP
	init_qnap_progress(opts2->dev_name);
	set_qnap_progress(0);	
#endif
	if (!opts2) {
