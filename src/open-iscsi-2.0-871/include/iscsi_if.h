...
struct iscsi_uevent {
	...
	union {
		...
		struct msg_bind_conn {
			...
			uint32_t	is_leading;
//Patch by QNAP:fix mount fail in x86_64 platform
			uint32_t	reserved;
		} b_conn;
...
#ifdef VIRTUAL_JBOD
/* tricky method to treat it as bitmap setting */
#define CONNECT_NON_BLOCKING			(1 << 0)
#define CONNECT_VJBOD_ISER_BIND_SRC_ADDR	(1 << 1)
#endif
...
#ifdef VIRTUAL_JBOD
	/* take care this, the enum iscsi_host_param MAY NOT
	 * be the same as in kernel side (include/scsi/iscsi_if.h)
	 */
