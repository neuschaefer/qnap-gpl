...
#ifdef __QNAP__
static int add_portal_raw(struct list_head *rec_list, discovery_rec_t *drec,
		      char *targetname, char *address, char *port, char *tag)
{
	struct sockaddr_storage ss;
	char host[NI_MAXHOST];
	struct node_rec *rec;
	char *addr_bak = strdup(address);

	/* resolve the address, in case it was a DNS name */
	if (resolve_address(addr_bak, port, &ss)) {
		log_error("cannot resolve %s", addr_bak);
		return 0;
	}
	...
	log_debug(5, "addr_bak=[%s], host=[%s]", addr_bak, host);
		      ...
		      char *targetname, char *address, char *port, char *tag)
{
	...
	strlcpy(rec->conn[0].address, address, NI_MAXHOST);
#ifdef __QNAP__
	log_debug(5, "rec conn address=[%s]", address);
#endif
	list_add_tail(&rec->list, rec_list);
	return 1;
}
		  ...
		  struct list_head *rec_list, char *default_port)
{
	...
	if (text >= end) {
		if (drec->address == NULL) {
			...
			log_debug(8, "TargetAddress not found!!");
			snprintf(str_port, sizeof(str_port), "%d", drec->port);
			snprintf(str_tag, sizeof(str_tag), "%d", def_tag);
			log_debug(7, "assume add_portal(%s, %s:%s, %s) for %s", name, drec->address, str_port, str_tag, drec->address);
			if (!add_portal(rec_list, drec, name, drec->address, str_port, str_tag)) {
				log_error("failed to add assume portal, ignoring target %s", name);
				return 0;
			}
			return 1;
#else
	...
	while (text < end) {
		...
		if (strncmp(text, "TargetAddress=", 14) == 0) {
			...
			log_debug(7, "add_portal(%s, %s:%d, %s) for %s", name, address, port, tag, drec->address);
...
#ifdef __QNAP__
			if (strcasecmp(address, drec->address) == 0)
				is_match = 1;
			if (tag_count < 256) {
				int is_exist = 0;
	...
	if (!is_match) {
		char port[12];
		snprintf(port, sizeof(port), "%d", drec->port);
		while (-- tag_count >= 0) {
			if (!add_portal_raw(rec_list, drec, name, drec->address, port,
					tag_list[tag_count])) {
				log_error("failed to add extra portal, "
					 "ignoring target %s [%s:%s]", name, drec->address, port);
				return 1;
			}
		}
	}
...
#ifdef __QNAP__
	/* original design will not do logout after to do disocvery process,
	 * it may not make sense here
	 */
	if (rc == 0){
		log_debug(7, "start to logout after discovery action");
		iscsi_logout_and_disconnect(session);
	}
#endif
