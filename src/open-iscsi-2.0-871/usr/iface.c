...
#ifdef VIRTUAL_JBOD
			if (!strlen(iface->qnap_netdev))
				strcpy(iface->qnap_netdev, def_iface->qnap_netdev);
#endif
		} else {
			iface_init(iface);
...
#ifdef VIRTUAL_JBOD
	if (strlen(src->qnap_netdev))
		strcpy(dst->qnap_netdev, src->qnap_netdev);
#endif
}
...
#ifdef VIRTUAL_JBOD
int iface_is_bound_by_qnap_netdev(struct iface_rec *iface)
{
	if (iface && strlen(iface->qnap_netdev) 
	&& strcmp(iface->qnap_netdev, DEFAULT_HWADDRESS)
	)
		return 1;
	return 0;
}
...
#ifdef VIRTUAL_JBOD
	if (strlen(iface->qnap_netdev))
		printf("%sIface QNAP Netdev: %s\n", prefix, iface->qnap_netdev);
	else
		printf("%sIface QNAP Netdev: %s\n", prefix, UNKNOWN_VALUE);
#endif
}
