...
#ifdef __QNAP__
	/* 1. do nothing (normal login) if authmethod = 0 (None)
	 * even user create username / password
	 *
	 * 2. do nothing (normal login) if authmethod = 1 (CHAP)
	 * even user did NOT create username / password
	 */
	if (auth_cfg->authmethod == 0)
		return;
#endif
...
#ifdef VIRTUAL_JBOD
	session->skip_probe_sd = rec->skip_probe_sd;
#endif
...
setup_full_feature_phase(iscsi_conn_t *conn)
{
...
#ifdef VIRTUAL_JBOD
		{
			.param = ISCSI_HOST_PARAM_QNAP_NETDEV_NAME,
			.value = session->nrec.iface.qnap_netdev,
			.type = ISCSI_STRING,
			.set = 1,
		},
#endif
	...
	for (i = 0; i < MAX_HOST_PARAMS; i++) {
		if (!hosttbl[i].set)
			continue;
#ifdef VIRTUAL_JBOD
		if (hosttbl[i].param == ISCSI_HOST_PARAM_QNAP_NETDEV_NAME) {
			if (!session->skip_probe_sd)
				continue;
			if (!iface_is_bound_by_qnap_netdev(&session->nrec.iface))
				continue;
		}
#endif
...
retry_create:
#ifdef VIRTUAL_JBOD
	/* only 16 bit length data will be passed to create_session() */
	tmp_cmds_max = session->nrec.session.cmds_max;

	if (tmp_cmds_max >= 0x0000fffd)
		tmp_cmds_max = 0x0000fffd;

	if (tmp_cmds_max == 0 || tmp_cmds_max == 1)
		tmp_cmds_max = 2;
	else {
		if (tmp_cmds_max & 0x00000001)
			tmp_cmds_max += 1;
	}

	/* For session->nrec.session.cmds_max, there are some
	 * limitations for it and please refer the iscsi_session_setup().
	 *
	 * 1. ISCSI_TOTAL_CMDS_MIN <= cmds_max (total) <= ISCSI_TOTAL_CMDS_MAX
	 * 2. and, cmds_max must be power of 2
	 * For theses, we take bit 0 to be a temp buffer for VIRTUAL JBOD,
	 * don't worry about 2^0 = 1 for cmds_max this case
	 */
	if (session->skip_probe_sd == 1)
		tmp_cmds_max |= 0x00000001;

	session->nrec.session.cmds_max = tmp_cmds_max;
#endif
...
#ifdef VIRTUAL_JBOD
	if (iface_is_bound_by_qnap_netdev(iface) 
	&& session->skip_probe_sd
	) 
	{
		rc = __iscsi_host_set_param(t, session->hostno,
					    ISCSI_HOST_PARAM_QNAP_NETDEV_NAME,
					    iface->qnap_netdev, ISCSI_STRING);
		if (rc)
			return rc;
	}
#endif
