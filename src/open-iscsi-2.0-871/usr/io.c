...
/* following is tricky way ... please take care it */
#define VJBOD_WAY_BIT    (1)

static inline int *__qnap_is_vjbod_way_bit(char *data)
{
	return (((unsigned long)data & VJBOD_WAY_BIT) != 0);
}

static inline void *__qnap_set_vjbod_way_bit(char *data)
{
	unsigned long p = (unsigned long)data;
	return ((void *)(p + VJBOD_WAY_BIT));
}

static inline void *__qnap_clear_vjbod_way_bit(char *data)
{
	unsigned long p = (unsigned long)data;
	return ((void *)(p - VJBOD_WAY_BIT));
}

static int qnap_vjbod_get_netdev_from_hwaddress(
	struct iface_rec *iface, 
	char *netdev
	)
{
	char *qnap_netdev = NULL;
	unsigned long p;
	int ret;

	qnap_netdev = calloc(1, IFNAMSIZ);
	if (!qnap_netdev)
		return 0;

	/* tricky method !! */
	snprintf(qnap_netdev, IFNAMSIZ, "%s", iface->qnap_netdev);
	p = __qnap_set_vjbod_way_bit(qnap_netdev);

	ret = get_netdev_from_hwaddress(iface->hwaddress, (char *)p);
	if (ret == 1)
		snprintf(netdev, IFNAMSIZ, "%s", qnap_netdev);
	else
		log_error("[VJBOD] Cannot match %s to net/scsi interface: %s",
			iface->hwaddress, iface->qnap_netdev);
...
static int get_netdev_from_hwaddress(char *hwaddress, char *netdev)
{
	...
	if (__qnap_is_vjbod_way_bit(netdev)) {
		log_debug(4, "[VJBOD] get VJBOD way for get_netdev_from_hwaddress()");
		is_vjbod_way = 1;
		/* for vjbod way, the netdev buffer contains the device string
		 * we want to match
		 */
		netdev = __qnap_clear_vjbod_way_bit(netdev);
	}
#endif
...
#ifdef VIRTUAL_JBOD
			/* For qnap vjbod way, if we bind from hwaddress, we need
			 * to check nic interface again to avoid we encounter
			 * virtual-switch case (bridge dev)
			 * Especially, the system may contains eth0, mgmt0 and
			 * qvs0 and their hwaddress are all the SAME ...
			 */
			if (is_vjbod_way) {
				/* For vjbod way, netdev buffer contains the
				 * interface name we want to get
				 */
				if (strcasecmp(netdev, n->if_name))
					continue;

				log_debug(4, "[VJBOD] found matched "
					"nic interface, netdev:%s, "
					"n->if_name:%s", netdev, n->if_name);
			}
#endif
...
#ifdef VIRTUAL_JBOD
	/* check is it vjbod way first, this call comes from following path
	 *
	 * ---> bind_conn_to_iface()
	 * --> iscsi_io_tcp_connect()
	 * -> iscsi_conn_connect()
	 */
	if (session->skip_probe_sd
	&& iface_is_bound_by_hwaddr(iface)
	)
	{
		if(!qnap_vjbod_get_netdev_from_hwaddress(iface, session->netdev))
			return -1;
	} 
	else
#endif
