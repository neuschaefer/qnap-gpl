...
#ifdef VIRTUAL_JBOD
	/* if not found just print out default */
	ret = sysfs_get_str(id, ISCSI_HOST_SUBSYS, "qnap_netdev",
			    iface->qnap_netdev, sizeof(iface->qnap_netdev));
	if (ret)
		log_debug(7, "could not read qnap netdev for host%d\n", host_no);
#endif
...
#ifdef VIRTUAL_JBOD
int __iscsi_sysfs_get_dev_vendor(char *value, int host_no, int target, int lun)
{
	char id[NAME_SIZE];

	snprintf(id, sizeof(id), "%d:0:%d:%d", host_no, target, lun);
	if (sysfs_get_str(id, SCSI_SUBSYS, "vendor", value,
			  SCSI_ATTR_BUF_LEN)) {
		return EIO;
	}
	return 0;
...
int __iscsi_sysfs_get_dev_model(char *value, int host_no, int target, int lun)
{
	char id[NAME_SIZE];

	snprintf(id, sizeof(id), "%d:0:%d:%d", host_no, target, lun);
	if (sysfs_get_str(id, SCSI_SUBSYS, "model", value,
			  SCSI_ATTR_BUF_LEN)) {
		return EIO;
	}
	return 0;
...
int __iscsi_sysfs_get_dev_naa(char *value, int host_no, int target, int lun)
{
	char id[NAME_SIZE];

	snprintf(id, sizeof(id), "%d:0:%d:%d", host_no, target, lun);
	if (sysfs_get_str(id, SCSI_SUBSYS, "qnap_dev_naa", value,
			  SCSI_ATTR_BUF_LEN)) {
		return EIO;
	}
	return 0;
...
int __iscsi_sysfs_get_probe_sd_done(int *value, int host_no, int target, int lun)
{
	char id[NAME_SIZE];

	snprintf(id, sizeof(id), "%d:0:%d:%d", host_no, target, lun);

	if(sysfs_get_int(id, SCSI_SUBSYS, "qnap_probe_sd_done", value))
		return EIO;
	return 0;
}

int __iscsi_sysfs_get_probe_sd_type(int *value, int host_no, int target, int lun)
{
	char id[NAME_SIZE];

	snprintf(id, sizeof(id), "%d:0:%d:%d", host_no, target, lun);

	if(sysfs_get_int(id, SCSI_SUBSYS, "qnap_probe_sd_type", value))
		return EIO;
	return 0;
}

int __iscsi_sysfs_set_probe_sd_type(char *buf, int host_no, int target, int lun)
{
	char id[NAME_SIZE];
	...
	snprintf(id, sizeof(id), "%d:0:%d:%d", host_no, target, lun);
	if(sysfs_set_param(id, SCSI_SUBSYS, "qnap_probe_sd_type", buf,
			strlen(buf)))
		return EIO;
...
int __iscsi_sysfs_set_probe_sd(void *data, int host_no, int target, int lun)
{
	char *write_buf = "1";
	char id[NAME_SIZE];

	snprintf(id, sizeof(id), "%d:0:%d:%d", host_no, target, lun);
	if(sysfs_set_param(id, SCSI_SUBSYS, "qnap_probe_sd", write_buf,
			strlen(write_buf)))
		return EIO;
...
int __iscsi_sysfs_set_remove_sd(void *data, int host_no, int target, int lun)
{
	char *write_buf = "1";
	char id[NAME_SIZE];

	snprintf(id, sizeof(id), "%d:0:%d:%d", host_no, target, lun);
	if(sysfs_set_param(id, SCSI_SUBSYS, "delete", write_buf,
			strlen(write_buf)))
		return EIO;
...
int __iscsi_sysfs_get_dev_state(char *buf, int host_no, int target, int lun)
{
	char id[NAME_SIZE];

	snprintf(id, sizeof(id), "%d:0:%d:%d", host_no, target, lun);
	if (sysfs_get_str(id, SCSI_SUBSYS, "state", buf,
			  SCSI_ATTR_BUF_LEN)) {
		return EIO;
	}
	return 0;
...
int __iscsi_sysfs_get_dev_size_info(char *buf, int host_no, int target, int lun)
{
	char id[NAME_SIZE];

	snprintf(id, sizeof(id), "%d:0:%d:%d", host_no, target, lun);
	if (sysfs_get_str(id, SCSI_SUBSYS, "qnap_dev_size_info", buf,
			  SCSI_ATTR_BUF_LEN)) {
		return EIO;
	}
	return 0;
...
int __iscsi_sysfs_get_dev_sd_name(char *buf, int host_no, int target, int lun)
{
	char id[NAME_SIZE];

	snprintf(id, sizeof(id), "%d:0:%d:%d", host_no, target, lun);
	if (sysfs_get_str(id, SCSI_SUBSYS, "qnap_dev_name", buf,
			  SCSI_ATTR_BUF_LEN)) {
		return EIO;
	}
	return 0;
...
int __iscsi_sysfs_get_dev_sn(char *buf, int host_no, int target, int lun)
{
	char id[NAME_SIZE];

	snprintf(id, sizeof(id), "%d:0:%d:%d", host_no, target, lun);
	if (sysfs_get_str(id, SCSI_SUBSYS, "qnap_dev_sn", buf,
			  SCSI_ATTR_BUF_LEN)) {
		return EIO;
	}
	return 0;
