...
#define NLM_SETPARAM_DEFAULT_MAX \
	(NI_MAXHOST + NI_MAXHOST + 1 + sizeof(struct iscsi_uevent))
#else
#define NLM_SETPARAM_DEFAULT_MAX \
...
ktransport_ep_connect(iscsi_conn_t *conn, int non_blocking)
{
	...
	/* This is only for vjbod. 
	 *
	 * the backtrace was comed from following list.
	 * session_login_task()
	 * - iscsi_conn_connect()
	 * -- xxx->ep_connect()
	 *
	 * And, __session_create() was called already
	 *
	 * hummm ... code is to support ipv4 now .. it shall be modified in future
	 */
	if (sess->skip_probe_sd && !qlib_check_do_iser_bind(conn, AF_INET)) {
		int dst_addrlen = addrlen;
		int src_addr_len = sizeof(struct sockaddr_in);

		/* conn->bind_ep will be set to 1 in iface_set_param() and 
		 * it was used in ktransport_ep_connect() now
		 */
		if (conn->bind_ep)
			ev->u.ep_connect_through_host.non_blocking |= (CONNECT_VJBOD_ISER_BIND_SRC_ADDR);
		else
			ev->u.ep_connect.non_blocking |= (CONNECT_VJBOD_ISER_BIND_SRC_ADDR);

		/* dst addr */
		memcpy(setparam_buf + sizeof(*ev), dst_addr, dst_addrlen);

		/* src addr */
		memcpy(setparam_buf + sizeof(*ev) + dst_addrlen, 
			(struct sockaddr *)&conn->qnap_iser_bind_addr, src_addr_len);

		addrlen = dst_addrlen + src_addr_len;
	} 
	else
#endif
