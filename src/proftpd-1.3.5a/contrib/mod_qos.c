...
#ifdef _QNAP_
      // Jacky20140911 - Debug
      pr_log_debug(DEBUG4, "QoS : DataQoS = %s/%d", cmd->argv[i], dataqos);
#endif
      if (dataqos == -1) {
...
#ifdef _QNAP_
      // Jacky20140911 - Debug
      pr_log_debug(DEBUG4, "QoS : CtrlQoS = %s/%d", cmd->argv[i], ctrlqos);
#endif
...
#ifdef _QNAP_
  // Jacky - 20140911 : Add IPv6 Support
  config_rec *c = find_config(sc->server->conf, CONF_PARAM, "QoSOptions", FALSE);
  if (c) {
      int qos = *((int *) c->argv[0]);
      if (qos != 0) {
          if (pr_netaddr_get_family(sc->addr) == AF_INET) {
              if(setsockopt(sc->sockfd, ip_level, IP_TOS, (void *) &qos, sizeof(qos)) < 0) {
...
#ifdef IPV6_TCLASS
              if(setsockopt(sc->sockfd, IPPROTO_IPV6, IPV6_TCLASS, (void *) &qos, sizeof(qos)) < 0) {
                  pr_log_pri(PR_LOG_NOTICE, MOD_QOS_VERSION
                          ": error setting control socket IPV6_TCLASS: %s", strerror(errno));
              }
#endif
          }
      }
