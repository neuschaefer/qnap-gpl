...
#define NVR_EVENT_REPORT_USER_ACCOUNT_PREFIX	"_nvrevtrpt"
#endif
...
struct server_struc;
#ifdef _QNAP_
int g_home;           // chroot to home	    0: disable 1: enable
int l_home;           // make link to home  0: disable 1: enable
extern int custom_root;
extern char custom_root_path[512];
#endif
...
#ifdef _QNAP_
  int user_type;        // 0: local user, 1: NT DOMAIN user, 2: AD user
#endif
...
#ifdef _NVR_
typedef struct ms_fen_info_st
{
    char *address;
    int ftp_evt_notifiy_enabled;
} TMsFenInfo;

typedef struct mvr_fen_info_st
{
    int channel_num;
    TMsFenInfo *fen_info;
} TNvrFenInfo;

extern TNvrFenInfo __nvr_fen_info;
#endif
