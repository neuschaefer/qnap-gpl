...
#ifdef _NVR_
#define FTP_REMOTE_ADDRESS remote_addr
#endif
...
#ifdef _QNAP_
#ifdef STATISTICS
void add_ftp_user(char *username, char *addr)
{
  USER_INFO user;
  int ret;
  ...
  strcpy(user.ip, addr);
  strcpy(user.sharename, "---");
  user.app_id=shm_get_app_id("proftpd");
  user.schedule=SCHEDULE_PID;
  user.pid=getpid();
  ret = shm_add_user(&user);
  pr_log_debug(DEBUG0, "shm_add_user(%s, %s) ret %d", user.username, user.ip, ret);
}
#endif
#endif
...
#ifdef _NVR_
pr_log_debug(DEBUG0, "NVR::%s::(%d, %d)", __FUNCTION__, session.is_event_report_session, logged_in);
    if( session.is_event_report_session && logged_in)
        return PR_HANDLED(cmd);
#endif
...
#ifdef _QNAP_
static struct passwd *passwd_dup(pool *p, struct passwd *pw, int allow_symlinks) {
  struct passwd *npw;
  char _home_dir[64];
  struct stat _st;
#else
...
#ifdef _QNAP_
  set_homedir(pw->pw_dir);
  set_fullhomedir(dir_best_path(p, pw->pw_dir));
  pr_log_auth(PR_LOG_NOTICE, "pw_dir [%s]", pw->pw_dir);
  if (g_home) {
    pr_log_auth(PR_LOG_NOTICE, "g_home [%s]", g_home);
	seteuid(0);
	setegid(0);
	sprintf(_home_dir, "%s/%s", USER_HOME_PREFIX, pw->pw_name);	// default home = USER_HOME_PREFIX/%s
	if (stat(_home_dir, &_st) != 0)	{		// if home directory is not exist, then create it
		mkdir(_home_dir, 0777);
		chown(_home_dir, pw->pw_uid, pw->pw_gid);
	}
	npw->pw_dir = pstrdup(p,_home_dir);
	setegid(session.gid);
	seteuid(session.uid);
  }
  else {
	get_default_root(p, allow_symlinks, &npw->pw_dir);
  }
  pr_log_auth(PR_LOG_NOTICE, "New pw_dir [%s]", npw->pw_dir);
#else
...
static int setup_env(pool *p, cmd_rec *cmd, char *user, char *pass) {
  ...
  pw = passwd_dup(p, pw, allow_chroot_symlinks);
#else
  pw = passwd_dup(p, pw);
#endif
...
#ifdef _QNAP_
    if (l_home){
	pr_log_auth(PR_LOG_NOTICE, "Create USER %s 's Home", user);
	NAS_Create_User_Home_By_Name(user);
    }
#endif
...
#ifdef _QNAP_
/* Marked by Ricky */
/* solve administrator shell->kentbash  */
/* End */
#else
...
#ifdef _QNAP_
//  else
// 2004.11.03, Johnson Cheng
// Check if the user belongs AD domain or NT domain, and keep it in session.user_type
// that will be used in _check_limit_allow().
  {
    char workgroup[128];
    char security[20];
    if ( (Get_Private_Profile_String("global", "workgroup", "", workgroup, sizeof(workgroup), SMB_CONFIG_FILE) >= 0) &&
        (Get_Private_Profile_String("global", "security", "", security, sizeof(security), SMB_CONFIG_FILE) >= 0)){
      pr_log_debug(DEBUG1, "Get_Private_Profile_String (security: %s, workgroup: %s)", security, workgroup);
      if ( !strcasecmp(security, "ADS") ){
        if ( !strncasecmp(session.user, workgroup, strlen(workgroup)) )
          session.user_type = 2;
      }
      else if ( !strcmp(security, "DOMAIN") ){
        if ( !strncasecmp(session.user, workgroup, strlen(workgroup)) )
          session.user_type = 1;
      }
      else    // security = USER
        session.user_type = 0;
      pr_log_debug(DEBUG1, "user: %s, type: %d", session.user, session.user_type);
    }
    else{
      pr_log_debug(DEBUG0, "Get_Private_Profile_String failed !!");
                session.user_type = 0;
    }
  }
...
{
    int ret = 0;
    pr_log_debug(DEBUG0, "SendConnToLogEngine(%s, %s, CONN_ACTION_LOGIN_SUCC) return %d", session.user, (char *)session.c->remote_name, ret);
    ret = SendConnToLogEngine(EVENT_TYPE_INFO, session.user, (char *)session.c->remote_name, "", CONN_SERV_FTP, CONN_ACTION_LOGIN_SUCC, "---");
}
#endif
  return 1;
...
#ifdef _QNAP_
  {
  int ret = SendConnToLogEngine(EVENT_TYPE_WARN, user, (char *)pr_netaddr_get_ipstr(session.c->remote_addr), "", CONN_SERV_FTP, CONN_ACTION_LOGIN_FAIL, "---");

    pr_log_debug(DEBUG0, "SendConnToLogEngine(%s, %s, CONN_ACTION_LOGIN_FAIL) return %d", session.user, (char *)session.c->remote_name, ret);
  pr_log_auth(PR_LOG_NOTICE, "USER %s: Failure login from [%s] to %s(%d).", user,
      pr_netaddr_get_ipstr(session.c->remote_addr),
      pr_netaddr_get_ipstr(session.c->local_addr), session.c->local_port);
  }
#endif
...
#ifdef _NVR_
  int nvr_user_rpt_type = 0;
//pr_log_debug(DEBUG0, "do auth_user\n" );
#endif
...
#ifdef _NVR_
    // check if "event report account"
#ifndef NVR_EVENT_REPORT_USER_ACCOUNT_PREFIX_NO_DASH
#define NVR_EVENT_REPORT_USER_ACCOUNT_PREFIX_NO_DASH "nvrevtrpt"
#endif

    if(!strncmp(cmd->arg, NVR_EVENT_REPORT_USER_ACCOUNT_PREFIX, strlen(NVR_EVENT_REPORT_USER_ACCOUNT_PREFIX)))
      nvr_user_rpt_type = 1;
    else if(!strncmp(cmd->arg, NVR_EVENT_REPORT_USER_ACCOUNT_PREFIX_NO_DASH, strlen(NVR_EVENT_REPORT_USER_ACCOUNT_PREFIX_NO_DASH)))
      nvr_user_rpt_type = 2;

    if (nvr_user_rpt_type > 0)
    {
	// check ip address legal or not 
	// if inet_ntoa(*(session.c->FTP_REMOTE_ADDRESS)) is the same as in nvrd.xml
        int channel_id=-1;
	if (nvr_user_rpt_type == 1)
            sscanf( &cmd->arg[strlen(NVR_EVENT_REPORT_USER_ACCOUNT_PREFIX)], "%04d", &channel_id);
	else
	    sscanf( &cmd->arg[strlen(NVR_EVENT_REPORT_USER_ACCOUNT_PREFIX_NO_DASH)], "%04d", &channel_id);  // Hugo: for toshiba, ftp user alphabeta limitation
	if( __nvr_fen_info.fen_info && channel_id>=0 && channel_id<__nvr_fen_info.channel_num && !strcmp(__nvr_fen_info.fen_info[channel_id].address, pr_netaddr_get_ipstr(session.c->FTP_REMOTE_ADDRESS)))
	{
	    session.is_event_report_session = 1;
	    ...
	    session.user = NULL;
	    session.group = NULL;
pr_log_debug(DEBUG0, "NVR::is_event_report_session!\n" );
	    return HANDLED(cmd);
	}
	else
	{
	    char msg[128];
	    //extern int Write_Log( char *msg,int);
	    sprintf( msg, "Event Notificaton Account Error(%d,%s).", channel_id, pr_netaddr_get_ipstr(session.c->FTP_REMOTE_ADDRESS) );
	    pr_response_send(R_550, msg);
pr_log_debug(DEBUG0, "NVR::Event Notificaton Account Error!\n" );
	    //sprintf( msg, "Abnormal event - invalid FTP event notification account name(%d,%s).", channel_id, pr_netaddr_get_ipstr(session.c->FTP_REMOTE_ADDRESS) );
	    //Write_Log( msg,1);
	    return HANDLED(cmd);
	}
    } 
/* Hugo. NAS need not block anonymous user
    else if(!strcmp(cmd->arg, "anonymous"))
    {
	    char msg[128];
	    sprintf( msg, "Anonymous is not allowed." );
	    pr_response_send(R_550, msg);
	    return HANDLED(cmd);
    }
*/
...
char    buf[256] = {0};
  ...
  if (!user) {
	...
	{
    	    config_rec *c = NULL;
	    c = add_config_param_set(&cmd->server->conf, "authenticated", 1, (void *) TRUE);
    	    c->argv[0] = pcalloc(c->pool, sizeof(unsigned char));
            *((unsigned char *) c->argv[0]) = TRUE;
    	    set_auth_check(NULL);
		...
		pr_log_debug(DEBUG0, "NVR::login succ!");
	    return HANDLED(cmd);
	}
    }
#endif
