...
/* add by Kent */
/* check pwd from NAS API */
#ifdef _QNAP_
//#ifndef HAVE_PAM
        Get_Exact_NAS_User_Name(cmd->argv[0]);
        PRIVS_ROOT 
        passwd=getpwnam(cmd->argv[0]);
        PRIVS_RELINQUISH 
        if (passwd!=NULL)
        {
                setegid(0);
        ...
        pr_log_debug(0, "NAS user=%s", cmd->argv[0]);
        strcpy(buf, cmd->argv[0]);

        Get_Exact_NAS_User_Name(buf);
#ifdef APPLICATION_PRIVILEGE
	if (Check_NAS_User_Password(buf, cmd->argv[1])==0 && naspriv_check_user_app_by_name(buf, NASPRIV_FTP_STR, 1) == 0)
#else
        if (Check_NAS_User_Password(buf, cmd->argv[1])==0)
#endif
        {
                seteuid(passwd->pw_uid);
                setegid(passwd->pw_gid);
#ifdef APPLICATION_PRIVILEGE
		pr_log_debug(0, "NAS chkpwd and permission success !!");
#else
                pr_log_debug(0, "NAS chkpwd success !!");
#endif
                return HANDLED(cmd);
        }
        else
        {
                seteuid(passwd->pw_uid);
                setegid(passwd->pw_gid);
                pr_log_debug(0, "NAS chkpwd fail !!");
...
#ifdef _QNAP_
  if((passwd = getpwnam(cmd->argv[1])) != NULL)
    if (passwd->pw_passwd[0]==0x0 && pw[0]!=0x0)
      return ERROR(cmd);
#endif /* _QNAP_ */
#endif /* PR_USE_SIA */
