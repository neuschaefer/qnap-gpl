...
extern int q_shmid;;
#define USB_PREFIX_SIZE_SHORT	sizeof("/external/") - 1
#define USB_PREFIX_SHORT	    "/external/"

extern unsigned char is_master;

char *get_log_name(char *path) 
{
    /* Richard Chen 20090402, Fix Bug 8976.
     * Sice our convention is /_DATA/, we offset strlen("_DATA/"). */
    char *p = strstr(path, "_DATA/");
    if(p)
        return p + 6;
    if(strncmp(path, USB_PREFIX_SHORT, USB_PREFIX_SIZE_SHORT))
        return (path[0] == '/') ? path + 1 : path;
    return path + USB_PREFIX_SIZE_SHORT;
}
#endif
...
#ifdef _QNAP_
// Hugo add
MODRET set_enableuserwanip(cmd_rec *cmd) {
  int enable_userwanip;
  config_rec *c = NULL;
  ...
  c = add_config_param(cmd->argv[0], 1, NULL);
  c->argv[0] = pcalloc(c->pool, sizeof(int));
  *((int *) c->argv[0]) = enable_userwanip;

  return PR_HANDLED(cmd);
}
// End
MODRET set_wanip(cmd_rec *cmd) {
  config_rec *c = NULL;
...
MODRET set_passiveports(cmd_rec *cmd) {
  ...
  if (pasv_max_port < pasv_min_port)
    CONF_ERROR(cmd, "min port must be less than or equal to max port");
#else
  if (pasv_max_port <= pasv_min_port)
...
#ifdef _QNAP_
    // Jacky20140903 - resolve the real dir later
    flags |= CF_DEFER;
#else
...
#ifdef _QNAP_
// Ricky
// to avoid root folder cause fail
...
_pasv_retry_:
  ...
  if (pr_inet_listen(session.pool, session.d, 1, 0) < 0) {
    int xerrno = errno;
#ifdef _QNAP_
    if(xerrno == EADDRINUSE && retry-- > 0) {
        if (session.d) {
            pr_log_debug(DEBUG1, "listen %u failed, retry#%d", session.d->local_port, retry);
            pr_inet_close(session.d->pool, session.d);
            session.d = NULL;
        }
        goto _pasv_retry_;
...
#ifdef _QNAP_
// Hugo add
  if ((c = find_config(main_server->conf, CONF_PARAM, "EnableUserWanIp", FALSE)) != NULL) {
    char *q_shmaddr;
    char swanip[128];
    swanip[0] = 0;
#if 1
	if ( *((int *)c->argv[0]) ) { // use use wan ip instead of lan ip in passive mode
      // we need check wanip changed every time
		if ((c = find_config(main_server->conf, CONF_PARAM, "WanIp", FALSE)) != NULL){
			char *display = c->argv[0];
			if(display && strlen(display) > 1)
				strcpy(swanip,display);
		}
		
		q_shmaddr = (char *)shmat(q_shmid, NULL, SHM_RDONLY);
		if ((int)q_shmaddr != -1) {
			if (swanip[0])
				strcpy(addrstr, swanip);
			else{
				if (q_shmaddr[0])
					strcpy(addrstr, q_shmaddr);
			}
			shmdt(q_shmaddr);
		}
	}
...
#ifdef _QNAP_
static int rmdir_1(const char *dir)
{
    DIR *d = opendir(dir);
    if(!d)
        return 0;
    ...
    while((e = readdir(d))) {
        if(strcmp(".", e->d_name) == 0 || strcmp("..", e->d_name) == 0)
            continue;
        snprintf(filename, sizeof(filename), "%s/%s", dir, e->d_name);
        if(e->d_type == DT_DIR)  {
            count += rmdir_1(filename);
            if(rmdir(filename) == 0)
                count++;
        }
        else {
...
static int rmdir_r(const char *dir)
{
    int retry = 3;
    while(retry-- > 0) {
        int ret = rmdir_1(dir);
...
MODRET core_rmd(cmd_rec *cmd) {
...
#ifdef _QNAP_
  // Ricky
  /* don't rmdir in the /share folder */
  if (!custom_root && Is_NAS_Default_Root(dir+1)) {
...
#ifdef _QNAP_
    if(xerrno == ENOTEMPTY) {
        int clen = strlen(session.vwd);
        char cpath[PATH_MAX+1];
        if(clen > 0 && strncmp(session.vwd, dir, clen)==0)
            snprintf(cpath, PATH_MAX, "%s%s", session.cwd, dir+clen);
        else
            strncpy(cpath, dir, PATH_MAX);
        cpath[PATH_MAX] = 0;
        clen = strlen(cpath);
        DIR *d = opendir(cpath);
        if(d) {
            ...
            while((e = readdir(d))) {
                if(strcmp(e->d_name, ".") == 0 || strcmp(e->d_name, "..") == 0)
                    continue;
                snprintf(cpath+clen, PATH_MAX-clen, "/%s", e->d_name);
                if(!dir_hide_file(cpath))
                    break;
            }
            closedir(d);
            if(!e) {
  ...
  pr_response_add(R_250, _("%s command successful"), cmd->argv[0]);
    ...
    if(custom_root) {
        ...
        snprintf(_fname, PATH_MAX, "%s%s%s", custom_root_path+7, (dir[0] == '/') ? "" : "/", dir);
        ret = SendConnToLogEngineEx(EVENT_TYPE_INFO, session.user,
                                    (char *)session.c->remote_name, "",
                                    CONN_SERV_FTP, CONN_ACTION_DEL, "%s/",
                                    _fname);
    }
    else {
        ret = SendConnToLogEngineEx(EVENT_TYPE_INFO, session.user,
    ...
    pr_log_debug(DEBUG2, "ret = [%d], Rmdir %s/ from %s:[%s]", ret, dir, session.user, session.c->remote_name);
    }
#endif
  return PR_HANDLED(cmd);
}
...
#ifdef _QNAP_
// Ricky
/* don't mkdir in the /share folder */
  if (!custom_root && Is_NAS_Default_Root(dir+1)) {
    pr_response_add_err(R_550,"%s: Forbidden filename",cmd->arg);
    return ERROR(cmd);
  }
...
/* end */
...
#ifdef _QNAP_
  {
    ...
    if(custom_root) {
        ...
        ret = SendConnToLogEngineEx(EVENT_TYPE_INFO, session.user,
                                    (char *)session.c->remote_name, "",
                                    CONN_SERV_FTP, CONN_ACTION_MKDIR, "%s/",
                                    _fname);
    }
    else {
        ret = SendConnToLogEngineEx(EVENT_TYPE_INFO, session.user,
    ...
    pr_log_debug(DEBUG2, "ret = [%d], Mkdir %s from %s:[%s]", ret, dir, session.user, session.c->remote_name);
  }
#endif
...
//Ricky
#ifdef _QNAP_
int Is_NAS_Default_Root(char *path)
{
        char *ptr;

	if (g_home)	// enable home directory, always return 0
		return 0;
        ptr=path;
        while (*ptr!=0x0)
        {
...
#ifdef _QNAP_
/* check if the rename file is under defaultroot folder */
/* if yes, deny it                                      */
/* dest delete file = "path"                            */
  ...
  if (pr_fsio_access_ex(path) < 0 || pr_fsio_access_acl(path) < 0 || pr_fsio_unlink_ex(fullpath) < 0) 
#else
...
#ifdef _QNAP_
  {
    ...
    pr_log_debug(DEBUG2, "ret = [%d], Delete %s from %s:[%s]", ret, fullpath, session.user, session.c->remote_name);
  }
#endif  
  pr_response_add(R_250, _("%s command successful"), cmd->argv[0]);
  return PR_HANDLED(cmd);
...
/* end */
...
#ifdef _QNAP_
  {
    ...
    if(custom_root) {
        char _from[PATH_MAX];
        char _to[PATH_MAX];
        snprintf(_from, PATH_MAX, "%s%s%s", custom_root_path+7, (session.xfer.path[0] == '/') ? "" : "/", session.xfer.path);
        snprintf(_to, PATH_MAX, "%s%s%s", custom_root_path+7, (path[0] == '/') ? "" : "/", path);
        ret = SendConnToLogEngineEx(EVENT_TYPE_INFO, session.user,
                                    (char *)session.c->remote_name, "",
                                    CONN_SERV_FTP, CONN_ACTION_RENAME, "%s to %s",
                                    _from, _to);
    }
    else {
        ret = SendConnToLogEngineEx(EVENT_TYPE_INFO, session.user, 
                                    (char *)session.c->remote_name, "", 
                                    CONN_SERV_FTP, CONN_ACTION_RENAME, 
                                    "%s to %s", 
                                    get_log_name(session.xfer.path), get_log_name(path));
    }
    pr_log_debug(DEBUG2, "ret = [%d], Rename %s to %s from %s:[%s]", ret, session.xfer.path, path, session.user, session.c->remote_name);
  }
#endif
...
{
char *test = NULL;
test= get_param_ptr(TOPLEVEL_CONF, "UseUTF8", FALSE);
if(test !=NULL){
        if(*test ==FALSE){
		pr_feat_remove("UTF8");
                pr_log_debug(DEBUG1,"Not support UTF8\n");
	}
        else{
		pr_feat_add("UTF8");
                pr_log_debug(DEBUG1,"Support UTF8\n");
	}
}
}
#endif
...
#ifdef _QNAP_
  { "EnableUserWanIp",	set_enableuserwanip,	NULL },
  { "WanIp",	set_wanip,	NULL },
#endif
  { "From",			add_from,			NULL },
