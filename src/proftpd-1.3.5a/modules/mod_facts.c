...
#if 0 // QNAP - patched from proftpd git
...
MODRET facts_mlsd(cmd_rec *cmd) {
  ...
  while ((dent = pr_fsio_readdir(dirh)) != NULL) {
    ...
    if(!sp)
        sp = make_sub_pool(cmd->tmp_pool);

    rel_path = pdircat(sp, best_path, dent->d_name, NULL);
    res = dir_check(sp, cmd, cmd->group, rel_path, &hidden);
    if (!res || hidden) {
      continue;
    }

    /* Check that the file can be listed. */
    abs_path = dir_realpath(sp, rel_path);
    if (abs_path) {
      res = dir_check(sp, cmd, cmd->group, abs_path, &hidden);
      
    } else {
      abs_path = dir_canonical_path(sp, rel_path);
      if (abs_path == NULL) {
        abs_path = rel_path;
      }

      res = dir_check_canon(sp, cmd, cmd->group, abs_path, &hidden);
    }
     ...
     */
    info.path = pr_fs_encode_path(sp, dent->d_name);
