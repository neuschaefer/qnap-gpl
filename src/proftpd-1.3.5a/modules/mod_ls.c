...
#ifdef _QNAP_
extern uint64_t rlimit_memory_soft;
static int max_ls_nr = 0;
#endif
...
#ifdef _QNAP_
	strcpy(_user, MAP_UID(st.st_uid));
	strcpy(_group, MAP_GID(st.st_gid));
	ptr = _user;
	while (*ptr != 0x0) {
		if (*ptr == ' ') *ptr = '_';
		ptr++;
...
#if defined(PR_USE_NLS) && defined(HAVE_STRCOLL) && !defined(_QNAP_)
...
#ifdef _QNAP_
  if(max_ls_nr <= 0 && rlimit_memory_soft > 0) {
      if(rlimit_memory_soft < (1024 * 1024 * 1024))
          max_ls_nr = 50000;
      else if(rlimit_memory_soft < (2 * 1024 * 1024 * 1024ULL))
          max_ls_nr = 100000;
      else
          max_ls_nr = 200000;
      pr_log_debug(DEBUG2, "max LIST # = %d", max_ls_nr);
  }
  if(max_ls_nr > 0)
      dsize = max_ls_nr + 10;
  else
...
#ifdef _QNAP_
    if (i >= max_ls_nr && max_ls_nr > 0) {
      pr_log_debug(DEBUG2, "reach the MAX file #%d", max_ls_nr);
      break;
    }
#endif
  }
