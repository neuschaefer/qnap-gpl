...
extern char *get_log_name(char *path);
//#ifdef TS409
...
#ifdef _NVR_
#ifdef _NVR_USR_ACC_CTL
#include "cusracc.h"
#endif
#include <sys/un.h>
#include "cfg_qsurveillance.h"

#define AEVT_MGR_SRPC_PATH              "/tmp/.aevtsrpc"
#define CMSVR_SRPC_PATH          "/tmp/.cmsvrpc"
#define TERMINATE_SRPC_SESSION  0xfffffff0

static int __srpc_sock = -1;

static int init_socket()
{
    struct sockaddr_un un_name;
	
    __srpc_sock=socket( AF_LOCAL,SOCK_STREAM, 0 );
    un_name.sun_family = AF_LOCAL;
#if 1//def _VIOSTOR_
    if (Is_Advanced_Event_Management_Enabled())
    	strcpy( un_name.sun_path, AEVT_MGR_SRPC_PATH );
    else
    	strcpy( un_name.sun_path, CMSVR_SRPC_PATH );
#else
    strcpy( un_name.sun_path, CMSVR_SRPC_PATH );
#endif
    
    if(connect( __srpc_sock, (struct sockaddr *)&un_name, sizeof(un_name)))
    {   
#if 0
char cmd[128];
sprintf(cmd, "touch /tmp/ftp_srpc_err_%d", errno ); 
system(cmd);
#endif
        close(__srpc_sock);
        __srpc_sock=-1;
...
static int SendPacket( unsigned char *data, int len )
{    
    int slen=0;
    if( __srpc_sock<0 )return -1;
    while( len>0 )
    {
        slen= write( __srpc_sock, data, len );
...
static int RecvPacket( unsigned char *data, int len )
{    
    int slen=0;   
    if( __srpc_sock<0 )return -1;
    while( len>0 )
    {
        slen = read( __srpc_sock, data, len );
        if( slen<=0 )
        {   
	    close(__srpc_sock);
            return -1;
...
static void srpc_client_SendCommand(int com)
{
	SendPacket( (unsigned char *)&com, sizeof(int) );
}

static void srpc_client_StopConnect()
{    
	int num = TERMINATE_SRPC_SESSION;
	SendPacket( (unsigned char *)&num, sizeof(int) );
}

static void srpc_client_SendInt(int data)
{
	SendPacket( (unsigned char *)&data, sizeof(int) );
}

static void srpc_client_SendString( char *str )
{    
	int len=str?strlen(str):0; 
	//DEBUGMSG("strlen = %d\n",len);
	srpc_client_SendInt(len);
	SendPacket( (unsigned char *)str, len );	
}

static void srpc_client_SendData( unsigned char *data, int len )
{    
	//DEBUGMSG("data size=%d\n",len);
	if( len==0 )	return;
	SendPacket( data, len );
}

static void srpc_client_RecvInt(int *data)
{
	RecvPacket( (unsigned char *)data, sizeof(int) );
}

#define CANONICAL_COMMAND_FTP_NOTIFY                    0x10004

int notify_event( int channel_id, char *ftp_file_path )
{
    int com = CANONICAL_COMMAND_FTP_NOTIFY;
    int ret = -1;  
//log_debug(DEBUG0, "notify event..\n");
    seteuid(0);
//log_debug(DEBUG0, "end seteuid 0..\n");
    if( !strncmp( ftp_file_path+13, "MOSI", 4 ))
    {
	// this is used for test
	FILE *file=fopen("/var/ftp_event_test_ok", "w");
	if(!file)
	    return -1;
	fclose(file);
	return 0;
    ...
    if( init_socket() == -1)
    {
//pr_log_debug(DEBUG0, "init sock failed\n");
#if 0
	// log event here!!!
	extern int Write_Log( char *msg,int);
	char tpstr[64],msg[256];
	int type;
	strncpy( tpstr, ftp_file_path, 8 );
	tpstr[5+8]='\0';
	sscanf( &tpstr[5], "%08X", &type );
	if( type==1 )strcpy( tpstr, "motion detected");
	else if( type==2 )strcpy( tpstr, "alarm 1 (or sensor)");
	else if( type==3 )strcpy( tpstr, "alarm 2");
	else strcpy( tpstr, "");
	sprintf( msg, "Failed to handle %s event from Camera %d.", tpstr, channel_id+1);
	Write_Log(msg,1);
#endif
	return -1;
    }
    ...
    //send com to server
//system("touch /tmp/not2");	
    srpc_client_SendCommand(com); 
	
    //send log level
//system("touch /tmp/not3");	
    srpc_client_SendInt(channel_id);
    srpc_client_SendString( ftp_file_path );
	
//system("touch /tmp/not4");	
    srpc_client_RecvInt(&ret);
	
//system("touch /tmp/not5");	
    srpc_client_StopConnect();
//system("touch /tmp/not6");	
...
#ifdef _NVR_
// Hugo add. 2007.03.09
// check if it is nvr defined path
// the format is NVR_EVENT_REPORT_EVENT_PREFIX in NvrApp/include/nvr_glb.h
int is_nvr_event_path(char *path)
{
    char tmp[256];
...
MODRET xfer_stor(cmd_rec *cmd) {
  ...
  {
    int ret = 0;
      session.xfer.filename = path;
//    session.xfer.filename = dir_best_path(cmd->tmp_pool, cmd->arg);
//    session.xfer.filename = dir_abs_path(session.xfer.p, session.xfer.path, TRUE);
//    ret = SendEVToLogEngineEx(EVENT_TYPE_INFO, session.user, (char *)session.c->remote_name, "", "Stor %s from %s:[%s]", session.xfer.filename, session.user, session.c->remote_name);
#ifdef _NVR_
  if( !session.is_event_report_session )
  {
    ...
    if(custom_root) {
        snprintf(_fname, PATH_MAX, "%s%s%s", custom_root_path+7, (path[0] == '/') ? "" : "/", path);
        lname = _fname;
    }
    else 
        lname = get_log_name(path);
    ret = SendConnToLogEngine(EVENT_TYPE_INFO, session.user, (char *)session.c->remote_name, "", CONN_SERV_FTP, CONN_ACTION_WRITE, lname);
    pr_log_debug(DEBUG2, "ret = [%d], Stor %s from %s:[%s]", ret, session.xfer.filename, session.user, session.c->remote_name);
#ifdef _NVR_
  }
#endif
  }
...
#ifdef _NVR_
  /* added by Jeff on 2006/10/24 for IPcam event notification */
  if( session.is_event_report_session )
  {
//log_debug(DEBUG0, "store, is_event_report_session..\n");
    // notify msend
    // Hugo add.
    if( is_nvr_event_path(cmd->arg))
      notify_event( session.channel_id, cmd->arg);//panasonic
    else
      notify_event( session.channel_id, session.cwd_path);//axis, sanyo
#if 0 // for test
{
    char command[256];
    sprintf( command, "touch \"/tmp/event_from_ipcam_addr_%08X_%s_ch_%d_%s_%s\"", *(session.c->remote_addr), inet_ntoa(*(session.c->remote_addr)), channel_id, usr, cmd->arg);
    //sprintf( command, "touch \"/tmp/event_from_ipcam_addr_%08X_%s_ch_%d_%s\"", *(session.c->remote_addr), inet_ntoa(*(session.c->remote_addr)), channel_id, usr);
    //sprintf( command, "touch /tmp/event_from_ipcam_addr_%08X_%s_ch_%d", *(session.c->remote_addr), inet_ntoa(*(session.c->remote_addr)), channel_id);
    system(command);
}
#endif
    // end needed data transmission
...
#ifdef _QNAP_
  if (!custom_root && Is_NAS_Default_Root(path+1))
  {
...
{
    int ret;
    session.xfer.filename = session.xfer.path;

    char *lname = session.xfer.filename;
    char _fname[PATH_MAX];
    if(custom_root) {
        snprintf(_fname, PATH_MAX, "%s%s%s", custom_root_path+7, (lname[0] == '/') ? "" : "/", lname);
        lname = _fname;
    }
    else 
        lname = get_log_name(lname);
    ret = SendConnToLogEngine(EVENT_TYPE_INFO, session.user, (char *)session.c->remote_name, "", CONN_SERV_FTP, CONN_ACTION_READ, lname);
}
#endif
