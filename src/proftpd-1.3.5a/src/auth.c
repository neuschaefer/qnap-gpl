...
#ifdef _QNAP_
  char buf[80] = {0};
  if(!name || strlen(name) >= sizeof(buf))
      return NULL;
  strncpy(buf, name, sizeof(buf)-1);
  Get_Exact_NAS_User_Name(buf);
  pr_log_debug(0, "NAS buf=%s name=%s", buf, name);
  cmd=make_cmd(p,1,buf);
#else
...
const char *pr_auth_gid2name(pool *p, gid_t gid) {
  ...
  if (MODRET_ISHANDLED(mr) &&
    ...
    snprintf(namebuf, sizeof(namebuf), "%u", gid);
    res = namebuf;
  }
#endif
...
#ifdef _QNAP_
	// set supplementary group IDs before chroot
	seteuid(0);
	setegid(0);
	NAS_Set_Groups(session.user);
	setegid(session.gid);
	seteuid(session.uid);
#endif
...
#ifdef _QNAP_
    #ifdef STATISTICS
      add_ftp_user(session.user, (char *)pr_netaddr_get_ipstr(session.c->remote_addr));
    #endif /* STATISTICS */
#endif /* _QNAP_ */
