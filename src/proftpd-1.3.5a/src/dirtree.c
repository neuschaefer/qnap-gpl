...
#ifdef _QNAP_
void trace_node(config_rec *c)
{
config_rec *tmp = c;
while (tmp) {

    if(tmp->name)
       pr_log_debug(DEBUG0,"%s: [%s]", __FUNCTION__, tmp->name);
    else
       pr_log_debug(DEBUG0, "%s: [null]", __FUNCTION__);
    tmp=tmp->next;
  }
...
#else
  if (!strstr(*path, "%u") && !strstr(*path, "%H"))
#endif
    return *path;
...
#ifdef _QNAP_
  // Jacky 20140911 - fix double '/' issue
  if (substr_path) {
      char *end = strstr(substr_path, "%H");
      if(end) {
          *end = 0;
          new_path = pdircat(path_pool, substr_path, get_homedir(), NULL);
          //pr_log_debug(DEBUG4, "home:%s, sub:%s, new:%s", get_homedir(), substr_path, new_path);
      }
      else
          new_path = substr_path;
  }
...
#ifdef _QNAP_
    /* home path's permission have highest priority
     * (ex: the permission of /homes/$username will overwrite /homes 's)
     */
    if(l_home && strcmp(get_fullhomedir(), c->name) == 0)
	return res;
#endif
  }
