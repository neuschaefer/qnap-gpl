...
int pr_expr_eval_class_and(char **expr) {
  ...
  for (; *expr; expr++) {
    ...
    if (session.conn_class != NULL &&
#ifdef _QNAP_
        strcasecmp(session.conn_class->cls_name, class) == 0) {
#else
...
int pr_expr_eval_class_or(char **expr) {
  ...
  for (; *expr; expr++) {
    ...
    if (session.conn_class == NULL)
      return found;
#ifdef _QNAP_
    if (strcasecmp(session.conn_class->cls_name, class) == 0)
#else
...
#ifdef _QNAP_
    if (session.group && strcasecmp(session.group, grp) == 0) {
#else
    if (session.group &&
...
int pr_expr_eval_group_or(char **expr) {
  ...
  for (; *expr; expr++) {
    ...
    if (session.group &&
#ifdef _QNAP_
        strcasecmp(session.group, grp) == 0) {
#else
        strcmp(session.group, grp) == 0) {
#endif
      found = !found;
...
int pr_expr_eval_user_and(char **expr) {
  ...
  for (; *expr; expr++) {
    ...
    if (session.user &&
#ifdef _QNAP_
        strcasecmp(session.user, user) == 0)
#else
