...
static struct dirent home_dirent;
#endif
...
#ifdef _QNAP_
	pr_log_pri(PR_LOG_DEBUG, "fsio_chroot_path=%p", fsio_chroot_path);
  if (is_master || fsio_chroot_path == NULL) {
	pr_log_pri(PR_LOG_DEBUG, "Call chroot(%s)", path);
#endif
  if (chroot(path) < 0)
    return -1;
#ifdef _QNAP_
	fsio_chroot_path = path;
  	}
#endif
...
#ifdef _QNAP_
struct dirent* get_home_link(pr_fs_t *fs, void *dir){
  struct dirent *dent, *result = NULL;
  void * dirh;
  char user_path[256], old_path[256], username[256];
  char separator, *ptr;
  int ret = 0, ad_user = 0;
  if(strcmp(cwd, "/"))
    return NULL;
  sprintf(old_path, "%s", cwd);
  sprintf(user_path, "%s", get_homedir());
  if(ptr = strrchr(user_path, '/')){
    *ptr = '\0';
  }
  ret = pr_fsio_chdir(user_path, 0);

  dirh = pr_fsio_opendir(".");

  make_home_link = 1;
  if((ptr = strchr(session.user, '+')))
    separator = '+';
  else
    separator = '\\';

  //check user is AD or local
  if((ptr = strchr(session.user, separator))){
    ad_user = 1;
    ptr++;
    sprintf(username, "%s", ptr);
  }else{
    ad_user = 0;
    sprintf(username, "%s", session.user);
  }
  ...
  if(dent){
    result = &home_dirent;
    memcpy(result, dent, sizeof(struct dirent));
  }
  pr_fsio_closedir(dirh);

  if(result == NULL){
    pr_log_pri(PR_LOG_INFO, "make home link fail");

  }else{
    sprintf(result->d_name, "%s", USER_HOME_SHARE_NAME);
    pr_log_pri(PR_LOG_INFO, "make home link %s",result->d_name );
  }
  ret = pr_fsio_chdir(old_path, 0);
  //pr_log_pri(PR_LOG_INFO, "%s %d old=%s cwd=%s",__func__, __LINE__,old_path, cwd );
  return result;
}
#endif
...
#ifdef _QNAP_
  if(res == NULL && make_home_link == 0 && l_home == 1){
    make_home_link = 1;
    res = get_home_link(fs, dir);
  }
#endif
...
#ifdef _QNAP_
int pr_fsio_stat(const char *opath, struct stat *sbuf) {

  char home[64];
  char *path;
  sprintf(home, "/%s", USER_HOME_SHARE_NAME);
  if(strcmp(cwd, "/") == 0 && l_home == 1 && strcmp(opath, home) == 0){
    sprintf(home, "%s", get_homedir());
    path=home;
  }else{
    path=opath;
  }
...
#ifdef _QNAP_
int pr_fsio_lstat(const char *opath, struct stat *sbuf) {
  ...
  if(strcmp(cwd, "/") == 0 && l_home == 1 && strcmp(opath, USER_HOME_SHARE_NAME) == 0 ){
    sprintf(home, "%s", get_homedir());
    path=home+1;
  }else if(strcmp(cwd, "/") == 0 && l_home == 1 && strcmp(opath, home) == 0 ){
    sprintf(home, "%s", get_homedir());
    path=home+1;
  }else{
    path=opath;
...
#ifdef _QNAP_	
int pr_fsio_access_ex(const char *name) 
{
    if(session.uid == 0)
        return 0;
    ...
    uid_t uid = geteuid();
    char *ptr = strrchr(tmp_file + 1, '/');
    while (ptr) {
        struct stat statbuf;
        *ptr = 0;
        int ret = stat(tmp_file, &statbuf);
        *ptr = '/';
        //check if only owner can delete files
        if (ret != 0 || (statbuf.st_mode & S_ISVTX) == 0)
            break;

        ret = stat(tmp_file, &statbuf);
        //check if the user is the file's owner
        if (ret == 0 && statbuf.st_uid != uid) {
            free(tmp_file);
            errno = EACCES;
            return -1;
        }
...
#include "acl.h"
static int convert_permission(acl_entry_t entry)
{
	acl_permset_t permset;
	acl_get_permset(entry, &permset);
    int perm = ((acl_get_perm(permset, ACL_READ)) ? 4 : 0) |
...
#include "xattr.h"
static int get_acl_permission(const char *file)
{
    uid_t o_uid = -1;
    uid_t o_uid_p = -1;
    ...
    if(stat(file, &st))
        return 0;
    o_uid = st.st_uid;
    char *ftmp = strdup(file);
    if(ftmp) {
        char *dname = dirname(ftmp);
        if(dname) {
            if(stat(dname, &st) == 0)
                o_uid_p = st.st_uid;
        }
    ...
    if(!acl) {
        pr_log_debug(DEBUG1, "get %s ACL failed: %s\n", file, strerror(errno));
        return 0;
    }

    uid_t uid = session.uid;
    int uperm = -1;
    int operm = -1;
    int gperm = -1;

    int ngr = (session.gids) ? session.gids->nelts : 0;
    gid_t *groups = (session.gids) ? session.gids->elts : NULL;

	acl_entry_t ent;
	if(acl_get_entry(acl, ACL_FIRST_ENTRY, &ent) <= 0) {
        acl_free(acl);
        return 0;
    }
    ...
    if(o_uid == uid && o_uid_p == uid)
        return operm;
	return (uperm > gperm) ? uperm : gperm;
}

int pr_fsio_access_acl(const char *file) 
{
    if(!file) {
        errno = ENOENT;
        return -1;
    ...
    if(getxattr(file, "system.posix_acl_access", NULL, 0) < 0 && errno == EOPNOTSUPP)
        return 0;

    int g_access = get_acl_permission(file);
    pr_log_debug(DEBUG5, "%s access = %d\n", file, g_access);
    if(!(g_access & 2)) {
        errno = EACCES;
        return -1;
    }
...
int pr_fsio_unlink_ex(const char *name) {
  int res = -1;

  if (!is_master)
      res = pipe_cmd("unlink", name);
...
#ifdef _QNAP_
// Jacky20140904 - fix read fd close too early issue, and add retry
pr_fh_t *pr_fsio_open_by_pipe(const char *name, int flags) {
  pool *tmp_pool = NULL;
  pr_fh_t *fh = NULL;
  pr_fs_t *fs = NULL;
  int w_pipe = -1;
  ...
  pr_trace_msg(trace_channel, 8, "using %s open() for path '%s'", fs->fs_name, name);

  fh->fh_fd = QOpenFIFO(session.pid, CONF_P2C, O_RDONLY|O_NONBLOCK);
  if(fh->fh_fd < 0) {
    destroy_pool(fh->fh_pool);
    return NULL;
  }
  w_pipe = QOpenFIFO(session.pid, CONF_C2P, O_RDWR);
  if(w_pipe >= 0) {
      write(w_pipe, name, strlen(name)+1);
      ...
      usleep(2000);
      {
          ...
          char res[4] = {0};
          do {
              int ret = read(fh->fh_fd, res+total, 4-total);
              if(ret < 0)
                  break;
              if(ret > 0)
                  total += ret;
              usleep(5000);
          }while(total < 4 && ++retry < 200);

          if(total != 4 || strncmp(res, "SUCC", 4) != 0) {
              destroy_pool(fh->fh_pool);
              close(w_pipe);
              pr_log_pri(PR_LOG_DEBUG, "%s:%d:open failed", __FUNCTION__, __LINE__);
              return NULL;
          }
          pr_log_pri(PR_LOG_DEBUG, "%s:%d:open successfully", __FUNCTION__, __LINE__);
      }
      close(w_pipe);
	  if (fcntl(fh->fh_fd, F_SETFD, FD_CLOEXEC) < 0) {
	    if (errno != EBADF) {
      ...
      close(fh->fh_fd);
      fh->fh_fd = -1;
      pr_log_pri(PR_LOG_DEBUG, "%s:%d:open write pipe falied", __FUNCTION__, __LINE__);
      return NULL;
  }
...
int pr_fsio_close(pr_fh_t *fh) {
  ...
  pr_trace_msg(trace_channel, 8, "using %s close() for path '%s'", fs->fs_name,
    fh->fh_path);
#ifdef _QNAP_
  // Jacky20140904 - close the read fd here
  if (fh->fh_is_pipe) {
      close(fh->fh_fd);
  }
  else
