...
#include "privs.h"
#ifdef _QNAP_
static char __homedir[PR_TUNABLE_PATH_MAX + 1] = "/";
static char __fullhomedir[PR_TUNABLE_PATH_MAX + 1] = "/";
#endif
...
void set_homedir(const char *dir) {
  sstrncpy(__homedir, dir+6, sizeof(__homedir));
  __homedir[sizeof(__homedir) - 1] = '\0';
}
...
void set_fullhomedir(const char *dir) {
  if(dir){
    sstrncpy(__fullhomedir, dir, sizeof(__fullhomedir));
    __fullhomedir[sizeof(__fullhomedir) - 1] = '\0';
  }
}

const char *get_fullhomedir(void) {
  return __fullhomedir;
}
#endif
