...
int pr_parser_parse_file(pool *p, const char *path, config_rec *start,
  ...
  if (!(flags & PR_PARSER_FL_DYNAMIC_CONFIG))
	...
	if (is_master)
		...
		fh = pr_fsio_open_by_pipe(path, O_RDONLY);
#else
  fh = pr_fsio_open(path, O_RDONLY);
#endif
  if (fh == NULL) {
  ...
  while ((cmd = pr_parser_parse_line(tmp_pool)) != NULL) {
	...
	int end_parser = 0;
	if (!is_master) {
		if (//strcasecmp("DefaultRoot", cmd->argv[0]) == 0 ||
		strcasecmp("User", cmd->argv[0]) == 0 ||
		strcasecmp("Group", cmd->argv[0]) == 0 ||
		strcasecmp("TLSEngine", cmd->argv[0]) == 0 ||
		strcasecmp("TLSRequired", cmd->argv[0]) == 0 ||
		strcasecmp("TLSRSACertificateFile", cmd->argv[0]) == 0 ||
		strcasecmp("TLSRSACertificateKeyFile", cmd->argv[0]) == 0 ||
		strcasecmp("TLSCACertificateFile", cmd->argv[0]) == 0 ||
		strcasecmp("TLSOptions", cmd->argv[0]) == 0 ||
		strcasecmp("TLSVerifyClient", cmd->argv[0]) == 0) {
			pr_log_pri(PR_LOG_DEBUG, "Skip %s for child process", cmd->argv[0]);
			goto NEXT_LINE;
		}
		if (strcasecmp("QNAP_PIPE_FD_END", cmd->argv[0]) == 0) {
			end_parser = 1;
			goto NEXT_LINE;
		}
	}
