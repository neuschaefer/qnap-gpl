...
#if defined(Athens_PROJECT)
#define WELCOME_MSG		"NASFTPD NSS 300 Series Smart Storage"
#elif defined(_QNAP_)
#define WELCOME_MSG		"NASFTPD Turbo station " PROFTPD_VERSION_TEXT
#else
#define WELCOME_MSG		"ProFTPD " PROFTPD_VERSION_TEXT
#endif
...
void pr_session_send_banner(server_rec *s, int flags) {
  ...
  if (c == NULL ||
    ...
    if (c &&
      ...
      if (flags & PR_DISPLAY_FL_SEND_NOW) {
        pr_response_send(R_220, WELCOME_MSG
          " Server ready.");

      } else {
        pr_response_add(R_220, WELCOME_MSG
          " Server ready.");
      }
