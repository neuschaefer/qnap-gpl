...
#ifdef _QNAP_
    const char *cwd = pr_fs_getcwd();
    char *np = strchr(path, '/');
    if(strcmp(cwd, "/") == 0 && path[0] != '/' && np != NULL) {
        char tcwd[PR_TUNABLE_PATH_MAX + 1] = {0};
        char tmp[PR_TUNABLE_PATH_MAX + 1] = {0};
        *np = 0;
        snprintf(tcwd, sizeof(tcwd), "/%s", path);
        pr_fs_resolve_path(tcwd, tmp, sizeof(tmp), 0);
        *np = '/';
        if (pr_fs_dircat(work, sizeof(work), tmp, np+1) < 0)
            return NULL;
    }
    else if (pr_fs_dircat(work, sizeof(work), cwd, path) < 0)
#else
    if (pr_fs_dircat(work, sizeof(work), pr_fs_getcwd(), path) < 0)
#endif
      return NULL;
