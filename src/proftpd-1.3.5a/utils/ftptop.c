...
#ifdef _QNAP_
static int ha_kill_sessions_flag = 0; // QNAP JeffHsueh add 20141009
#endif
static int delay = 2;
...
#ifdef _QNAP_
static char ha_vip[32] = "0"; // QNAP JeffHsueh add 20141009
#endif
...
static void process_opts(int argc, char *argv[]) {
  int optc = 0;
#ifdef _QNAP_
  const char *prgopts = "AaDS:d:f:hIiUVk:";
#else
...
#ifdef _QNAP_
// QNAP JeffHsueh add 20141009 start
        case 'k':
            ha_kill_sessions_flag = 1;
            strcpy(ha_vip, optarg);
            break;
// QNAP JeffHsueh add 20141009 end
#endif
...
#ifdef _QNAP_
// QNAP JeffHsueh add 20141009 start
static void ha_kill_sessions(void)
{
    char cmd[256];
    char pid[16];
    char state[8];
    ...
    if(ftp_sessions && ftp_nsessions)
    {
        ...
        for (i = 0; i < ftp_nsessions; i++)
        {
            sscanf(ftp_sessions[i], "%s %s %s %s %s", pid, state, user, client, server);
            if(!strncmp(ha_vip, server, strlen(ha_vip)))
            {
                sprintf(cmd, "/bin/kill %s", pid);
                system(cmd);
                sprintf(cmd, "/bin/echo 'ha_kill_sessions kill pid=%s' >> /var/log/qha.log", pid);
                system(cmd);
            }
        }
    }
...
#ifdef _QNAP_
  fprintf(stdout, "\t-k <vip>\t\tHA kill sessions for vip\n"); // QNAP JeffHsueh add 20141009
#endif
  fprintf(stdout, "\n");
