...
/* xattr storage size for ext4 is 4096k bytes*/
#define MAX_ACL_BITS 5013504
...
extern struct richacl *qnap_richacl_move_deny_to_head(struct richacl *);
extern id_t qnap_name_to_id(char *name, int isgroup);
extern char *richacl_to_text(const struct richacl *, int);
extern char *richacl_to_string(struct richacl*);
extern struct richacl *richacl_from_text(int, const char *, int *,
					 void (*)(const char *, ...));
...
extern void richacl_copy(struct richacl *, const struct richacl *);
extern struct richace *richacl_append_entry(struct richacl_alloc *x);
...
extern int richacl_move_everyone_aces_down(struct richacl_alloc *x);
