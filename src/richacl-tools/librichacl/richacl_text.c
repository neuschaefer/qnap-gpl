...
static void write_mask(struct string_buffer *buffer, uint32_t mask, int fmt)
{
	...
	for (i = 0; i < ARRAY_SIZE(mask_flags) - 3; i++) {
		int found = 0;
...
char *richacl_to_text(const struct richacl *acl, int fmt)
{
	...
	richacl_for_each_entry(ace, acl) {
		counter++;
		buffer_sprintf(buffer, "[%*d]", 3, counter);
...
id_t qnap_name_to_id(char *str, int isgroup)
{
        ...
        if (isgroup) {
                ...
                if (!group) {
                        error("Warning: group `%s' does not exist\n", str);
                        goto fail;
                }
                return group->gr_gid;
        } else {
                ...
                if (!passwd) {
                        error("Warning: User `%s' does not exist\n", str);
                        goto fail;
                }
...
static int identifier_from_text(int format, const char *str, struct richace *ace,
				void (*error)(const char *, ...))
{
	char *c;
	unsigned long l;
    ...
    if(format & RICHACL_TEXT_NUMERIC_IDS) {
	    l = strtoul(str, &c, 0);
	    if (*c == 0) {
		    ace->e_id = l;
		    return 0;
...
struct richacl *richacl_from_text(int format, const char *str, int *pflags,
				  void (*error)(const char *, ...))
{
	...
	while (*str) {
		...
		if (!strcasecmp(type_str, "MASK")) {
			...
			if (ace_flags_from_text(flags_str, ace, error))
				goto fail_einval;
			if (identifier_from_text(format, who_str, ace, error))
				goto fail_einval;
