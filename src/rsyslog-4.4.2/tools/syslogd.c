...
#ifdef QNAP_REVISION
static uchar template_SyslogProtocol23Format[] = "\"<%PRI%>1 %TIMESTAMP:::date-rfc3339% %HOSTNAME% %APP-NAME% %PROCID% %MSGID% %STRUCTURED-DATA%%msg:::drop-last-lf%\n\"";
#else
...
die(int sig)
{
	...
	if (sig) {
#ifdef QNAP_REVISION
		(void) snprintf(buf, sizeof(buf) / sizeof(char)," exiting on signal %d.", sig);
#else
...
#	undef setQPROPstr
	 ...
	 */
#ifdef QNAP_REVISION
	snprintf(bufStartUpMsg, sizeof(bufStartUpMsg)/sizeof(char), " (re)start");
#else
...
doHUP(void)
{
	...
	char buf[512];
#ifdef QNAP_REVISION
	snprintf(buf, sizeof(buf) / sizeof(char)," rsyslogd was HUPed, type '%s'.", glbl.GetHUPisRestart() ? "restart" : "lightweight");
		errno = 0;
#else
