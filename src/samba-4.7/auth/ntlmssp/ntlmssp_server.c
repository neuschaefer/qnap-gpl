...
#define LP_GET_REMOTE_ARCH         "get_remote_arch"

/* QNAPNAS_TIMEMACHINE
 * Bug#85269 (KS-Redmine#20813)
 *
 * Call get_remote_arch() at run-time.
 */
static enum remote_arch_types dlopen_get_remote_arch(void)
{
	void *handle = NULL;
	const char *error = NULL;
	enum remote_arch_types ret = RA_UNKNOWN;
	enum remote_arch_types (*func_ptr_to_get_remote_arch)(void) = NULL;
	...
	/* Search symbol. */
	func_ptr_to_get_remote_arch = dlsym(handle, LP_GET_REMOTE_ARCH);
	error = dlerror();
	if (func_ptr_to_get_remote_arch == NULL) {
	...
	/* Execute symbol. */
	ret = func_ptr_to_get_remote_arch();
		...
		*chal_flags |= NTLMSSP_REQUEST_TARGET;
		if (ntlmssp_state->server.is_standalone) {
			...
			*chal_flags |= NTLMSSP_TARGET_TYPE_DOMAIN;
			 ...
			 * Bug#85269 (KS-Redmine#20813)
			 *
			 * Reply 'Hostname' to TargetName field when MacOS re-send NTLM Negotiate in a second time.
			 */
			if (dlopen_get_remote_arch() == RA_OSX) {
				static bool macos_retry = false;
				if (macos_retry == false) {
					macos_retry = true;
...
#ifdef QNAPNAS
		/* Bug#96541 (KS-Redmine#24472)*/
		if (lpcfg_server_role(gensec_security->settings->lp_ctx) != ROLE_ACTIVE_DIRECTORY_DC) {
			if (session_info->unix_token != NULL
				&& session_info->unix_token->uid == (uid_t)65534) {
				/* BUG#94950 (KS-Redmine#23857)
				 *
				 * uid 65534 is guest account at QNAP NAS
				 */
				ntlmssp_state->new_spnego = false;
				ntlmssp_state->neg_flags &= ~NTLMSSP_NEGOTIATE_SIGN;
				ntlmssp_state->neg_flags &= ~NTLMSSP_NEGOTIATE_SEAL;
