...
#ifdef QNAPNAS
	ret = state->ops->_recvfrom_recv(subreq, &sys_errno, state,
					 &state->buf, &state->src, &state->indev);
#else
...
#ifdef QNAPNAS
ssize_t _tdgram_recvfrom_recv(struct tevent_req *req,
			      int *perrno,
			      TALLOC_CTX *mem_ctx,
			      uint8_t **buf,
			      struct tsocket_address **src,
...
#ifdef QNAPNAS
struct tevent_req *_tdgram_sendto_send(TALLOC_CTX *mem_ctx,
                                      struct tevent_context *ev,
                                      struct tdgram_context *dgram,
				      ...
				      char *indev)
{
	...
	subreq = state->ops->_sendto_send(state, ev, dgram,
					  buf, len, dst, indev);
	if (tevent_req_nomem(subreq, req)) {
		goto post;
	}
