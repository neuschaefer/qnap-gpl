...
#ifdef QNAPNAS
static void find_incoming_dev(struct msghdr *msgh, char **indev)
{
	char ifname[IF_NAMESIZE], *res = NULL;
	struct cmsghdr *cmsg = CMSG_FIRSTHDR(msgh);

	memset(ifname, 0, sizeof(ifname));

	for (; cmsg != NULL; cmsg = CMSG_NXTHDR(msgh, cmsg)) {
		if (cmsg->cmsg_level == SOL_IP && cmsg->cmsg_type == IP_PKTINFO) {
			struct in_pktinfo *i = (struct in_pktinfo *)CMSG_DATA(cmsg);

			if(i->ipi_ifindex)
				res = if_indextoname(i->ipi_ifindex, ifname);
			break;
		}
	}

	if (res != NULL && *ifname != '\0' && strcasecmp(ifname, "lo") != 0) {
		if ((*indev = (char *)malloc(strlen(ifname) + 1)) != NULL)
			strcpy(*indev, ifname);
		else
			*indev = NULL;
	}
...
#ifdef QNAPNAS
	sockopt_ret = setsockopt(bsds->fd, SOL_IP, IP_PKTINFO, &opt, sizeof(opt));

	if (sockopt_ret != 0) {
		DEBUG(1, ("set sockopt IP_PKTINFO fail\n"));
		...
		iov[0].iov_base = state->buf;
		iov[0].iov_len = state->len;
		...
		ret = recvmsg(bsds->fd, &msgh, 0);

		memcpy(&bsda->u.sa, (struct sockaddr *)&src_addr, sizeof(struct sockaddr));
		bsda->sa_socklen = sizeof(struct sockaddr);
	}
#else
	ret = recvfrom(bsds->fd, state->buf, state->len, 0,
		       &bsda->u.sa, &bsda->sa_socklen);
...
#ifdef QNAPNAS
	if (sockopt_ret == 0)
		find_incoming_dev(&msgh, &state->indev);
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
static ssize_t _tdgram_bsd_recvfrom_recv(struct tevent_req *req,
					 int *perrno,
					 TALLOC_CTX *mem_ctx,
					 uint8_t **buf,
					 struct tsocket_address **src,
...
#ifdef QNAPNAS
static struct tevent_req *_tdgram_bsd_sendto_send(TALLOC_CTX *mem_ctx,
						  struct tevent_context *ev,
						  struct tdgram_context *dgram,
						  const uint8_t *buf,
						  size_t len,
...
#ifdef QNAPNAS
static int bind_socket_to(int sockfd, const char *dev)
{
	if (sockfd < 0 || dev == NULL || strcasecmp(dev, "unknown") == 0)
		return -1;

	return setsockopt(sockfd, SOL_SOCKET, SO_BINDTODEVICE, dev, strlen(dev));
}
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
	/* coverity issue 14661 #2 of 2 */
	if ((sockopt_ret = bind_socket_to(bsds->fd, state->indev)) == -1)
		DEBUG(1, ("bind socket fail\n"));
#endif /* QNAPNAS */
	ret = sendto(bsds->fd, state->buf, state->len, 0, sa, sa_socklen);
#ifdef QNAPNAS
	/* coverity issue 14661 #1 of 2 */
	if (sockopt_ret == 0) {
		if ((sockopt_ret = bind_socket_to(bsds->fd, "")) == -1)
			DEBUG(1, ("unbind socket fail\n"));
	}
...
#ifdef QNAPNAS
	._recvfrom_recv         = _tdgram_bsd_recvfrom_recv,
#endif /* QNAPNAS */

	.sendto_send		= tdgram_bsd_sendto_send,
#ifdef QNAPNAS
	._sendto_send           = _tdgram_bsd_sendto_send,
#endif /* QNAPNAS */
