...
#ifdef QNAPNAS
#define LIBSMBCONF_SO_PATH         "/usr/local/samba/lib/libsmbconf.so.0"
#define LP_REMOVE_EVERYONE_SYMBOL  "lp_remove_everyone"
#define SID_STRING_DBG_SYMBOL      "sid_string_dbg"

/* Workaround. Call lp_remove_everyone() at run-time. */
static bool dlopen_lp_remove_everyone(void)
{
	bool ret = false;
	void *handle = NULL;
	const char *error = NULL;
	bool (*func_ptr_to_lp_remove_everyone)(void) = NULL;
	
	/* Open library. */
	handle = dlopen(LIBSMBCONF_SO_PATH, RTLD_LAZY);
	error = dlerror();
	if (handle == NULL) {
		DEBUG(0, ("Failed to dlopen %s: %s\n", 
			  LIBSMBCONF_SO_PATH,
			  error ? error : ""));
		goto exit_without_dlclose;
	}

	/* Search symbol. */
	func_ptr_to_lp_remove_everyone = dlsym(handle, LP_REMOVE_EVERYONE_SYMBOL);
	error = dlerror();
	if (func_ptr_to_lp_remove_everyone == NULL) {
		DEBUG(0, ("Failed to dlsym %s in %s: %s\n", 
			  LP_REMOVE_EVERYONE_SYMBOL,
			  LIBSMBCONF_SO_PATH,
			  error ? error : ""));
		goto exit_with_dlclose;
	}

	/* Execute symbol. */
	ret = func_ptr_to_lp_remove_everyone();

exit_with_dlclose:
	/* Close library. */
	dlclose(handle);

exit_without_dlclose:
	return ret;
}

/* Workaround. Call sid_string_dbg() at run-time. */
static char* dlopen_sid_string_dbg(const struct dom_sid *sid)
{
	char * ret = NULL;
	void *handle = NULL;
	const char *error = NULL;
	char* (*func_ptr_to_sid_string_dbg)(const struct dom_sid *sid) = NULL;
	...
	/* Search symbol. */
	func_ptr_to_sid_string_dbg = dlsym(handle, SID_STRING_DBG_SYMBOL);
	error = dlerror();
	if (func_ptr_to_sid_string_dbg == NULL) {
		DEBUG(0, ("Failed to dlsym %s in %s: %s\n", 
			  SID_STRING_DBG_SYMBOL,
			  LIBSMBCONF_SO_PATH,
			  error ? error : ""));
		goto exit_with_dlclose;
	}
	...
	/* Execute symbol. */
	ret = func_ptr_to_sid_string_dbg(sid);
			  ...
			  uint32_t *access_granted)
{
	...
	*access_granted = access_desired;
	...
	for (i=0; bits_remaining && i < sd->dacl->num_aces; i++) {
		...
		case SEC_ACE_TYPE_ACCESS_DENIED_OBJECT:
#ifdef QNAPNAS
			if (dlopen_lp_remove_everyone() && dom_sid_equal(&ace->trustee, &global_sid_World)) {
				DEBUG(1, ("Ignore Everyone's (%s) denied permission\n", 
					  dlopen_sid_string_dbg(&ace->trustee)));
				continue;
			}
#endif /*QNAPNAS */
	...
	if (security_token_has_sid(token, sd->owner_sid)) {
		if (owner_rights_default) {
			 ...
			 */
#ifdef QNAPNAS
			/* FIXME: Code flow changed, should be put here? */
