				  ...
				  uint8_t *pauthoritative)
{
	...
	*pauthoritative = 1;
	...
	if (unix_username == NULL) {
		unix_username = user_info->client.account_name;
	}

	/* Check account expiry. */
	char cmd_account_chk[64] = {0};
	snprintf(cmd_account_chk, sizeof(cmd_account_chk),
		 "/sbin/user_expiry -C -A -u %d", (int)nametouid(unix_username));
	DEBUG(5, ("cmd=%s\n", cmd_account_chk));
	if (smbrun(cmd_account_chk, NULL, NULL) != 0) {
		DEBUG(0, ("check_ntlm_password:  user: %s(%d) NT_STATUS_ACCOUNT_EXPIRED \n",
			  unix_username, (int)nametouid(unix_username))); 
		nt_status = NT_STATUS_ACCOUNT_EXPIRED;
		goto fail;
	}

	/* Check password expiry. */
	char cmd_pwd_chk[64] = {0};
	snprintf(cmd_pwd_chk, sizeof(cmd_pwd_chk), 
		 "/sbin/user_expiry -C -P -u %d", (int)nametouid(unix_username));
	DEBUG(5, ("cmd=%s\n", cmd_pwd_chk));
	if (smbrun(cmd_pwd_chk, NULL, NULL) != 0) {
		DEBUG(0, ("check_ntlm_password:  user: %s(%d) NT_STATUS_PASSWORD_EXPIRED \n",
			  unix_username, (int)nametouid(unix_username))); 
		nt_status = NT_STATUS_PASSWORD_EXPIRED;
		goto fail;
	}

#ifdef APPLICATION_PRIVILEGE 
	/*
	 * KS-Redmine#36654
	 * The Application Privileges on UI for "Microsoft Network" is gray-out.
	 * It's not supported when NAS enable as DC.
	 */
	if (lp_server_role() != ROLE_ACTIVE_DIRECTORY_DC) {
		char cmd[64] = {0};
		snprintf(cmd, sizeof(cmd), "/sbin/appriv -C -u %d -S1",
			 (int)nametouid(unix_username));
		DEBUG(5, ("cmd=%s\n", cmd));
		if (smbrun(cmd, NULL, NULL) != 0) {
			DEBUG(0, ("check_ntlm_password:  user: %s(%d) appriv return NT_STATUS_ACCESS_DENIED \n",
				  unix_username, (int)nametouid(unix_username)));
			nt_status = NT_STATUS_ACCESS_DENIED;
			goto fail;
		}
	}
#endif /* APPLICATION_PRIVILEGE */

	/* Set user quota. */
	if (!(server_info->guest)) {
		char cmd[64] = {0};
		snprintf(cmd, sizeof(cmd), "/sbin/create_home -q %d",
			 (int)nametouid(unix_username));
		DEBUG(5, ("cmd=%s\n", cmd));
		if (smbrun(cmd, NULL, NULL) != 0) {
			DEBUG(3, ("check_ntlm_password:  user: %s(%d) set quota fail \n",
				  unix_username, (int)nametouid(unix_username))); 
		}
	}

	write_connlog_login(user_info, CONN_TYPE_INFO, CONN_ACTION_LOGIN_SUCC);
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
	write_connlog_login(user_info, CONN_TYPE_WARN, CONN_ACTION_LOGIN_FAIL);
#endif /* QNAPNAS */
