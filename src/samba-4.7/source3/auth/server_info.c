	...
	if (sid_check_is_in_unix_groups(group_sid)) {
		...
		if (!ok) {
			DEBUG(1, ("The primary group domain sid(%s) does not "
				...
				sid_string_dbg(user_sid)));
#ifdef QNAPNAS
			/* FIXME: Workaround to samba bug #8598.
			 * The code logic in samba-3.6.x would fail out while
			 * there is mismatched sid existed. This failure cause
			 * code flow does not reach later authentication, though
			 * end users had entered correct account and password,
			 * still failed to login thru samba. Avoid to confuse
			 * these vaild users, pass this failure until accepted
			 * patch is available. */
			DEBUG(1, ("Pass mismatched sid. Code flow is still "
				  "good to go\n"));
			/* Copy group sid into extra primary group sid.
			 * This extra information could help code flow move on
			 * while there is mismatched sid existed.
			 * See create_local_nt_token_from_info3(). */
			sid_copy(&extra->pgid_sid, group_sid);
			/* Do not release info3, do not return error. Code flow
			 * is still good to go. */
#else /* QNAPNAS */
