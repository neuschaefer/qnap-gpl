			...
			size_t count)
{
	...
	static int threshold = 1024 * 128; // 128k
#else /* QNAPNAS_RECVFILE */
	...
	while (total_written < count) {
		...
		/* FIXME: transfer_count = MIN(need_written, threshold); */
		transfer_count = (need_written > threshold) ? threshold : need_written;
		
		/* Call splice with src fd and dest fd. */
		nread = splice(fromfd, NULL, tofd, &offset, transfer_count, 0);
#else /* 
       * QNAPNAS_RECVFILE. Original code. 
       * Loop until count reached 0.
       * From src fd to pipe. 
       */
	while (count > 0) {
		int nread, to_write;
...
#ifndef QNAPNAS_RECVFILE /* 
			  * QNAPNAS_RECVFILE. Original code. 
			  * From pipe to dest fd. 
			  */
		to_write = nread;
		while (to_write > 0) {
			int thistime;
