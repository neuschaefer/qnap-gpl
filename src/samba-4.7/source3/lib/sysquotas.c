...
#ifdef QNAPNAS
	/* Fix Bug#72930 */
	if (ret != 0 && strstr_m(bdev, ".__eN__") != NULL) {
		char *real_mntpath = NULL;
		char *real_bdev = NULL;
		char *real_fs = NULL;
		char *real_path = SMB_STRDUP(bdev);
		
		if ((ret = sys_path_to_bdev(real_path, &real_mntpath, &real_bdev, &real_fs)) == 0) {
			DEBUG(3, ("sys_path_to_bdev() for path [%s] mntpath[%s] bdev[%s] !\n", 
				  real_path, real_mntpath, real_bdev));

			ret = sys_get_vfs_quota(real_mntpath, real_bdev, qtype, id, dp);
			if (ret!=0) {
				DEBUG(3, ("sys_get_%s_quota() failed for mntpath[%s] bdev[%s] qtype[%d] id[%d]: %s\n",
					 "vfs", real_mntpath, real_bdev, qtype,
					 (qtype == SMB_GROUP_QUOTA_TYPE ? id.gid : id.uid), strerror(errno)));
			} else {
				DEBUG(3, ("sys_get_%s_quota() called for mntpath[%s] bdev[%s] qtype[%d] id[%d].\n",
					  "vfs", real_mntpath, real_bdev,qtype, 
					  (qtype == SMB_GROUP_QUOTA_TYPE ? id.gid : id.uid)));
			}
		}

		SAFE_FREE(real_mntpath);
		SAFE_FREE(real_bdev);
		SAFE_FREE(real_fs);
		SAFE_FREE(real_path);
	}
