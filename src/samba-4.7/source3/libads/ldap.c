...
#ifdef QNAPNAS
char *ads_get_samaccountname_ou_string(ADS_STRUCT *ads, const char *samaccountname)
{
        ADS_STATUS status;
        LDAPMessage *res = NULL;
        char *base, *wkn_dn = NULL, *ret = NULL;
        ...
        if (asprintf(&base, "(sAMAccountName=%s)", samaccountname) == -1) {
                DEBUG(1, ("asprintf failed!\n"));
                return NULL;
        }
        status = ads_do_search(ads, ads->config.bind_path, LDAP_SCOPE_SUBTREE,
                                  base, attrs, &res);
	//status = ads_search_retry(ads, &res, base, attrs);
        if (!ADS_ERR_OK(status)) {
	...
	char *s = NULL;
	if((s=strcasestr(wkn_dn,ads->config.bind_path))!=NULL){
		*s='\0';
		s = NULL;
		
		if (asprintf(&s, "%s", wkn_dn) == -1) {
			SAFE_FREE(ret);
			goto out;
		}
		SAFE_FREE(ret);
