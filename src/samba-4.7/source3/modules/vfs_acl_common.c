				...
				struct security_descriptor *psd)
{
	...
	access_mask = map_canon_ace_perms(SNUM(conn), &acltype,
				(mode << 6) & 0700, false);
	init_sec_ace(&new_ace_list[num_aces+num],
			&global_sid_World,
			acltype,
			access_mask,
			SEC_ACE_FLAG_CONTAINER_INHERIT|
...
#ifdef QNAPNAS
/*******************************************************************
 replace Everyone (sid: S-1-1-0) to nas's everyone (gid: 100)
*******************************************************************/

static void replace_Everyone_ace(struct security_descriptor *psd)
{
	int i = 0, remove_flag = 0;
	int debug = 0;
	if (DEBUGLVL(10)) {
		debug = 1;
	}
	if(psd && psd->dacl){
		DEBUG(10, ("dacl's ace number=%d acl size=%d\n",psd->dacl->num_aces, psd->dacl->size));
	}else{
		return;
	}
...
#if 0
		DEBUG(1,("%s %d size=%d sid=%s everyone=%s cmp=%d\n",__func__,__LINE__,
		    psd->dacl->aces[i].size,
		    sid_string_dbg(&psd->dacl->aces[i].trustee),
		    sid_string_dbg(&global_sid_World),
		    sid_equal(&psd->dacl->aces[i].trustee, &global_sid_World)));
#endif
		if (debug) {
			dbgtext("dacl sid=%s ace size=%d\n",
				sid_string_dbg(&psd->dacl->aces[i].trustee),
				psd->dacl->aces[i].size);
		}
		if(dom_sid_equal(&psd->dacl->aces[i].trustee, &global_sid_World)){
			//find everyone's sid
			struct dom_sid sid;
			gid_to_sid( &sid, (gid_t)100);
			if (debug) {
				dbgtext("    replace Everyone (%s) to everyone (uid:100 sid:%s)\n",
					sid_string_dbg(&global_sid_World),
					sid_string_dbg(&sid));
			}
			//replace sid
			psd->dacl->aces[i].trustee=sid;
			//replace size
			//DEBUG(1,("%s %d size1=%d size2=%d \n",__func__,__LINE__,psd->dacl->aces[i].size,ndr_size_dom_sid(&sid, 0)+8));
			psd->dacl->size +=  ndr_size_dom_sid(&sid, 0) + 8 - psd->dacl->aces[i].size;
			psd->dacl->aces[i].size += ndr_size_dom_sid(&sid, 0) + 8 - psd->dacl->aces[i].size;
			i--;
			remove_flag = 1;
		}
	}
	...
	DEBUG(10,("After replacing Everyone dacl number=%d size=%d\n", psd->dacl->num_aces, psd->dacl->size));
	if (debug) {
		for (i = 0; i < psd->dacl->num_aces; i++){
			dbgtext("dacl sid=%s ace size=%d\n",
				sid_string_dbg(&psd->dacl->aces[i].trustee),
...
/*******************************************************************
 remove Everyone (sid: S-1-1-0)
*******************************************************************/

static void remove_Everyone_ace(struct security_descriptor *psd)
{
	int i = 0, remove_flag = 0;
	int debug = 0;
	if (DEBUGLVL(10)) {
	...
	for (i = 0; i < psd->dacl->num_aces; i++){
		if (debug) {
			dbgtext("dacl->aces[%d] sid=%s ace size=%d\n",
				i,
				sid_string_dbg(&psd->dacl->aces[i].trustee),
				psd->dacl->aces[i].size);
		}
		if(dom_sid_equal(&psd->dacl->aces[i].trustee, &global_sid_World)){
			...
			int j, k = 0;
			if (debug) {
				dbgtext("    remove Everyone (%s)\n",
					sid_string_dbg(&global_sid_World));
			}

			psd->dacl->size -=  psd->dacl->aces[i].size;

			for(j=0; j < psd->dacl->num_aces; j++){
				if(j == i) 
					continue;
				memmove(&new_ace_list[k++], &psd->dacl->aces[j], sizeof(struct security_ace));
			}
	...
	DEBUG(10,("After removing Everyone dacl number=%d size=%d\n", psd->dacl->num_aces, psd->dacl->size));
	if (debug) {
		for (i = 0; i < psd->dacl->num_aces; i++){
			dbgtext("dacl->aces[%d]  sid=%s ace size=%d\n",
				i,
				    ...
				    struct security_descriptor **ppdesc)
{
	...
	if (psd == NULL) {
		 ...
		 * known */
#ifdef QNAPNAS
		/* Bug#124647 (KS-Redmine#34296)
		 *
		 * Always call SMB_VFS_NEXT_FGET_NT_ACL to fetch default Windows ACL
		 */
#else
		if (config->ignore_system_acls) {
			SMB_STRUCT_STAT sbuf;
...
#ifdef QNAPNAS
	/* The Portal folder need Everyone permission to pass permission 
	   check. Otherwise, we remove Everyone (S-1-1-0) ace. */
	if(lp_remove_everyone() && !lp_msdfs_root(SNUM(handle->conn)))
		remove_Everyone_ace(psd);
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
        // replace Everyone (S-1-1-0) to everyone (gid:100)
        if(lp_remove_everyone())
                replace_Everyone_ace(psd);
#endif /* QNAPNAS */
