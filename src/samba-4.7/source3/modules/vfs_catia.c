...
#ifdef QNAPNAS
		/* No mappings found. Check if Qmapping is necessary. */
		if (IsClientMacOs() && IsQmappingEnable() && access(name_in, F_OK) == -1) {
			DEBUG(10, ("vfs_catia: It's MacOS client and filename not found in NFC format.\n"));
			char *result = RunQmapping(name_in);
			if (result == NULL)
				*mapped_name = talloc_strdup(NULL, name_in);
			else {
				*mapped_name = talloc_strdup(NULL, result);
				free(result);
			}
		}
		else
