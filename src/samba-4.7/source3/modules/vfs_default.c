...
#ifdef QNAPNAS
//#include <cfg_recycle_bin.h>
#ifndef __CFG_RECYCLE_BIN_H__
#define __CFG_RECYCLE_BIN_H__
enum {
	QUNLINK = 0,
	UNLINK
};
#endif /* __CFG_RECYCLE_BIN_H__ */
...
#ifdef QNAPNAS
/* QNAPNAS_TIMEMACHINE
 * KS-Redmine#20912 note-10 & KS-Redmine#22870
 *
 * fsync() for parent folder required after follwing operations :
 * unlink, rmdir, rename, and create new file ...
 */
int sync_parent_dir(const int snum, const char *name)
{
	char fullpath[4096] = {0};
	int ret;

	if ((get_remote_arch() != RA_OSX) || !lp_strict_sync(snum)) {
		return 0;
	}
	...
	if (strstr(fullpath, "sparsebundle") == NULL) {
		return 0;
	}
	...
	int fd = open(fullpath,O_SYNC|O_DIRECTORY);
	if (fd != -1) {
		ret = fsync(fd);
		if (ret != 0) {
			DEBUG(3,("[QNAPNAS] sync_parent_dir do fsync() fail ,errno [%d]\n", errno));
		}
		close(fd);
	} else {
		DEBUG(3,("[QNAPNAS] sync_parent_dir : open [%s] fail\n", fullpath));
		ret = -1;
	}
...
#ifdef QNAPNAS
	result = qnap_rmdir(smb_fname->base_name);

	/* QNAPNAS_TIMEMACHINE */
	sync_parent_dir(SNUM(handle->conn), smb_fname->base_name);
#else /* QNAPNAS */
...
#ifdef QNAPNAS
	if (qnap_unlink (dest) && errno != ENOENT) {
#else /* QNAPNAS */
			  ...
			  const struct smb_filename *smb_fname_dst)
{
	...
	/* QNAPNAS_TIMEMACHINE */
	sync_parent_dir(SNUM(handle->conn), smb_fname_dst->base_name);
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
static int _is_word_temp(char *pathname)
{
	char *tmp1, *tmp2 = 0 , tmp3[256] = "/";
	tmp1 = pathname;
	if(*pathname != '/'){
		tmp2 = strncat(tmp3, pathname, strlen(pathname));
		goto result;
	}else
		while(*tmp1 != '\0')
		{
...
static int _is_in_usb(const char *pathname)
{
	if (!strncmp("/share/external", pathname, strlen("/share/external"))) {
		return 1;
	}
...
static int _is_in_share_folder(char *pathname)
{
	char *tmp = pathname+4;
	char *tmp1 = pathname+10;
	static char sharepath[2048];
	static int checkMultimediaPath = 1;

	/* Bug#116796 (KS-Redmine#31634)
	 *
	 * Checking multimedia share path 'once' while file deleteion
	 */
	if (checkMultimediaPath == 1) {
		char sharename[33] = {"Qmultimedia"};
		FILE *fp = popen("/bin/grep defMultimedia /etc/config/def_share.info | /bin/cut -d ' ' -f 3", "r");
		if (fp != NULL) {
			fgets(sharename, 33, fp);
			sharename[strlen(sharename)-1] = 0x0;

			pclose(fp);
		}
		strncpy(sharepath, "/share/", strlen("/share/"));
		strncat(sharepath, sharename, strlen(sharename));

		checkMultimediaPath = 0;
	}
	...
	if(!strncmp("_DATA/", tmp1, 6))
		...
		return 2;
	else if(!strncmp(sharepath, pathname, 18))
		return 1;
	// HAL single disk share is in the WWN form. e.g "/share/5000C500186DAE24_DATA"
	else if (strstr(pathname, "_DATA/")) {
		if (!strncmp("/share", pathname, strlen("/share")))
			return 1;
		else
			return 2;
	}
...
static int _is_in_trash(char *pathname)
{
	char *tmp = pathname+4;
	...
	if(!strncmp("/share/HDA_DATA/Network Recycle Bin", pathname, 35))
		...
		return 2;
	else if(!strncmp("_DATA/Network Recycle Bin", tmp2, 25))
		return 1;
	// HAL single disk share is in the WWN form. e.g "/share/5000C500186DAE24_DATA"
	else if (strstr(pathname, "_DATA/Network Recycle Bin")) {
		if (!strncmp("/share", pathname, strlen("/share")))
			return 1;
		else
			return 2;
...
static int _is_apple_special_file(const char *pathname)
{
	if (pathname[0] == '.') {
		if(!strncmp(pathname, ".AppleDouble", 12)
		|| !strncmp(pathname, ".DS_Store", 9)) {
			return 1;
		} else {
			return 0;
		}
...
static int _file_filter(const char *name)
{
	int ret1, ret2;
	struct stat file;
	...
	if (!_is_word_temp(full_path)) {
		return QUNLINK;
	}

	if (_is_in_usb(full_path)) {
		return QUNLINK;
	}

	if (_is_apple_special_file(name)) {
		return QUNLINK;
	}

	ret1 = _is_in_share_folder(full_path);
	ret2 = _is_in_trash(full_path);
			  ...
			  const struct smb_filename *smb_fname)
{
...
#ifdef QNAPNAS
	if (recycle_enable && (_file_filter(smb_fname->base_name) == UNLINK)) {
		uid_t euid = geteuid();
		//We have to set uid be root or unlink will fail if the option "Only the owner can delete the contents" is enabled
		seteuid(0);
#endif /* QNAPNAS */
	result = unlink(smb_fname->base_name);
		...
		seteuid(euid);
                ...
                result = qnap_unlink(smb_fname->base_name);
	}
...
#ifdef QNAPNAS
	/*
	 * If POSIX ACL (Advanced Folder Permission) is disabled,
	 * 'remove everyone = no', acl_get_file() is not required,
	 * this can save time by avoid syscall: getxattr().
	 * 
	 * FIXME: Move to vfswrap_sys_acl_get_XXX is sufficient?
	 */
	if (!lp_remove_everyone()) {
		goto skip_chmod_acl;
	}
#endif /* QNAPNAS */
	/*
...
#ifdef QNAPNAS
		/* Bug#225723 (JIRA#SW2-333) */
		if (result == 0) {
			/* Update the files_struct after successful ftruncate */
			vfs_stat_fsp(fsp);
		}
#endif
...
//Patch by QNAP: Scan and compare by case insensitive filename
	...
	*found_name = talloc_strdup(mem_ctx,name);
	...
	result = qnap_find_filename(path,*found_name);
	if(result !=  0){
		TALLOC_FREE(*found_name);
		//for kernel haven't support this syscall
		if(result == -2 || (result == -1 && errno == ENXIO)){
			errno = EOPNOTSUPP;
			result = -1;
		}
	}

	/* if underlying filesystem is not ext4, guess it's Gluster FS */
	if (result == -1 && errno == EOPNOTSUPP) {
		char *attr_name = talloc_zero_size(mem_ctx, strlen(name)+32);
		char *value = talloc_strdup(mem_ctx, name);
		fstrcat(attr_name, "system.qnap.cisearch.");
		fstrcat(attr_name, name);
		char *resolved_name = talloc_zero_size(mem_ctx, PATH_MAX);
		char *real_path = realpath(path, resolved_name);
		int ret = getxattr(real_path, attr_name, value, strlen(value));
		if (ret > 0) {
			result = errno = 0;
			*found_name = value;
	...
	DEBUG(3,("path = %s,name = %s,result = %d\n",path,*found_name,result));
					  ...
					  TALLOC_CTX *mem_ctx)
{
	 ...
	 * this can save time by avoid syscall: getxattr().
	 * 
	 * FIXME: 'remove everyone' is so far boolean for POSIX ACL.
	 *        Revise me if design changed.
	 *        POSIX ACL enabled,  'remove everyone = yes'.
	 *        POSIX ACL disabled, 'remove everyone = no'.
	 */
	if (!lp_remove_everyone()) {
		return NULL;
	}
