...
#ifdef QNAPNAS
	/* Bug#114828 (KS-Redmine#31135)
	 *
	 * Ignore wrong size Finder Info entry in AppleDouble file,
	 * yet preserve them when moving or copying files.
	 */
	return len;
#endif /* QNAPNAS */
...
#if 0//def QNAPNAS
	/*
	 * FIXME: Supposed to load vfs_fruit only for Mac OSX client?
	 * Only Mac OSX client will read org.netatalk.Metadata,
	 * Windows client can save time by avoid syscall: getxattr().
	 */
...
#ifdef QNAPNAS
	/* Bug#127492 (KS-Redmine#36167) */
	config->open_zero_rsrc =
	    lp_parm_bool(SNUM(handle->conn), FRUIT_PARAM_TYPE_NAME, "open_zero_rsrc", true);
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
		/* Bug#140743 (Jira SW2-53) 
		 *
		 * Fake an empty finderinfo.
		 */
		ai->afpi_Signature = AFP_Signature;
		ai->afpi_Version = AFP_Version;
		memset(ai->afpi_FinderInfo, 0, sizeof(ai->afpi_FinderInfo));
#else
		TALLOC_FREE(ai);
#endif
	}
...
#ifdef QNAPNAS
	/* Bug#111440 (KS-Redmine#29898)
	 *
	 * Whould not delete ._ AppleDouble files when fruit:veto_appledouble is set to no
	 */
	if (config->veto_appledouble == false) {
		return SMB_VFS_NEXT_UNLINK(handle, smb_fname);
	}
#endif  /* QNAPNAS*/
				 ...
				 size_t n, off_t offset)
{
	 ...
	 */
#ifdef QNAPNAS
	/* Bug#105743 (KS-Redmine#29479)
	 *
	 * fruit_pwrite_meta_netatalk() always writes 60 bytes
	 * even if AFP_AfpInfo stream is oversize.
	 */
	if (offset != 0) {
		DBG_ERR("unexpected offset=%jd or size=%jd\n",
			(intmax_t)offset, (intmax_t)n);
		return -1;
	} else if (n != AFP_INFO_SIZE) {
		if (fio->config->meta == FRUIT_META_NETATALK) {
			/* We pass through this situation */
		} else {
			DBG_ERR("unexpected offset=%jd or size=%jd\n",
				(intmax_t)offset, (intmax_t)n);
			return -1;
