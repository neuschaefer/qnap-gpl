...
#ifdef QNAPNAS
	/* QTS_SNAPSHOT
	 * Bug#120538 (KS-Redmine#33692)
	 *
	 * Make sure operations for @GMT file are fine when 'Advancd permissions' is enabled.
	 */
			...
			uint32_t attr)
{
	 ...
	 * Make sure operations for @GMT file are fine when 'Advancd permissions' is enabled.
	 */
	if (ret == NULL && lp_inherit_acls(SNUM(handle->conn))) {
		become_root();
		ret = SMB_VFS_NEXT_OPENDIR(handle, conv_smb_fname, mask, attr);
		unbecome_root();
		if (ret != NULL) {
			     ...
			     struct smb_filename *smb_fname)
{
	 ...
	 * Make sure operations for @GMT file are fine when 'Advancd permissions' is enabled.
	 */
	if (ret == -1 && lp_inherit_acls(SNUM(handle->conn))) {
		become_root();
		ret = SMB_VFS_NEXT_STAT(handle, smb_fname);
		unbecome_root();
		if (ret != -1) {
...
#ifdef QNAPNAS
	/* QTS_SNAPSHOT
	  * Bug#103056 (KS-Redmine#26958)
	  *
	  * Skip checking snapdir's permissions
	  */
	status = NT_STATUS_OK;
#else
...
#ifdef QNAPNAS
	/* QTS_SNAPSHOT
	  * Spec#11527 (KS-Redmine#25003)
	  */
	bool show_previous = lp_parm_bool(SNUM(handle->conn), "shadow", "showprevious", true);
	if (show_previous == false) {
...
#ifdef QNAPNAS
		/* QTS_SNAPSHOT
		 * Bug#54598 (KS-redmine#11281)
		 *
		 * When user log in home folder at the first time, home folder should be created.
		 */
		if (config->mount_point == NULL) {
			if (!strncasecmp_m(handle->conn->connectpath , "/share/homes/", strlen("/share/homes/"))
				 /*Spec#8987 (KS-Redmine#22893)*/
				|| lp_tmredirect(SNUM(handle->conn))) {
				if (*lp_root_preexec(talloc_tos(), SNUM(handle->conn))) {
					char *cmd = talloc_sub_advanced(talloc_tos(),
							...
							lp_root_preexec(talloc_tos(), SNUM(handle->conn)));
					DEBUG(3,("create user home folder : cmd=%s\n", cmd));
