...
#ifdef QNAPNAS
/* Bug#106107 (KS-Redmine#28189)
 *
 * 1. chmod 777 to .streams/XX/YY/aaaa.bbbb
 * 2. Clear POSIX ACLs and NTACL
 */
static void stream_dir_chmod(vfs_handle_struct *handle, const char *path)
{
	become_root();
	if (chmod(path, 0777) != 0)
		DEBUG(10, ("stream_dir_chmod: chmod fail: [%s][%s]\n", path, strerror(errno)));

	if (lp_inherit_acls(SNUM(handle->conn))) {
		removexattr(path, "system.posix_acl_access");
		removexattr(path, "system.posix_acl_default");
		removexattr(path, "security.NTACL");
	}
	unbecome_root();
}
#endif	/* QNAPNAS */
...
#ifdef QNAPNAS
	/* Bug#135419 (KS-Redmine#38496) */
	if (!create_it && (SMB_VFS_NEXT_STAT(handle, smb_fname_hash) != 0)) {
		char *path;

		stream_dir_chmod(handle, rootdir);

		path = talloc_asprintf(result, "%s/%2.2X", rootdir, first);
		if (path != NULL) {
			stream_dir_chmod(handle, path);
			TALLOC_FREE(path);
		}
		path = talloc_asprintf(result, "%s/%2.2X/%2.2X", rootdir, first, second);
		if (path != NULL) {
			stream_dir_chmod(handle, path);
			TALLOC_FREE(path);
		}

		stream_dir_chmod(handle, result);
	}
#endif
...
#ifdef QNAPNAS
	become_root();
	if ((SMB_VFS_NEXT_MKDIR(handle, rootdir_fname, 0777) != 0)
	    && (errno != EEXIST)) {
	    unbecome_root();
		goto fail;
	}
	unbecome_root();
	stream_dir_chmod(handle, rootdir_fname->base_name);
#else
...
#ifdef QNAPNAS
	become_root();
	if ((SMB_VFS_NEXT_MKDIR(handle, tmp_fname, 0777) != 0)
	    && (errno != EEXIST)) {
	    unbecome_root();
		goto fail;
	}
	unbecome_root();
	stream_dir_chmod(handle, tmp_fname->base_name);
#else
...
#ifdef QNAPNAS
	become_root();
	if ((SMB_VFS_NEXT_MKDIR(handle, smb_fname_hash, 0777) != 0)
	    && (errno != EEXIST)) {
		unbecome_root();
		goto fail;
	}
	unbecome_root();
	stream_dir_chmod(handle, smb_fname_hash->base_name);
#else
...
#ifdef QNAPNAS
	/* Bug#135419 (KS-Redmine#38496) */
	stream_dir_chmod(handle, stream_fname);
#endif
...
#ifdef QNAPNAS
	if (dirhandle == NULL && NT_STATUS_EQUAL(map_nt_error_from_unix(errno), NT_STATUS_ACCESS_DENIED)) {
		stream_dir_chmod(handle, dirname);
