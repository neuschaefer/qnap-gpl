			     ...
			     size_t smb_maxcnt)
{
	...
	if (fsp->base_fsp != NULL) {
...
#ifdef QNAPNAS
        int i=0, get_offset=0;

        for(i=0;i<SMART_AIO_RECORD_MAX;i++){
                if (fsp->record[i].pre_offset == startpos) {
                        if (fsp->record[i].con_access < 100) {
                                fsp->record[i].con_access++;
                        }
                        get_offset=1;
                        break;
                }
        ...
        if(!get_offset){
                fsp->i_count=fsp->i_count%SMART_AIO_RECORD_MAX;
                i=fsp->i_count++;

                fsp->record[i].con_access = 0;

        }
        fsp->record[i].pre_offset = startpos + smb_maxcnt;
        if (get_offset) {
                DEBUG(10,("smb2: con_access get_offset\n"));
                return NT_STATUS_RETRY;
        }

        if (fsp->record[i].con_access > 5) {
                DEBUG(10,("smb2: con_access large than 10\n"));
                return NT_STATUS_RETRY;
        }
        else
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
	/* only log at the beginning of operation */
	if (lp_conn_log() && in_offset == 0 && !is_ntfs_stream_smb_fname(fsp->fsp_name)) {
		write_connlog_write(conn, fsp_str_dbg(fsp));
	}
#endif /* QNAPNAS */
