...
#ifdef QNAPNAS
		if (lp_conn_log()) {
			write_connlog_delonclose(conn, fsp_str_dbg(fsp));
		}
#endif /* QNAPNAS */
	}
...
static NTSTATUS rmdir_internals(TALLOC_CTX *ctx, files_struct *fsp)
{
	...
	if(((errno == ENOTEMPTY)||(errno == EEXIST))
		&& ((get_remote_extended_arch() != ERA_QNAP) && *lp_veto_files(talloc_tos(), SNUM(conn)))) {
#else
...
#ifdef QNAPNAS
			if (lp_conn_log()) {
				write_connlog_rmdir(fsp->conn, fsp_str_dbg(fsp));
			}
#endif /* QNAPNAS */
		}
	} else {
