...
uint32_t dos_mode(connection_struct *conn, struct smb_filename *smb_fname)
{
	...
	if (result & FILE_ATTRIBUTE_DIRECTORY) {
		if ((strcmp(smb_fname->base_name, "@Recycle") == 0) ||
		    (strcmp(smb_fname->base_name, "./@Recycle") == 0) ) {
			DEBUG(8,("dos_mode: set recycle bin %s\n", smb_fname->base_name));
			result |= FILE_ATTRIBUTE_SYSTEM;
		}
	}
#endif /* QNAPNAS */
		     ...
		     uint32_t dosmode, const char *parent_dir, bool newfile)
{
	...
	if (!lp_remove_everyone()) {
		goto skip_get_acl_group_bits;
	}
#endif /* QNAPNAS */
	get_acl_group_bits(conn, smb_fname,
			&smb_fname->st.st_ex_mode);
#ifdef QNAPNAS
skip_get_acl_group_bits:
#endif /* QNAPNAS */
