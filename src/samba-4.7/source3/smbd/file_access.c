...
#ifdef QNAPNAS
// Fix  Bug#39196 & Bug#59172 
bool can_delete_file_in_directory_stickybit(connection_struct *conn,
				  const struct smb_filename *smb_fname)
{
	...
	if (smb_fname_parent->st.st_ex_mode & S_ISVTX) {
		...
		if ((get_current_uid(conn) !=
			...
			DEBUG(10,("can_delete_file_in_directory_stickybit: not "
...
#ifdef QNAPNAS
		if (!can_delete_file_in_directory_stickybit(fsp->conn, fsp->fsp_name)) {
			return NT_STATUS_ACCESS_DENIED;
		}
#endif /* QNAPNAS */
