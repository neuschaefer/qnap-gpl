...
#ifdef QNAPNAS
	else {	//We support not all lower case base_name. QNAP Charley
		if (strcasecmp_m(pdp->reqpath, smb_fname->base_name) == 0 && strcmp(pdp->reqpath, smb_fname->base_name) != 0) {
			memcpy(smb_fname->base_name, pdp->reqpath, strlen(pdp->reqpath)+1);
		}
		DEBUG(1,("dfs_path_lookup: reqpath='%s', base_name='%s'\n", pdp->reqpath, smb_fname->base_name));
	}
#endif /* QNAPNAS */
