...
#ifdef QNAPNAS
	if (create_disposition == FILE_OVERWRITE_IF || create_disposition == FILE_CREATE) {
		if (lp_conn_log() && (create_options & FILE_DIRECTORY_FILE)) {
			write_connlog_mkdir(conn, fname);
//              	smb_fname->st.st_ex_mode = smb_fname->st.st_ex_mode | S_IRWXU;
//              	smb_fname->st.st_ex_mode = smb_fname->st.st_ex_mode & (~S_ISVTX);       // force to remove sticky bit
//              	SMB_VFS_CHMOD(conn, smb_fname->base_name, smb_fname->st.st_ex_mode);
		}
	}
#endif /* QNAPNAS */
