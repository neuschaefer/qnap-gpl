				...
				uint32_t access_mask)
{
	...
	if (!lp_inherit_acls(SNUM(conn))) {
		unx_mode = (smb_fname->st.st_ex_mode & 0777);
		if ((unx_mode == 0777) || (unx_mode == 0666)) {
			DEBUG(10,("smbd_check_access_rights: skip mode on %s 0%o\n",
				smb_fname_str_dbg(smb_fname),(int)unx_mode ));
			return NT_STATUS_OK;
		}
	}

	if (!strcmp(smb_fname_str_dbg(smb_fname),"@Recycle")
		&&lp_recycle_bin_administrators_only(SNUM(conn)))
			return NT_STATUS_OK;
#endif /* QNAPNAS */
	...
	if (!lp_inherit_acls(SNUM(conn))) {
		if (stat(parent_dir, &st) == 0) { 
			if ((st.st_mode & 0777) == 0777) {
				DEBUG(10,("check_parent_access: skip mode 0777 on %s\n",parent_dir));
				return NT_STATUS_OK;
			}
		}
	}
...
#ifdef QNAPNAS
	if (lp_conn_log() && *p_file_created && !is_ntfs_stream_smb_fname(smb_fname)) {
		write_connlog_add(conn, smb_fname_str_dbg(smb_fname));
	}

	if ((smb_fname->st.st_ex_mode & (S_IRUSR|S_IWUSR)) != (S_IRUSR|S_IWUSR)) {
		DEBUG(2, ("st.st_ex_mode=%o \n", smb_fname->st.st_ex_mode));
		smb_fname->st.st_ex_mode = smb_fname->st.st_ex_mode | S_IRUSR | S_IWUSR;
		SMB_VFS_CHMOD(conn, smb_fname, smb_fname->st.st_ex_mode);
	}
#endif /* QNAPNAS */
...
#ifdef QNAPNAS	
		if (!can_delete_file_in_directory_stickybit(conn, smb_fname)) {
			status = NT_STATUS_ACCESS_DENIED;
				     ...
				     int *pinfo)
{
	...
	*result = fsp;
	 ...
	 * KS-Redmine#22870 
	 */
	if (info == FILE_WAS_CREATED && strstr_m(fsp->fsp_name->base_name,"sparsebundle")!=NULL) {
		sync_parent_dir(SNUM(conn), fsp->fsp_name->base_name);
	}
#endif /* QNAPNAS */
