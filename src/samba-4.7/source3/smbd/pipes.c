...
#if defined(WITH_SPOTLIGHT) && defined(QNAPNAS)

	if (!strcmp(name, "mdssvc")) {

		FILE *fp;
		char qsirch_is_enabled[8] = {0};

		fp = popen("/sbin/getcfg Qsirch Enable -f /etc/config/qpkg.conf","r");

		if (fp!=NULL) {
			fgets(qsirch_is_enabled, sizeof(qsirch_is_enabled), fp);
			if (strcmp(qsirch_is_enabled, "TRUE\n")!=0) {
				DEBUG(1, ("[QNAP] Samba Spotlight is disalbed since Qsirch is disabled or not installled\n"));
				status = NT_STATUS_OBJECT_NAME_NOT_FOUND;
			} else {
				DEBUG(1, ("[QNAP] Samba Spotlight is enabled\n"));
			}
			pclose(fp);
		} else {
			DEBUG(1, ("[QNAP] Error: Unable to get Qsirch status\n"));
			status = NT_STATUS_OBJECT_NAME_NOT_FOUND;
		}
	}
#endif
