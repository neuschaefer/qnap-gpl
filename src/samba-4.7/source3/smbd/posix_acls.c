...
#ifdef QNAPNAS
#define FILE_SPECIFIC_READ_BITS (FILE_READ_DATA)
#define FILE_SPECIFIC_WRITE_BITS (FILE_WRITE_DATA)
...
#ifdef QNAPNAS
	// 'enhance acl v1' ignore owner group permission
	if (lp_enhance_acl_v1()) {
		got_group = true;
	}

	// Fix Bug#84111
	// Keep owner group permission when advanced permssions are disabled
					...
					const SMB_STRUCT_STAT *pst)
{
...
#ifdef QNAPNAS
	bool add_default_user = false;
	bool add_default_group = false;

	DEBUG(10, ("ensure_canon_entry_valid_on_set: is_directory:%s\n", is_directory?"yes":"no"));
#endif /* QNAPNAS */
	...
	if (!pace_user) {
		...
		DEBUG(10,("ensure_canon_entry_valid_on_set: add SMB_ACL_USER_OBJ.\n"));
#endif /* QNAPNAS */
		canon_ace *pace_iter;
	...
	if (!pace_group) {
		...
		DEBUG(10,("ensure_canon_entry_valid_on_set: add SMB_ACL_GROUP_OBJ.\n"));
#endif
		if ((pace = talloc(talloc_tos(), canon_ace)) == NULL) {
			DEBUG(0,("talloc fail.\n"));
			return false;
...
#ifdef QNAPNAS
	if (!got_duplicate_user && !add_default_user) {
		DEBUG(10,("ensure_canon_entry_valid_on_set: duplicate SMB_ACL_USER.\n"));
#else /* QNAPNAS */
...
#ifdef QNAPNAS
	if (!got_duplicate_group && !add_default_group) {
		DEBUG(10,("ensure_canon_entry_valid_on_set: duplicate SMB_ACL_GROUP.\n"));
#else /* QNAPNAS */
	...
	for(i = 0; i < dacl->num_aces; i++) {
		...
		if( dom_sid_equal(&current_ace->trustee, &global_sid_World)) {
			...
			if (unixid.type == ID_TYPE_BOTH) {
				...
				if (wbinfo_lookupsid_type(sid_string_dbg(&current_ace->trustee), &SidType)) {
					DEBUG(3,("uid(%d) sid(%s) (%s)\n", unixid.id, sid_string_dbg(&current_ace->trustee), wbcSidTypeString(SidType)));
					if (SidType == WBC_SID_NAME_USER) {
						current_ace->owner_type = UID_ACE;
						current_ace->unix_ug.type = ID_TYPE_UID;
						current_ace->unix_ug.id = unixid.id;
						current_ace->type = (unixid.id == pst->st_ex_uid) ? SMB_ACL_USER_OBJ : SMB_ACL_USER;
						goto ADD_ACE_TO_ACL;
					} else if (SidType == WBC_SID_NAME_DOM_GRP
							|| SidType == WBC_SID_NAME_ALIAS
							|| SidType == WBC_SID_NAME_WKN_GRP) {
						current_ace->owner_type = GID_ACE;
						current_ace->unix_ug.type = ID_TYPE_GID;
						current_ace->unix_ug.id = unixid.id;
 						current_ace->type = (unixid.id == pst->st_ex_gid) ? SMB_ACL_GROUP_OBJ : SMB_ACL_GROUP;
						goto ADD_ACE_TO_ACL;
					}
				}
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
				if (lp_enhance_acl_v1()) {
					DEBUG(10,("[enhance acl] ignore group_obj\n"));
					continue;
				}
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
	DEBUG(10,("before merge_default_aces num_aces=%d\n", num_aces));
	//for (i = 0; i < num_aces; i++) {
	//      dbgtext("    nt_ace_list[%d] sid=%s\n",i, sid_string_dbg(&nt_ace_list[i].trustee));
	//}
#endif /* QNAPNAS */
	for (i = 0; i < num_aces; i++) {
...
#ifdef QNAPNAS
        DEBUG(10,("after merge_default_aces num_aces=%d\n", num_aces));
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
		struct dom_sid *powner_sid = NULL;
		struct dom_sid *pgroup_sid = NULL;
		if (lp_enhance_acl_v1()) {
			powner_sid = &owner_sid;
			pgroup_sid = &group_sid;
		}else{
			powner_sid = &global_sid_Creator_Owner;
			pgroup_sid = &global_sid_Creator_Group;
		}
...
#ifdef QNAPNAS
				/* Bug#140403 (KS-Redmine#43283) */
				const struct security_token *token = get_current_nt_user_token(conn);
				if ((acc == 0) && (token != NULL) && security_token_is_sid(token, &ace->trustee)) {
					acc = SEC_STD_READ_CONTROL;
					nt_acl_type = SEC_ACE_TYPE_ACCESS_DENIED;
				}

				/* Enhancement Bug#141290 (JIRA#SW2-201) */
				if (lp_only_admin_change_posixacl(SNUM(conn)) == true) {
					if (get_current_uid(conn) == (uid_t)0) {
						acc |= SEC_STD_WRITE_DAC;
					} else {
						acc = (acc & ~SEC_STD_WRITE_DAC);
					}
				}
#endif
	...
	if (acl_perms && acl_set_support && fsp->is_directory) {
		if (dir_ace_list) {
			...
			if (sret == -1) {
				...
				if (sret == -1) {
#ifdef QNAPNAS       /* ignore posix acl return value to fix bug 25183 */
					DEBUG(1,("set_nt_acl: ignore sys_acl_delete_def_file failed (%s)\n", strerror(errno)));
#else /* QNAPNAS */
