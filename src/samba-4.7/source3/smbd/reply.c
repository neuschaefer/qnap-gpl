...
#ifdef QNAPNAS
	if (lp_conn_log() && !is_ntfs_stream_smb_fname(smb_fname)) {
		write_connlog_unlink(conn, smb_fname_str_dbg(smb_fname));
	}
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
	/* only log at the beginning of operation */
	if (lp_conn_log() && startpos == 0 && !is_ntfs_stream_smb_fname(fsp->fsp_name)) {
		write_connlog_read(conn, fsp_str_dbg(fsp));
	}
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
	if (!big_readX && lp_aio_read_size(SNUM(conn))) {
#else
	if (!big_readX) {
#endif /* QNAPNAS */
...
void reply_write(struct smb_request *req)
{
	...
	/* only log at the beginning of operation */
	if (lp_conn_log() && startpos == 0 && !is_ntfs_stream_smb_fname(fsp->fsp_name)) {
		write_connlog_write(conn, fsp_str_dbg(fsp));
	}
#endif /* QNAPNAS */
...
#ifdef QNAPNAS_RECVFILE
	if (lp_encryption(SNUM(fsp->conn))) {
		/* Bug#44753: Ecryptfs. Cannot recvfile. */
		DEBUG(10,("is_valid_writeX_buffer: encryption (Ecryptfs) = yes\n"));
		return false;
	}

	if (!lp_q_use_receivefile(SNUM(fsp->conn))) {
		/* Bug#84134: External drives. Cannot recvfile. */
		DEBUG(10,("is_valid_writeX_buffer: q use receivefile = no\n"));
		return false;
	}
#endif /* QNAPNAS_RECVFILE */
...
#ifdef QNAPNAS
	if (lp_conn_log()) {
		write_connlog_mkdir(conn, smb_fname_str_dbg(smb_dname));
	}
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
		if (lp_conn_log()) {
			write_connlog_mv(conn, smb_fname_str_dbg(fsp->fsp_name), 
					 smb_fname_str_dbg(smb_fname_dst));
		}
#endif /* QNAPNAS */
