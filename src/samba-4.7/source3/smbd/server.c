...
#ifdef QNAPNAS
// Modified by Jeff Chang 2011/5/9, for service binding
extern int g_bHUP;
...
T_BIND_ADDR g_tbaDaemon = {0, 0, 0, NULL, NULL};
#endif /* QNAPNAS */
...
static int Get_Binding_Address(PT_BIND_ADDR ptBindAddr)
{
	...
	if (NULL == (pfAddr = popen(SZP_GET_BIND_ADDR " CIFS 2>/dev/null", "r")))  return 0;
	while (NULL != fgets(szLine, sizeof(szLine), pfAddr))
	{
		szLine[sizeof(szLine)-1] = '\0';
		if (0 >= (cbAddr = strlen(szLine)))  continue;
		if ('\n' == szLine[cbAddr-1])  {szLine[cbAddr-1] = '\0';cbAddr = strlen(szLine);}
		if (NULL != strchr(szLine, ':'))
		{
            char **tmpp=NULL;
            if(NULL == (tmpp= (char**)realloc(ptBindAddr->ryszIpv6, (++ptBindAddr->cnIpv6)*sizeof(char*))))
            {
                ptBindAddr->cnIpv6--;
                goto _exit_;
            }
            ptBindAddr->ryszIpv6=tmpp;
            if(NULL == (ptBindAddr->ryszIpv6[ptBindAddr->cnIpv6-1] = (char*)malloc(INET6_ADDRSTRLEN)))
            {
                ptBindAddr->cnIpv6--;
                goto _exit_;
            }
            memcpy(ptBindAddr->ryszIpv6[ptBindAddr->cnIpv6-1], szLine, cbAddr);
            ptBindAddr->ryszIpv6[ptBindAddr->cnIpv6-1][cbAddr] = 0;
		}
		else if (NULL != strchr(szLine, '.'))
		{
            ...
            char **tmpp=NULL;
            if(NULL == (tmpp = (char**)realloc(ptBindAddr->ryszIpv4, (++ptBindAddr->cnIpv4)*sizeof(char*)*INET6_ADDRSTRLEN)))
            {
                ptBindAddr->cnIpv4--;
                goto _exit_;
            }
            ptBindAddr->ryszIpv4=tmpp;
            if(NULL == (ptBindAddr->ryszIpv4[ptBindAddr->cnIpv4-1] = (char*)malloc(INET_ADDRSTRLEN)))
            {
                ptBindAddr->cnIpv4--;
                goto _exit_;
            }
            memcpy(ptBindAddr->ryszIpv4[ptBindAddr->cnIpv4-1], szLine, cbAddr);
            ptBindAddr->ryszIpv4[ptBindAddr->cnIpv4-1][cbAddr] = 0;
        }
	}
	iRet = 0;
_exit_:
...
static int Check_Binding_Address(int fdAccept, PT_BIND_ADDR ptBindAddr, char *pszAddrBuf)
{
	...
	if (ptBindAddr->bEnabled)
	{
		...
		if ((AF_INET == tsaLocal.ss_family) && (0 < ptBindAddr->cnIpv4))
		{
			...
			char *pszINet = inet_ntoa(psiLocal->sin_addr);
			int cbINet = strlen(pszINet);
			memcpy(pszAddr, pszINet, cbINet);
			pszAddr[cbINet] = 0;
...
// Modified by Jeff Chang 2011/5/9, for service binding
	if(g_bHUP)
	{
		Get_Binding_Address(&g_tbaDaemon);
		g_bHUP = False;
	}

	// Check if the incomming interface matched the service binding settings.
	if (0 != (iBindRet = Check_Binding_Address(fd, &g_tbaDaemon, szIpAddr)))
	{
		close(fd);
		return;
	}
...
#ifdef QNAPNAS
		if (lp_use_interfaces_policy()) {
			struct sockaddr_storage local_sockaddr;
			socklen_t  local_sockaddr_len = sizeof(local_sockaddr);
		
			if (-1 != getsockname(fd, (struct sockaddr *)&local_sockaddr, &local_sockaddr_len)) {
				check_interfaces_policy(&local_sockaddr);
			}
		}
#endif /* QNAPNAS */
				 ...
				 uint16_t port)
{
	...
	if (s->fd == -1) {
#ifdef QNAPNAS
		DEBUG(3,("smbd_open_one_socket: open_socket_in: "
#else
					...
					void *private_data)
{
	...
	// Modified by Jeff Chang 2011/5/9, for service binding
	g_bHUP = True;
	DEBUG(3,("parent: Reloading services after SIGHUP\n"));
#else /* QNAPNAS */
 ...
 int main(int argc,const char *argv[])
{
...
#ifdef QNAPNAS
	{
		...
		system("/sbin/getcfg \"Network Recycle Bin\" Enable -u -d FALSE > /tmp/.recycle_bin");
		ret=system("/bin/grep FALSE /tmp/.recycle_bin 2>/dev/null 1>/dev/null");
		if (ret == 0)
			recycle_enable = 0;
		else
			recycle_enable = 1;
