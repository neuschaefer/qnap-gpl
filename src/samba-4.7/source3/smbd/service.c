					...
					const char *pdev)
{
	...
	status = share_sanity_checks(sconn->remote_address,
				       is_ipaddress(sconn->remote_hostname) ? get_remote_machine_name() : sconn->remote_hostname,
				       snum,
				       dev);
#else
	status = share_sanity_checks(sconn->remote_address,
...
#ifdef QNAPNAS
	conn->encrypt_level = (lp_server_max_protocol() >= PROTOCOL_SMB3_00? lp_smb_encrypt(snum) : SMB_SIGNING_OFF );
#else
...
#ifdef QNAPNAS
		DEBUG(5,("Can't become connected user!\n"));
#else
...
/* ROOT Activites: */
	...
	if (!IS_IPC(conn) && !IS_PRINT(conn)) {
		...
		if (get_remote_extended_arch() != ERA_QNAP) {
