...
#ifdef QNAPNAS
	if (session->global->channels[0].remote_name) {
		TALLOC_FREE(session->global->channels[0].remote_name);
	}
	session->global->channels[0].remote_name = get_remote_machine_name();
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
	/* 
	 * If already a root user, become_root/unbecome_root are not required,
	 * this can save time by avoid redundant syscalls, 
	 * e.g. [get|set]Xid(), [get|set]groups().
	 * FIXME: account = admin is so far design for root. 
	 *        Revise me if design changed. 
	 * FIXME: passdb backend = smbpasswd is so far design for local accounts.
	 *        Revise me if design changed. (e.g. tdbsam) 
	 */
	DEBUG(3,("username=%s, passdb backend=%s\n", username, lp_passdb_backend()));
	if (strcsequal(username, "admin") && strcsequal(lp_passdb_backend(), "smbpasswd")) {
		DEBUG(3,("set_smbd_shim_without_become_unbecome_root()\n"));
		set_smbd_shim_without_become_unbecome_root();
	}
#endif /* QNAPNAS */
