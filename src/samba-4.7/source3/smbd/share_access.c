...
#ifdef QNAPNAS
bool user_ok_token_usb(const char *username, const char *domain,
		   const struct security_token *token, int snum)
{
	...
	if (lp_valid_users(snum) != NULL) {
		char **userlist = str_list_copy(talloc_tos(),
				lp_valid_users(snum));
		if (userlist &&
		    str_list_substitute(userlist, "%u", username)) {
			if (!token_contains_name_in_list(username, domain,
							 lp_servicename(talloc_tos(), snum),
							 token,
							 (const char **)userlist)) {
	...
	if (lp_usb_users(snum) != NULL) {
		if (!token_contains_name_in_list(username, domain,
					lp_servicename(talloc_tos(), snum),
					token,
					lp_usb_users(snum))) {
			DEBUG(0, ("User %s not in 'usb users'\n",
						username));
			return False;
		}
	} else {
		DEBUG(0, ("User %s<,'usb users' is NULL\n", username));
	}

	DEBUG(10, ("user_ok_token_usb: share %s is ok for unix user %s\n",
