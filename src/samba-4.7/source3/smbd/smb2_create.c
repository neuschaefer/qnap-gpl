...
#ifdef QNAPNAS
			if (in_create_disposition == FILE_OVERWRITE_IF || in_create_disposition == FILE_CREATE) {
				if (lp_conn_log() && (in_create_options & FILE_DIRECTORY_FILE)) {
					write_connlog_mkdir(smb1req->conn, fname);
				}
			}
#endif /* QNAPNAS */
