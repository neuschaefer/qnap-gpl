...
#ifdef QNAPNAS
	/* QNAPNAS_TIMEMACHINE */
	uint16_t in_reserved1 = SVAL(inbody, 0x02);
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
	/* QNAPNAS_TIMEMACHINE */
	if (req->do_syncfs == true) {
		int ret = syscall(__NR_syncfs, req->fsp->fh->fd);
		if (ret != 0) {
			DEBUG(0,(" [QNAPNAS] syncfs() fail ,errno [%d]\n",errno));
		}
	}
#endif /* QNAPNAS */
