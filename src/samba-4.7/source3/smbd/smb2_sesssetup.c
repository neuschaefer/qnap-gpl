...
#ifdef QNAPNAS
	in_capabilities = IVAL(inbody, 0x04);
#endif
...
#ifdef QNAPNAS
	if (((lp_server_max_protocol() >= PROTOCOL_SMB3_00? lp_smb_encrypt(-1) : SMB_SIGNING_OFF )>= SMB_SIGNING_DESIRED) &&
#else
...
#ifdef QNAPNAS
	if(in_capabilities & SMB2_CAP_QNAP)
		set_remote_extended_arch(ERA_QNAP);	
#endif
#ifdef QNAPNAS
	if ( (lp_server_max_protocol() >= PROTOCOL_SMB3_00? lp_smb_encrypt(-1) : SMB_SIGNING_OFF ) == SMB_SIGNING_REQUIRED) {
#else
...
#ifdef QNAPNAS
        /*BUG#121326*/
        if(strlen(session_info->unix_info->sanitized_username)<=0)
                *out_session_flags &= ~SMB2_SESSION_FLAG_IS_GUEST;
#endif
...
#ifdef QNAPNAS
	state->in_capabilities = in_capabilities;
#endif
