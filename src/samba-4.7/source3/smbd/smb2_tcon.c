...
#ifdef QNAPNAS
	if (( (lp_server_max_protocol() >= PROTOCOL_SMB3_00? lp_smb_encrypt(snum) : SMB_SIGNING_OFF ) >= SMB_SIGNING_DESIRED )&&
#else
...
#ifdef QNAPNAS
	if ((   (lp_server_max_protocol() >= PROTOCOL_SMB3_00? lp_smb_encrypt(snum) : SMB_SIGNING_OFF )== SMB_SIGNING_REQUIRED) ){
#else
...
#ifdef QNAPNAS
	if ((get_remote_arch() == RA_OSX) && (lp_parm_bool(-1, "catia", "Qmappings", true))
		&& (NT_STATUS_EQUAL(status, NT_STATUS_BAD_NETWORK_NAME))){

		uint8_t *result = NULL;
		char *nfd_filename = NULL;
		size_t len = 0;
		result = u8_normalize(UNINORM_NFD, in_path, strlen(in_path)+1, NULL, &len);
		if (result != NULL) {
			nfd_filename = talloc_strdup(mem_ctx,(char *)result);
			state->in_path = nfd_filename;
			TALLOC_FREE(in_path);
			free(result);
			status = smbd_smb2_tree_connect(smb2req,
							state->in_path,
