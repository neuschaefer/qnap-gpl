...
#if defined(QNAPNAS) && defined(QTS_PLATFORM_MS) && defined(CONFIG_COMCERTO_SMB_SPLICE)
	// MINDSPEED
	// Splice will not use the pointer but the data length is valid
	// could give pipe instead ?? req->sconn->fdpipe[0]
#endif /* QNAPNAS && QTS_PLATFORM_MS && CONFIG_COMCERTO_SMB_SPLICE */
					       ...
					       uint32_t in_flags)
{
	...
	if (lp_use_interfaces_policy()) {
		set_cpu_affinity_interfaces_policy();
	}
#endif /* QNAPNAS */
	...
	if (smb2req->splice == true) {
		...
		nwritten = SMB_VFS_RECVFILE(xconn->transport.sock, fsp, in_offset, in_data.length);
	} else
#endif /* QNAPNAS && QTS_PLATFORM_MS && CONFIG_COMCERTO_SMB_SPLICE */
	/*
	...
	if (lp_conn_log() && in_offset == 0 && !is_ntfs_stream_smb_fname(fsp->fsp_name)) {
		write_connlog_write(fsp->conn, fsp_str_dbg(fsp));
	}
#endif /* QNAPNAS */
