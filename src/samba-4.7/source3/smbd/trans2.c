...
#ifdef QNAPNAS
static int get_volume_serial_number(int snum){
	
	char createtime[128]={0};
	char *chp=NULL,*pp=NULL;
	int ret=-1,YY=0,mm=0,dd=0,HH=0,MM=0,SS=0,nn=0;

	memset(createtime,0,sizeof(createtime));

	if(lp_create_time(talloc_tos(), snum)){
		snprintf(createtime,sizeof(createtime),"%s",lp_create_time(talloc_tos(), snum));

		if(!strchr(createtime,':') || strlen(createtime)<20)
			return str_checksum(lp_servicename(talloc_tos(), snum)) ^ (str_checksum(get_local_machine_name())<<16);
	
		pp=createtime;
		chp=strchr(createtime,':');
		*chp='\0';
		YY=atoi(pp);
		pp=chp+1;
		...
		*chp='\0';
        ...
        ret=(((HH<<8)|MM)+(YY))^((((mm<<8)|dd)+((SS<<8)|nn))<<16);
        return ret;
}
#endif
...
cBytesSector=%u, cUnitTotal=%u, cUnitAvail=%d\n", (unsigned int)st.st_ex_dev, (unsigned int)bsize, (unsigned int)sectors_per_unit,
		...
		case SMB_INFO_VOLUME:
			 ...
			 */
#ifdef QNAPNAS
			SIVAL(pdata,0,get_volume_serial_number(snum));
#else
		...
		case SMB_FS_VOLUME_INFORMATION:
			 ...
			 */
#ifdef QNAPNAS
			SIVAL(pdata,8,get_volume_serial_number(snum));
#else
...
#ifdef QNAPNAS
			/* 
			 * FIXME: If service name with multibyte string and length > 32bytes,
			 * truncate it to fit volume label length (max 32bytes) might return
			 * EILSEQ (Illegal multibyte sequence).
			 * This regression came from samba#10775 with samba-4.2.0 and later,
			 * https://bugzilla.samba.org/show_bug.cgi?id=10775
			 * WORKAROUND: 
			 * Refer to samba-4.0/4.1 never return NT_STATUS_ILLEGAL_CHARACTER,
			 * hence replace NT_STATUS_ILLEGAL_CHARACTER with NT_STATUS_OK.
			 */
			if (NT_STATUS_EQUAL(status, NT_STATUS_ILLEGAL_CHARACTER)) {
				status = NT_STATUS_OK;
			}				
...
#ifdef QNAPNAS
				if ( (lp_server_max_protocol() >= PROTOCOL_SMB3_00? lp_smb_encrypt(conn) : SMB_SIGNING_OFF ) == SMB_SIGNING_OFF) {
#else
...
#ifdef QNAPNAS
	if ((get_remote_arch() == RA_OSX) && (ft->create_time.tv_sec == 443779200)) {
		ft->create_time = ft->ctime;
		DEBUG(10,("smb_set_filetime: %s created time was 443779200,"
			  " and this will cause macOS finder file gray-out. Set it equal to file changed time\n",
			  smb_fname->base_name));
	}
#endif /* QNAPNAS */
...
#ifdef QNAPNAS	
	if (delete_on_close && !can_delete_file_in_directory_stickybit(conn,smb_fname)) {
