...
#ifdef QNAPNAS
	if (is_usb_interface(conn->sconn->locaddr))
	{
		if (!user_ok_token_usb(session_info->unix_info->unix_name,
                           session_info->info->domain_name,
                           session_info->security_token, snum))
                return NT_STATUS_ACCESS_DENIED;

	}else if (!user_ok_token(session_info->unix_info->unix_name,
#else
...
#ifdef QNAPNAS
const struct security_token *get_current_nt_user_token(connection_struct *conn)
{
	if (current_user.nt_user_token) {
		return current_user.nt_user_token;
	}
