...
#ifdef QNAPNAS
static void store_usergroup_dn(char *name, char *dn, int type)
{
	FILE *fp=NULL;
	char *ptr=NULL, dn_t[1024]={0};

	const char **ads_ou_ptr = lp_ads_ou();
	if(ads_ou_ptr==NULL)
		return;

	char file_path[512]={0};
	if(type==1)
		snprintf(file_path,sizeof(file_path),"%s/.domain_dn_g",lp_default_volume_path(talloc_tos()));
	else
		snprintf(file_path,sizeof(file_path),"%s/.domain_dn_u",lp_default_volume_path(talloc_tos()));

	fp=fopen(file_path,"a");
	ptr=strcasestr(dn,",DC=");
	if(ptr)
		*ptr='\0';
	if((ptr=strcasestr(dn,",OU="))!=NULL)
		snprintf(dn_t,sizeof(dn_t),"%s",ptr+1);
	else
		snprintf(dn_t,sizeof(dn_t),"%s",dn);
...
#ifdef QNAPNAS
	const char **ptr = lp_ads_ou();
#endif
	if (!field) { /* must be end of record */
		if (disp_fields[0]) {
#ifdef QNAPNAS
			/* Bug#76865, net ads user command filter out dollar sign in
			 * user account only if the dollar sign at the end of account string.
			 * */
			if (disp_fields[0][strlen_m(disp_fields[0]) - 1] != '$') {
#else
		...
		if(ptr!=NULL){
			uint32_t atype=0;
			atype=(uint32_t)atol(disp_fields[3]);
			switch(atype & 0xF0000000){
				case ATYPE_NORMAL_ACCOUNT:
					store_usergroup_dn(disp_fields[0], disp_fields[2],0);
					break;
				case ATYPE_SECURITY_GLOBAL_GROUP:
					store_usergroup_dn(disp_fields[0], disp_fields[2],1);
					break;
				default:
					break;
			}
...
#ifdef QNAPNAS
	if(ptr!=NULL){
		if (strcasecmp_m(field, "distinguishedName") == 0)
			disp_fields[2] = SMB_STRDUP((char *) values[0]);
	
		if (strcasecmp_m(field, "sAMAccountType") == 0)
	                disp_fields[3] = SMB_STRDUP((char *) values[0]);
	}
#endif
...
#ifdef QNAPNAS
static int ads_ou_include_check(char *ou){
	
	char ou_tmp[1024]={0};
        const char **ptr = lp_ads_ou();

    if(!lp_ads_ou_scope_subtree())
        return 0;


	snprintf(ou_tmp,sizeof(ou_tmp),"%s,",ou);
        while(*ptr){
                char *ptr_cpy=NULL;
                char mypath[1024]={0};
                ptr_cpy = SMB_STRDUP(*ptr);
                if (ptr_cpy) {
                        snprintf(mypath,sizeof(mypath),",%s,",ptr_cpy);
                        free(ptr_cpy);
                        if(strcasestr(ou_tmp,mypath)){
                                return 1;
                        }
                }
                ptr++;
...
int net_ads_user(struct net_context *c, int argc, const char **argv)
{
	...
	ADS_STATUS rc;
#ifdef QNAPNAS
	const char *shortattrs[] = {"sAMAccountName", "distinguishedName", "sAMAccountType", NULL};
	const char *longattrs[] = {"sAMAccountName", "description", "distinguishedName", "sAMAccountType", NULL};
	char *disp_fields[4] = {NULL, NULL, NULL, NULL};
#else
	...
	if (argc == 0) {
		...
		if(ptr==NULL){
			if(c->opt_bind_path)
				rc = ads_do_search_all_fn(ads, c->opt_bind_path,
							LDAP_SCOPE_SUBTREE,
							"(objectCategory=user)",
			...
			while (*ptr) {
				...
				char mypath[512]={0};
				ptr_cpy = SMB_STRDUP(*ptr);
				if (ptr_cpy && (strlen(ptr_cpy)>0)) {
					if(ads_ou_include_check(ptr_cpy)){
						free(ptr_cpy);
						ptr++;
						continue;
					}
					snprintf(mypath,sizeof(mypath),"%s,%s",ptr_cpy,ads->config.bind_path);
					free(ptr_cpy);
					rc = ads_do_search_all_fn(ads, mypath,
							lp_ads_ou_scope_subtree()?LDAP_SCOPE_SUBTREE:LDAP_SCOPE_ONELEVEL,
							"(objectCategory=user)",
							c->opt_long_list_entries ? longattrs :
							shortattrs, usergrp_display,
							disp_fields);
...
static int net_ads_search(struct net_context *c, int argc, const char **argv)
{
	...
	if(ptr==NULL){
		if(c->opt_bind_path)
			rc = ads_do_search_retry(ads, c->opt_bind_path,
							LDAP_SCOPE_SUBTREE,
							ldap_exp, attrs, &res);
		else
		...
		while (*ptr) {
			...
			if (ptr_cpy && (strlen(ptr_cpy)>0)) {
				...
				rc = ads_do_search_retry(ads, mypath,
                        lp_ads_ou_scope_subtree()?LDAP_SCOPE_SUBTREE:LDAP_SCOPE_ONELEVEL,
						ldap_exp, attrs, &res);

			        if (ADS_ERR_OK(rc)) {
                        reply+=ads_count_replies(ads, res);
...
#if defined(QNAPNAS) && defined(QTS_REDUNDANT_DC)
static int net_ads_ping(
	...
	const char **argv)
{
	...
	if (!ads_cldap_netlogon_5(
		talloc_tos(),
		&ss,
		NULL,
		&reply))
	{
		printf("%s ping failure.\n", server);
		return -1;
	}
...
#endif	/* QNAPNAS && QTS_REDUNDANT_DC */
