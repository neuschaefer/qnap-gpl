				...
				void *private_data)
{
	...
	const char *signing = "-";
#ifdef QNAPNAS
	char ip_family[32] = "unknown";
	char ip_address[32] = "unknown";
	char port[32] = "unknown";

	if (strstr(crec->addr, "ipv4:")) {
		/* crec->addr contains a string like: 'ipv4:127.0.0.1:48163'. */
		sscanf(crec->addr, "%[^:]%*1[:]%[^:]%*1[:]%s",
		       ip_family, ip_address, port);
	} else if (strstr(crec->addr, "ipv6:")) {
		char *check = NULL;
		sscanf(crec->addr, "%[^:]%*1[:]%s", ip_family, ip_address);
		if (strcmp(ip_address, "unknown")) {
			if ((check=strrchr(ip_address,':')) != NULL) {
				*check='\0';
				snprintf(port, sizeof(port), "%s", (check+1));
			}
		}
	}
#endif /* QNAPNAS */
			      ...
			      void *private_data)
{
	...
	if (strstr(session->ip_addr_str, "ipv4:")) {
		/* session->ip_addr_str & session->hostname contains a string like: 
		 * 'ipv4:127.0.0.1:48163'. */
		sscanf(session->ip_addr_str, "%[^:]%*1[:]%[^:]%*1[:]%s",
		       ip_family, ip_address, port);
	} else if (strstr(session->ip_addr_str, "ipv6:")) {
		char *check = NULL;
		sscanf(session->ip_addr_str, "%[^:]%*1[:]%s", ip_family, ip_address);
		if (strcmp(ip_address, "unknown")) {
			if ((check=strrchr(ip_address,':')) != NULL) {
				*check = '\0';
				snprintf(port,sizeof(port), "%s", (check+1));
...
#ifdef QNAPNAS
	d_printf("%5s   %-12s  %-12s  %-12s (%s) %-17s %-20s %-21s\n",
