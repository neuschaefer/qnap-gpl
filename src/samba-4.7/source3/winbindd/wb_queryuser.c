...
#ifdef QNAPNAS
uint32_t query_user_info(struct tevent_req *req, struct wbint_userinfo *info)
{
        ADS_STRUCT *ads = NULL;
        const char *attrs[] = { "primaryGroupID", NULL };
	...
	uint32_t ret=0;
        ...
        DEBUG(3,("ads: query_user_info\n"));

	domain = find_domain_from_sid(&info->user_sid);
        if (domain == NULL) {
        ...
        if (asprintf(&searchstring, "(ObjectSid=%s)", sid_string_dbg(&info->user_sid)) == -1) {
                goto done;
        }
        ...
        if (!ADS_ERR_OK(rc)) {
		DEBUG(10,("ads_search: %s\n", ads_errstr(rc)));
                goto done;
        }

        if (!ads_pull_uint32(ads, res, "primaryGroupID", &group_rid)) {
		DEBUG(10,("ads_pull_uint32 failed\n"));
                goto done;
        }
		ret=group_rid;
...
static void wb_queryuser_got_uid(struct tevent_req *subreq)
{
    ...
    if(lp_server_role() == ROLE_ACTIVE_DIRECTORY_DC)
	    grid=query_user_info(req, info);
	if(grid)
		sid_append_rid(&info->group_sid, grid);
	else
#endif
