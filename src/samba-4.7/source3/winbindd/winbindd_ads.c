...
*/
#ifdef QNAPNAS
ADS_STRUCT *ads_cached_connection(struct winbindd_domain *domain)
#else
...
int ou_filter_check(char *auth_name, struct winbindd_domain *domain){
        ...
        if(ptr!=NULL){
			...
			return 1;
                ...
                if((userp=strchr(auth_name,*lp_winbind_separator()))!=NULL)
                        snprintf(username,sizeof(username),"%s",(userp+1));
                else
                        snprintf(username,sizeof(username),"%s", auth_name);
                if((user_dn=ads_get_samaccountname_ou_string(ads, username))!=NULL){
                        while (*ptr) {
                                char *ptr_cpy=NULL;
                                ptr_cpy = SMB_STRDUP(*ptr);
                                if (ptr_cpy){
					char *checkp=NULL;
                                        if((checkp=strcasestr(user_dn,ptr_cpy))!=NULL){
...
int ads_ou_include_check(char *ou){
			       ...
			       uint32_t **prids)
{
	...
	if(ptr==NULL){
		...
		unlink(file_path);
		char bpath[512]={0};
		snprintf(bpath,sizeof(bpath),"%s",ads->config.bind_path);
		while (*ptr) {
			char *ptr_cpy=NULL;
			char mypath[1024]={0};
			ptr_cpy = SMB_STRDUP(*ptr);
			if (ptr_cpy) {
				...
				snprintf(mypath,sizeof(mypath),"%s,%s",ptr_cpy,bpath);
				free(ptr_cpy);
				...
				rc = ads_do_search_retry_q(&ads, mypath, lp_ads_ou_scope_subtree()?LDAP_SCOPE_SUBTREE:LDAP_SCOPE_ONELEVEL,"(objectCategory=user)", attrs, &res);
ptr_null:
				if (!ADS_ERR_OK(rc)) {
				...
				int count_t=0;
				if(rids){
					count_t=talloc_array_length(rids);
					rids= talloc_realloc(mem_ctx, rids, uint32_t, count_t + count);
					count=count_t;
				}else{
				...
				struct wb_acct_info **info)
{
	...
	const char *attrs[] = {"userPrincipalName", "sAMAccountName",
			       "name", "objectSid", "distinguishedName", NULL};
#else
	const char *attrs[] = {"userPrincipalName", "sAMAccountName",
			       "name", "objectSid", NULL};
#endif
	...
	if(ptr==NULL){
		...
		while (*ptr) {
			...
			if (ptr_cpy) {
				...
				rc = ads_do_search_retry_q(&ads, mypath, lp_ads_ou_scope_subtree()?LDAP_SCOPE_SUBTREE:LDAP_SCOPE_ONELEVEL, filter, attrs, &res);
ptr_null:
				...
				if (count == 0) {
					...
					goto out;
	                        ...
	                        int count_t=0;
	                        if(*info){
	                                count_t=talloc_array_length(*info);
	                                *info= talloc_realloc(mem_ctx, *info, struct wb_acct_info, count_t + count);
	                                i=count_t;
	                        }else{
				...
				for (msg = ads_first_entry(ads, res); msg; msg = ads_next_entry(ads, msg)) {
					...
					(*info)[i].rid = rid;
	                                ...
	                                char *dn=NULL;
	                                if((dn=ads_pull_string(ads, mem_ctx, msg, "distinguishedName"))!=NULL){
	                                        store_usergroup_dn(name, dn, 1);
	                                        talloc_free(dn);
	                                }
