...
#ifdef QNAPNAS
#define RDDC_FAILOVER_LOG	"/sbin/log_tool -a \"[Security mode] " \
				"Domain controller %s disconnected, " \
				"failover to %s.\" -t 1"

static int host_in_password_server(const char *dc, const char *entry)
{
	char entry_host[32];
	size_t entry_host_len = strchr_m(entry, '.') - entry + 1;

	if (entry_host_len >= sizeof(entry_host))
		return -1;

	snprintf(entry_host, entry_host_len, "%s", entry);

	if (strcasecmp_m(dc, entry_host) == 0)
		return 0;

	return -1;
}

static int dc_in_password_server_list(const char *dc)
{
	int ret = -1;
	char password_server[1024], *p, *entry;

	snprintf(password_server,
		 sizeof(password_server),
		 "%s",
		 lp_password_server());
	...
	while (p) {
		...
		if (strcasecmp_m(dc, entry) == 0 ||
		    (strchr_m(dc, '.') == NULL && host_in_password_server(dc, entry) == 0))
		{
			ret = 0;
			break;
		}
...
#ifdef QNAPNAS
	/* Bug#82138, show warning event on WEBUI and system log
	 * when domain controller failover. domain->dcname is
	 * NULL means samba was just start, no previous domain
	 * controller.
	 */
	TALLOC_CTX *mem_ctx;
	char *lc_olddc = NULL, *lc_newdc = NULL, *cmd = NULL;

	if ((mem_ctx = talloc_init("init_dc_connection_network")) == NULL) {
		return NT_STATUS_NO_MEMORY;
	}
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
	if (domain->dcname != NULL) {
		lc_olddc = (char *)talloc_memdup(mem_ctx,
					domain->dcname,
					strlen(domain->dcname) + 1);
	}
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
		if (domain->dcname != NULL) {
			lc_newdc = (char *)talloc_memdup(mem_ctx,
					domain->dcname,
					strlen(domain->dcname) + 1);
		}

		if (lc_olddc != NULL && lc_newdc != NULL) {
			(void)strlower_m(lc_olddc);
			(void)strlower_m(lc_newdc);

			if (strcmp(lc_olddc, lc_newdc) != 0 &&
			    lp_security() == SEC_ADS &&
			    dc_in_password_server_list(lc_olddc) == 0 &&
			    dc_in_password_server_list(lc_newdc) == 0)
			{
				asprintf(&cmd, RDDC_FAILOVER_LOG,
					lc_olddc,
					lc_newdc);
				system(cmd);
				SAFE_FREE(cmd);
