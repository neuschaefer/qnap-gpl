...
#ifdef QNAPNAS
static WERROR qnap_get_recs(struct dnsp_DnssrvRpcRecord **recs,
			    uint16_t *rec_count,
			    const char *name,
			    ...
			    TALLOC_CTX *mem_ctx)
{
	...
	werror = dns_name2dn(dns, mem_ctx, name, &dn);
...
static WERROR qnap_get_type_a_ip(struct dnsp_DnssrvRpcRecord *recs,
				 uint16_t rec_count,
				 struct dns_res_rec *addi,
				 uint16_t ai)
{
	unsigned int ri = 0;

	for (ri = 0; ri < rec_count; ri++) {
		if (recs[ri].wType == DNS_TYPE_A) {
			addi[ai].rdata.ipv4_record = talloc_strdup(
				 addi, recs[ri].data.ipv4);
			W_ERROR_HAVE_NO_MEMORY(addi[ai].rdata.ipv4_record);
			break;
		}
	}
...
static WERROR qnap_add_response_rr(TALLOC_CTX *mem_ctx,
				   const char *name,
				   ...
				   struct dns_server *dns, const char *forwarder)
{
	...
	struct dns_res_rec *addi = *additional;
	uint16_t ai = talloc_array_length(addi);
	...
	/*
	 * "addi" is always non-NULL and thus its own talloc context
	 */
	addi = talloc_realloc(addi, addi, struct dns_res_rec, ai+1);
	if (addi == NULL) {
	...
	werror = qnap_get_type_a_ip(recs, rec_count, addi, ai);
	if (!W_ERROR_IS_OK(werror)) {
		return werror;
	}

	addi[ai].name = talloc_strdup(
		 addi, rec->data.srv.nameTarget);
	W_ERROR_HAVE_NO_MEMORY(addi[ai].name);
	addi[ai].rr_type = DNS_QTYPE_A;
	addi[ai].rr_class = DNS_QCLASS_IN;
	addi[ai].ttl = rec->dwTtlSeconds;
	addi[ai].length = UINT16_MAX;
...
static WERROR qnap_add_response_rr_for_soa(
	const char *name,
	const struct dnsp_DnssrvRpcRecord *rec,
	struct dns_res_rec **answers )
{
	...
	if (ai == UINT16_MAX) return WERR_BUFFER_OVERFLOW;
	else if (ai == 0) {

		// append the additional information only if it is empty

		ans = talloc_realloc(ans, ans, struct dns_res_rec, ai+1);
		if (ans == NULL) return WERR_NOT_ENOUGH_MEMORY;
...
#ifdef QNAPNAS
	if (rec_count>0) snprintf(soa_mname_buffer,512,recs[0].data.soa.mname);
	else soa_mname_buffer[0] = 0;
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
		else if (recs[ri].wType == DNS_TYPE_A) {
			werror = qnap_add_response_rr_for_soa(soa_mname_buffer,&recs[ri], &addi);
			if (!W_ERROR_IS_OK(werror)) {
				return werror;
			}
		}
...
#ifdef QNAPNAS
	struct dns_res_rec **answers, struct dns_res_rec **nsrecs, struct dns_res_rec **additional);
#else
...
#ifdef QNAPNAS
		if (question->question_type == DNS_QTYPE_SRV) {
			werr = qnap_add_response_rr(mem_ctx,
						    question->name,
						    rec, state->additional,
						    ev, dns, forwarder);
			if (tevent_req_werror(req, werr)) {
				return tevent_req_post(req, ev);
			}
...
#ifdef QNAPNAS
			state->answers, state->nsrecs, state->additional);
#else
...
#ifdef QNAPNAS
		state->additional = talloc_array(state, struct dns_res_rec, 0);
		if (tevent_req_nomem(state->additional, req)) {
			return tevent_req_post(req, ev);
		}
#endif /* QNAPNAS */
		subreq = handle_authoritative_send(
...
#ifdef QNAPNAS
			&in->questions[0], &state->answers, &state->nsrecs, &state->additional);
#else
