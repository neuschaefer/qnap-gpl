...
#ifdef QNAPNAS
	len = _tdgram_recvfrom_recv(subreq, &sys_errno,
				   call, &buf, &call->src, &indev);
#else
...
#ifdef QNAPNAS
	subreq = _tdgram_sendto_queue_send(call,
					   dns->task->event_ctx,
					   sock->dgram,
					   sock->send_queue,
					   call->out.data,
...
#ifdef QNAPNAS
	if (lpcfg_dns_interfaces(task->lp_ctx)){
		load_dns_interface_list(task, task->lp_ctx, &ifaces);
#else
