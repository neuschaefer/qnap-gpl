...
#ifdef QNAPNAS
		/* Bug#70631 (KS-Redmine#15808) */
		ret = gendb_search(d_state->sam_ctx,
				   mem_ctx, d_state->domain_dn, &msgs, attrs,
				   "(&(objectSid=%s)(objectClass=group)"
				   "(|(groupType=%d)(groupType=%d)(groupType=%d)))",
				   ldap_encode_ndr_dom_sid(mem_ctx, sid),
				   GTYPE_SECURITY_UNIVERSAL_GROUP,
				   GTYPE_SECURITY_GLOBAL_GROUP,
				   GTYPE_SECURITY_DOMAIN_LOCAL_GROUP);
...
#ifdef QNAPNAS
/* Bug#72045 (KS-Redmind#16035) */
bool check_sids_contains_sid(const struct dom_sid *sids,
...
NTSTATUS expand_nested_groups(struct samr_domain_state *d_state, struct samr_account_state *a_state,
					TALLOC_CTX *mem_ctx, struct dom_sid *g_sid,
					struct dom_sid **res_sids, unsigned int *num_res_sids)
{
	const char * const attrs[3] = { "objectSid", "memberOf", NULL };
	int count;
	struct ldb_message **result;
	char membersidstr[DOM_SID_STR_BUFLEN] = {0};

	if (*res_sids == NULL) {
		*num_res_sids = 0;
	}

	dom_sid_string_buf(g_sid, membersidstr, sizeof(membersidstr)),

	count = samdb_search_domain(a_state->sam_ctx, mem_ctx,
				d_state->domain_dn, &result,
				...
				GTYPE_SECURITY_DOMAIN_LOCAL_GROUP);
	if (count > 0) {
		struct dom_sid *nested_sid;

		nested_sid = samdb_result_dom_sid(mem_ctx, result[0], "objectSid");
		if (nested_sid == NULL) {
			return NT_STATUS_INTERNAL_DB_CORRUPTION;
		}

		bool already_there = check_sids_contains_sid(*res_sids,
					  *num_res_sids, nested_sid);
		if (already_there) {
			DEBUG(3, ("SID{%s} already_there\n", ldap_encode_ndr_dom_sid(mem_ctx,g_sid)));
			return NT_STATUS_OK;
		}

		*res_sids = talloc_realloc(mem_ctx, *res_sids, struct dom_sid, *num_res_sids + 1);
		if (*res_sids == NULL) {
			return NT_STATUS_NO_MEMORY;
		}

		sid_copy(&(*res_sids)[*num_res_sids], nested_sid);
		++(*num_res_sids);

		struct ldb_message_element *el = ldb_msg_find_element(result[0], "memberOf");
		if (el != NULL) {
			int i;
			for (i = 0; i < el->num_values; i++) {
				expand_nested_groups(d_state, a_state, mem_ctx, nested_sid, res_sids, num_res_sids);
			}
		}
	}
		       ...
		       struct samr_GetGroupsForUser *r)
{
	...
	for (i = 0; i < count; i++) {
		struct dom_sid *g_sid = samdb_result_dom_sid(mem_ctx, res_memberof->msgs[i], "objectSid");
		if (g_sid == NULL) {
			return NT_STATUS_INTERNAL_DB_CORRUPTION;
		}

		sids = talloc_realloc(mem_ctx, sids, struct dom_sid, num_sids + 1);
		if (sids == NULL) {
			return NT_STATUS_NO_MEMORY;
		}

		sid_copy(&sids[num_sids], g_sid);
		++num_sids;

		memberof_el = ldb_msg_find_element(res_memberof->msgs[i], "memberOf");
		if (memberof_el != NULL) {
			int idx;
			for (idx = 0; idx < memberof_el->num_values; idx++) {
				expand_nested_groups(d_state, a_state, mem_ctx, g_sid, &sids, &num_sids);
			}
		}
	}
