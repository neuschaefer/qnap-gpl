...
static char * version_str = "0.26 20040109";
...
static int do_simple_inq(int sg_fd, int noisy, 
	                 struct simple_inquiry_t * inq_data)
{
    int res;
...
static void show_page_name(int page_no, struct simple_inquiry_t * inq_dat)
{
    ...
    printf("    0x%.2x [device_type: 0x%.2x\n", page_no, 
	   inq_dat->peripheral_type);
}
...
int main(int argc, char * argv[])
{
    ...
    if (do_simple_inq(sg_fd, 1, &inq_out))
    ...
    if (1 == do_temp)
        return fetchTemperature(sg_fd, rsp_buff, MX_ALLOC_LEN);
