 ...
 *  any later version.
 * 
 * $Id: sg_test_rwbuf.c,v 1.1.1.1 2008/01/17 10:52:02 ricky Exp $
 *
...
void usage ()
{
        ...
        printf ("(c) Douglas Gilbert, Kurt Garloff, 2000-2003, GNU GPL\n");
        exit (1);
}
...
int main (int argc, char * argv[])
{
        ...
        if ((ioctl(sg_fd, SG_GET_VERSION_NUM, &k) < 0) || (k < 30000)) {
                fprintf(stderr, ME "%s doesn't seem to be a version 3 sg "
                        "device\n", file_name);
                close(sg_fd);
                return 1;
        }
