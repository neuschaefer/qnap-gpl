...
static const char * sginfo_version_str = "Sginfo version 2.02 [20031215]";
				    ...
				    const char * prefix)
{
   ...
   /* variable length mode page, need to know its response length */
    ...
    for (k = 0, p = pagestart + 8; k < num_phys; ++k, p += 48) {
        ...
        hexdatafield(p + 24, 8, "Attached Phy identifier");
...
static int open_sg_io_dev(char * devname)
{
    ...
    if (fd >= 0) {
        if ((ioctl(fd, SG_GET_VERSION_NUM, &v) < 0) || (v < 30000)) {
            fprintf(stderr, "requires lk 2.4 or 2.6 (sg driver version 3)\n");
            close(fd);
            return -9999;
        }
        close(fd);
