...
void Statement::bind(const int aIndex, const sqlite_int64& aValue) // throw(SQLite::Exception)
{
...
// Bind a 64bits int value to a parameter "?NNN", ":VVV", "@VVV" or "$VVV" in the SQL prepared statement
void Statement::bind(const char* apName, const sqlite_int64& aValue) // throw(SQLite::Exception)
{
    int index = sqlite3_bind_parameter_index(mStmtPtr, apName);
