...
public:
     ...
     */
    void bind(const int aIndex, const sqlite_int64& aValue)  ; // throw(SQLite::Exception);
    /**
     ...
     */
    void bind(const char* apName, const sqlite_int64&  aValue)  ; // throw(SQLite::Exception);
    /**
    ...
    /// @brief Return the column type of selite: one of SQLITE_INTEGER, SQLITE_FLOAT, SQLITE_TEXT, SQLITE_BLOB or SQLITE_NULL.
    inline int getColumnType(const int aIndex) const
    {
        return sqlite3_column_type(mStmtPtr, aIndex);
    }
    /// @brief Return the column name of result
    inline const char* getColumnName(const int aIndex) const
    {
