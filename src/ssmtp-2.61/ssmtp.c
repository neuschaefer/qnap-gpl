/*
 ...
 $Id: ssmtp.c,v 1.10 2014/05/15 01:40:12 samsonchan Exp $
...
//added by Jeff Huang on 2013/4/3 supporting -C
#define MAX_CFG_FILE_NAME_SIZE		1024
static char config_file[MAX_CFG_FILE_NAME_SIZE];
...
bool_t read_config()
{
    ...
    char *tmp_ebuf = NULL;
    char *tmp_dbuf = NULL;
    tmp_ebuf = (char *)calloc(1, 512);
    tmp_dbuf = (char *)calloc(1, 512);

	if((fp = fopen(config_file[0]==0?CONFIGURATION_FILE:config_file, "r")) == NULL) {
		return(False);
	}

    //log_event(LOG_INFO, "%d, First auth_pass is (%s)!!!\n", __LINE__, auth_pass);
	while(fgets(buf, sizeof(buf), fp)) {
		/* Make comments invisible */
		if((p = strchr(buf, '#'))) {
			*p = (char)NULL;
		...
		if(((p = strtok(buf, "= \t\n")) != (char *)NULL)
            ...
            //log_event(LOG_INFO, "%d, p is (%s), q is (%s)!!!\n", __LINE__, p, q);
    ...
    // QNAP fix ssmtp config file contains plain text
    // we add encode/decode mechanism for password
    // Samson Modified 2014.02.25 start
    
	/*  because encrypted data include "=" will lead to parse fault
	 *  so we don't use original parse mechanism(due to "=" is string token)
	 *
	 *  if(strcasecmp(p, "enAuthPass") == 0 && !auth_pass){
	 *
	 *	    if((tmp_ebuf = strdup(q)) == (char *)NULL) {
	 *		    die("parse_config() -- strdup() failed");
	 *	    }
	 *  }
     */
    if( 0 > Conf_Get_Field(SSMTP_CONF, "", SSMTP_eAUTHPASS, tmp_ebuf, 512)){ // get SSMTP_eAUTHPASS fail
        if( 0 > Conf_Get_Field(SSMTP_CONF, "", SSMTP_AUTHPASS, tmp_dbuf, 512)){ // get SSMTP_AUTHPASS fail, skip
            if( (char *)NULL == tmp_dbuf){ // fix Bug #44068 hinet account can't send mail
                die("parse_config() -- Get AuthPass failed");
            }
        }else{ // get SSMTP_AUTHPASS success, write encrypted data to SSMTP_eAUTHPASS and remove SSMTP_AUTHPASS
            if(NULL == (tmp_ebuf = Password_Encode(tmp_dbuf, strlen(tmp_dbuf), SMTP_ENC_SALT))){
                log_event(LOG_INFO, "Password Encode Fail\n");
            }
            Conf_Remove_Field(SSMTP_CONF, "", SSMTP_AUTHPASS);
            Conf_Set_Field(SSMTP_CONF, "", SSMTP_eAUTHPASS, tmp_ebuf);
            SAFE_FREE(tmp_dbuf);
		}
        
		if(!(auth_pass=Password_Decode(tmp_ebuf, SMTP_ENC_SALT))){
            log_event(LOG_INFO, "Password Decode Fail\n");
        }
        SAFE_FREE(tmp_ebuf);
        log_event(LOG_INFO, "Set enAuthPass=\"%s\"\n", auth_pass);
        
    }else{ // get SSMTP_eAUTHPASS success
        if(!(auth_pass=Password_Decode(tmp_ebuf, SMTP_ENC_SALT))){
            log_event(LOG_INFO, "Password Decode Fail\n");
        }
        SAFE_FREE(tmp_ebuf);
    ...
    // Samson Modified 2014.02.25 end
	return(True);
}
...
int smtp_open(char *host, int port)
{
	...
	//Hot mail doesn't support DES-CBC3-SHA
	//Limit ciphers to be the same as old OPENSSL (0.9.7/0.9.8)
	// QNAP Charley Modified	2013.08.08
	err = SSL_CTX_set_cipher_list(ctx, "DHE-RSA-AES256-SHA:DHE-DSS-AES256-SHA:AES256-SHA:EDH-RSA-DES-CBC3-SHA:EDH-DSS-DES-CBC3-SHA:DES-CBC3-SHA:DES-CBC3-MD5:DHE-RSA-AES128-SHA:DHE-DSS-AES128-SHA:AES128-SHA:IDEA-CBC-SHA:IDEA-CBC-MD5:RC2-CBC-MD5:RC4-SHA:RC4-MD5:RC4-MD5:EDH-RSA-DES-CBC-SHA:EDH-DSS-DES-CBC-SHA:DES-CBC-SHA:DES-CBC-MD5:EXP-EDH-RSA-DES-CBC-SHA:EXP-EDH-DSS-DES-CBC-SHA:EXP-DES-CBC-SHA:EXP-RC2-CBC-MD5:EXP-RC2-CBC-MD5:EXP-RC4-MD5:EXP-RC4-MD5");
	if (err <= 0)
		perror("Set cipher list");
	// QNAP Charley Modified	2013.08.08
#endif
...
int ssmtp(char *argv[])
{
	...
	//Modify by Jimmy 2007/11/27
	if(have_from == False) {
		char NASName[128], from_addr[128];
		char *delim = "@", *domain;

		Get_Server_Name(NASName, sizeof(NASName));
		strtok(from, delim);
		domain = strtok(NULL, delim);
		strcpy(from_addr, NASName);
		strcat(from_addr, "@");
		strcat(from_addr, domain);
		smtp_write(sock, "From:\"%s\" <%s>", NASName, from_addr);
...
*/
			...
			case 'C':
				add++;
				strncpy( config_file, argv[i+1], MAX_CFG_FILE_NAME_SIZE);
				goto exit;
