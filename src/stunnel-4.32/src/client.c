...
#ifdef QNAPNAS
//2010-01-29 Richard Chang add send source ip to thttpd
            if(sock_dog){
                if(!strcmp(c->opt->servname, "https")){
                    char remote_env[STRLEN]={0}, *remote_port, local_addr[INET_ADDRSTRLEN + INET6_ADDRSTRLEN]={0};
                    int ret_num;
                    socklen_t  local_addr_len;
                    struct sockaddr_storage  sock_local;
                    safecopy(remote_env, "[");
                    safeconcat(remote_env, c->accepted_address);
                    remote_port = strrchr(remote_env, ':');
                    if(remote_port){ /* strip the port name */
                        //*remote_port++=']';
                         *remote_port++=',';
                        *remote_port='\0';
                    }
                    else
                        safeconcat(remote_env, ",");

                    //2012-02-08 Jeff Chang modified, for service binding, send local IP to thttpd.
                    local_addr_len = sizeof(sock_local);
                    if(-1 == getsockname(0, (struct sockaddr *)&sock_local, &local_addr_len))
                    {
                        local_addr[0] = '0';
                    }
                    else
                    {
                        if(AF_INET == sock_local.ss_family)
                        {
                            struct sockaddr_in *sockin_local = (struct sockaddr_in *)&sock_local;
                            strcpy(local_addr, inet_ntoa(sockin_local->sin_addr));
                        }
                        else if(AF_INET6 == sock_local.ss_family)
                        {
                            struct sockaddr_in6 *sockin6_local = (struct sockaddr_in6 *)&sock_local;
                            inet_ntop(sock_local.ss_family, &sockin6_local->sin6_addr, local_addr, INET6_ADDRSTRLEN);
                        }
                        else
			{
                            local_addr[0] = '0';
                    ...
                    safeconcat(remote_env, local_addr);
                    safeconcat(remote_env, "]");

                    ret_num = writesocket(c->remote_fd.fd, remote_env, strlen(remote_env));
                    switch(ret_num) {
                    case -1: /* error */
                        s_log(LOG_DEBUG, "writesocket -1");
                        break;
                    case 0:
                        s_log(LOG_DEBUG, "No data written to the socket: retrying");
                        break;
                    default:
                        s_log(LOG_DEBUG, "writesocket default, %s = %d", remote_env, ret_num);
                        c->sock_bytes += ret_num;
                    }
                    sock_dog = 0;
                }
            }
