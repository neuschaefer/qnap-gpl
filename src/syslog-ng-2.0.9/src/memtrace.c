 ...
 *
 * $Id: memtrace.c,v 1.1.1.1 2009/10/27 08:13:01 ken Exp $
 *
 * Author  : Bazsi
 * Auditor :
      ...
      while (cur != -1)
        {
          ...
          z_mem_trace_printf("ptr=%p, size=%d, backtrace=%s\n", entry->ptr, entry->size, z_mem_trace_format_bt(entry->backtrace, backtrace_buf, sizeof(backtrace_buf)));
  ...
  if (user_ptr)
    {
      ...
      if (old_size == -1 || !z_mem_trace_del(user_ptr))
        {
          z_mem_trace_printf("Trying to realloc a non-existing memory block; ptr=%p, size='%d', info='%s'", user_ptr, size, z_mem_trace_format_bt(backtrace, buf, sizeof(buf)));
          assert(0);
        }
    }
  if (TMP_ALLOCATED(raw_ptr))
