...
namespace {
	...
	struct superblock_disk {
		...
		caching::__u8 uuid[16];
		le64 magic;
		le32 version;

		caching::__u8 policy_name[CACHE_POLICY_NAME_SIZE];
		le32 policy_hint_size;

		caching::__u8 metadata_space_map_root[SPACE_MAP_ROOT_SIZE];
...
{
	superblock sb;
	block_manager<>::read_ref r = bm->read_lock(location, __validator::mk_v());
...
{
	block_manager<>::write_ref w = bm->superblock_zero(location, __validator::mk_v());
