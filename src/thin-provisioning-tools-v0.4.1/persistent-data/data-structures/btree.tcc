...
//----------------------------------------------------------------
	...
	btree<Levels, ValueTraits>::destroy()
	{
		...
		while (!s.is_empty()) {
			...
			if (n.get_type() == INTERNAL) {
				// TODO: why prefetch degrades performance?
				//if (!f.current_child_)
	...
	template <unsigned Levels, typename ValueTraits>
	void
	btree<Levels, ValueTraits>::visit_last_leaf(visitor &v) const
	{
		node_location loc;

		walk_last_leaf(v, loc, root_);
		v.visit_complete();
	}

	template <unsigned Levels, typename ValueTraits>
	void
	btree<Levels, ValueTraits>::walk_last_leaf(visitor &v,
						   node_location const &loc,
						   block_address b) const
	{
		try {
			walk_last_leaf_internal(v, loc, b);
	...
	template <unsigned Levels, typename ValueTraits>
	void
	btree<Levels, ValueTraits>::walk_last_leaf_internal(visitor &v,
							    node_location const &loc,
							    block_address b) const
	{
		using namespace btree_detail;
		...
		if (o.get_type() == INTERNAL) {
			if (v.visit_internal(loc, o)) {
				uint32_t last_idx = o.get_nr_entries() - 1;
				typename block_traits::value_type last_value =
						o.value_at(last_idx);

				tm_.prefetch(last_value);

				node_location loc2(loc);

				loc2.inc_depth();
				loc2.key = o.key_at(last_idx);

				walk_last_leaf(v, loc2, last_value);
			}
			...
			if (v.visit_internal_leaf(loc, o)) {
				...
				uint32_t last_idx = o.get_nr_entries() - 1;
				loc2.push_key(o.key_at(last_idx));
				loc2.key = boost::optional<uint64_t>();

				walk_last_leaf(v, loc2, o.value_at(last_idx));
			}
