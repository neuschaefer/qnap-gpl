...
namespace persistent_data {
	...
	namespace sm_disk_detail {
		...
		class index_entry_ref_counter {
		public:
			index_entry_ref_counter(space_map::ptr sm)
				: sm_(sm) {}

			void inc(index_entry ie) {
				// do nothing
				// actually it's impossible to have shared node in bitmap tree.
			}

			// decrease the bitmap's reference count while removing the index_entry
			void dec(index_entry ie) {
				sm_->dec(ie.blocknr_);
			}
		...
		struct index_entry_traits {
			...
			typedef index_entry_ref_counter ref_counter;
		...
		unsigned const MAX_METADATA_BITMAPS = (MD_BLOCK_SIZE == 8192) ? 511 : 255;
		unsigned const MAX_METADATA_BLOCKS = (MD_BLOCK_SIZE == 8192) ?
						     (511 * ((1 << 15) - 64)) : (255 * ((1 << 14) - 64));
