...
superblock_traits::unpack(superblock_disk const &disk, superblock &value)
{
	...
	::memcpy(value.tier0_data_space_map_root_,
		 disk.tier0_data_space_map_root_,
		 sizeof(value.tier0_data_space_map_root_));
	::memcpy(value.tier1_data_space_map_root_,
		 disk.tier1_data_space_map_root_,
		 sizeof(value.tier1_data_space_map_root_));
	::memcpy(value.tier2_data_space_map_root_,
		 disk.tier2_data_space_map_root_,
		 sizeof(value.tier2_data_space_map_root_));
	...
	value.backup_id_ = to_cpu<uint64_t>(disk.backup_id_);
	value.reserve_block_count_ = to_cpu<uint64_t>(disk.reserve_block_count_);
	value.clone_root_ = to_cpu<uint64_t>(disk.clone_root_);

	value.tier_num_ = to_cpu<uint32_t>(disk.tier_num_);
	value.pool_mapping_root_ = to_cpu<uint64_t>(disk.pool_mapping_root_);
	value.tier_block_size_ = to_cpu<uint32_t>(disk.tier_block_size_);
}
...
superblock_traits::pack(superblock const &value, superblock_disk &disk)
{
	...
	::memcpy(disk.tier0_data_space_map_root_,
		 value.tier0_data_space_map_root_,
		 sizeof(disk.tier0_data_space_map_root_));
	::memcpy(disk.tier1_data_space_map_root_,
		 value.tier1_data_space_map_root_,
		 sizeof(disk.tier1_data_space_map_root_));
	::memcpy(disk.tier2_data_space_map_root_,
		 value.tier2_data_space_map_root_,
		 sizeof(disk.tier2_data_space_map_root_));
	...
	disk.backup_id_ = to_disk<le64>(value.backup_id_);
	disk.reserve_block_count_ = to_disk<le64>(value.reserve_block_count_);
	disk.clone_root_ = to_disk<le64>(value.clone_root_);

	disk.tier_num_ = to_disk<le32>(value.tier_num_);
	disk.pool_mapping_root_ = to_disk<le64>(value.pool_mapping_root_);
	disk.tier_block_size_ = to_disk<le32>(value.tier_block_size_);
}
...
namespace {
	...
	struct sb_validator : public bcache::validator {
		...
		virtual void prepare(void *raw, block_address location) const {
			...
			sbd->blocknr_ = to_disk<base::le64, uint64_t>(location);
			crc32c sum(SUPERBLOCK_CSUM_SEED);
			sum.append(&sbd->flags_, MD_BLOCK_SIZE - sizeof(uint32_t));
...
namespace thin_provisioning {
	...
	void
	check_superblock(block_manager<>::ptr bm, block_address location,
			 superblock_detail::damage_visitor &visitor) {
		using namespace superblock_detail;

		try {
			bm->read_lock(location, superblock_validator());
	...
	check_superblock(block_manager<>::ptr bm,
			 superblock_detail::damage_visitor &visitor) {
		return check_superblock(bm, SUPERBLOCK_LOCATION, visitor);
	}
}
