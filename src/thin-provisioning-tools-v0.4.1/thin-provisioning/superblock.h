...
namespace thin_provisioning {
	namespace superblock_detail {
		...
		unsigned const SPACE_MAP_ROOT_SIZE = 40;
		...
		struct superblock_disk {
			...
			__u8 tier0_data_space_map_root_[SPACE_MAP_ROOT_SIZE];
			__u8 tier1_data_space_map_root_[SPACE_MAP_ROOT_SIZE];
			__u8 padding1[8];
			__u8 metadata_space_map_root_[SPACE_MAP_ROOT_SIZE];
			__u8 tier2_data_space_map_root_[SPACE_MAP_ROOT_SIZE];
			__u8 padding2[48];
			...
			le64 reserve_block_count_;
			le64 clone_root_;

			le32 tier_num_;
			le64 pool_mapping_root_;
			le32 tier_block_size_;
		...
		struct superblock {
			...
			unsigned char tier0_data_space_map_root_[SPACE_MAP_ROOT_SIZE];
			unsigned char tier1_data_space_map_root_[SPACE_MAP_ROOT_SIZE];
			unsigned char padding1[8];
			unsigned char metadata_space_map_root_[SPACE_MAP_ROOT_SIZE];
			unsigned char tier2_data_space_map_root_[SPACE_MAP_ROOT_SIZE];
			unsigned char padding2[48];
			...
			uint64_t reserve_block_count_;
			uint64_t clone_root_;

			uint32_t tier_num_;
			uint64_t pool_mapping_root_;
			uint32_t tier_block_size_;
