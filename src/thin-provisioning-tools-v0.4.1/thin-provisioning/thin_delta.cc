...
namespace local {
	...
	public:
		...
		uint64_t parse_int(string const &str) {
			try {
				return boost::lexical_cast<uint64_t>(str);

			} catch (...) {
				ostringstream out;
				out << "Couldn't parse string: '" << str << "'";
				die(out.str());
			}
	...
	void delta_(application &app, flags const &fs) {
...

		{
			...
			if (fs.snap1) {
				...
				snap1_root = dtree.lookup(k);
			} else if (fs.snap1_root)
				snap1_root = *fs.snap1_root;
			...
			if (fs.snap2) {
				...
				snap2_root = dtree.lookup(k);
			} else if (fs.snap2_root)
				snap2_root = *fs.snap2_root;
...
int thin_delta_main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, shortopts, longopts, NULL)) != -1) {
		...
		case 1:
			fs.snap1 = app.parse_int(optarg);
			break;

		case 2:
			fs.snap2 = app.parse_int(optarg);
			break;
		...
		case 5:
			fs.snap1_root = app.parse_int(optarg);
			break;

		case 6:
			fs.snap2_root = app.parse_int(optarg);
			break;
	...
	// FIXME: option snap1 and snap1_root are not compatible
	if (!fs.snap1 && !fs.snap1_root)
		app.die("--snap1 not specified.");

	// FIXME: option snap2 and snap2_root are not compatible
	if (!fs.snap2 && !fs.snap2_root)
