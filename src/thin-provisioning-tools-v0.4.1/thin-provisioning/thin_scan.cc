...
namespace {
	...
	public:
		...
		virtual boost::shared_ptr<block_range> clone() const {
			return boost::shared_ptr<block_range>(new block_range(*this));
		}
	...
	public:
		...
		virtual boost::shared_ptr<block_range> clone() const {
			return boost::shared_ptr<block_range>(new meta_block_range(*this));
		}
	...
	public:
		...
		virtual boost::shared_ptr<block_range> clone() const {
			return boost::shared_ptr<block_range>(new btree_block_range(*this));
		}
	...
	public:
		...
		// FIXME: replace shared_ptr by unique_ptr
		boost::shared_ptr<block_range> get_range() {
			boost::shared_ptr<block_range> ret;

			while (index_ < scan_end_) {
				block_range const &r = read_block(index_++);

				if (!run_range_->concat(r)) {
					ret = run_range_; // FIXME: use std::move() on unique_ptr
					run_range_ = r.clone();
					break;
				}
			}
	...
	private:
		...
		uint64_t scan_end_;
		uint64_t index_;
		boost::shared_ptr<block_range> run_range_;
	...
	int scan_metadata_(string const &input,
		...
		boost::shared_ptr<block_range> r;
...
int thin_scan_main(int argc, char **argv)
{
	...
	const struct option longopts[] = {
		...
		{ "non-exclusive", no_argument, NULL, 3},
...
base::command thin_provisioning::thin_scan_cmd("thin_scan", thin_scan_main);
