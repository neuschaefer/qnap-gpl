...
int get_telnet__state(int pid, char *user_name,  char *user_state)
{
	char telnet_pid[16] = "0";
	char buf[128] = "";
...
//Richard Chang 20070827 add for onlineuser
#ifdef STATISTICS
int add_telnet_user(char *username, char *user_IP)
{
	USER_INFO user;
	user.pid=getpid();
	get_telnet_ip(user.pid, user_IP);
	strcpy(user.username, username);
	strcpy(user.ip, user_IP);
	strcpy(user.sharename, "---");
	user.app_id=shm_get_app_id("sh");
	user.schedule=SCHEDULE_PID;
	shm_add_user(&user);
	return user.pid;
}
#endif
//Richard Chang 20070906 add log
static void add_loginsucc_log(char* user_name, char *user_IP, int pid)
{
	write_telnet_state(pid, user_name,  "success");
	SendConnToLogEngine(EVENT_TYPE_INFO, user_name, user_IP, 
			"---", CONN_SERV_TELNET, CONN_ACTION_LOGIN_SUCC,  "---");
}

static void add_loginfail_log(char* user_name, char *user_IP, int pid)
{
	USER_INFO user;
	user.pid=getpid();
	pid = user.pid;
	get_telnet_ip(pid, user_IP);
	write_telnet_state(pid, user_name,  "fail");
	SendConnToLogEngine(EVENT_TYPE_WARN, user_name, user_IP, 
		"---", CONN_SERV_TELNET, CONN_ACTION_LOGIN_FAIL,  "---");
}
//end
...
// add by Kent 2003/01/30
#ifdef QNAPNAS
	if (strcasecmp(name, "admin")){
		//Richard Chang 20071105 add log
		add_loginfail_log(name, userIP, pid);
		error_msg_and_die("STOP !!!!!\n");
		failed = 1;
	}
#endif
// end
...
#ifdef QNAPNAS
			if(Is_User_Password_Expiry(name)){
				add_loginfail_log(name, userIP, pid);
				error_msg_and_die("Password Expired !\n");
				failed = 1;
			}else if(Is_User_Account_Expiry(name)){
				add_loginfail_log(name, userIP, pid);
				error_msg_and_die("Account Expired !\n");
				failed = 1;
			}else
#endif
			goto auth_ok;
	  ...
	  auth_ok:
		if (!failed) {
			//Richard Chang 20070827 add for onlineuser
		#ifdef STATISTICS
			pid = add_telnet_user(name, userIP);
		#endif /* STATISTICS */
			//Richard Chang 20070906 add log
			add_loginsucc_log(name, userIP, pid);
			
			readfp = popen(tty,"r");
			if(readfp != NULL){
				fgets(full_tty, 10 ,readfp);
				pclose(readfp);
			}
		...
		if (su_mode) {
			...
			//Richard Chang 20070906 add log
			add_loginfail_log(user_name, userIP, pid);
