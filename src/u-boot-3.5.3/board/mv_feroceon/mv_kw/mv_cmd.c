...
U_BOOT_CMD(SatR, 5, 1, do_satr,
	...
	"SatR read.SSCG		- read SSCG state from S@R devices [0 ~ en]\n"
#ifdef DB_88F6282A
    "SatR read.PEXCLK[0/1]   - read PCI-E [0/1] clock state from S@R devices [0 ~ input]\n"
#else
   ...
   "SatR read.MPP18	    - reserved\n"
	...
	"SatR write.SSCG val	- write SSCG state val to S@R devices [0 ~ en]\n"
#ifdef DB_88F6282A
    "SatR write.PEXCLK[0/1]	- write PCI-E [0/1] clock state from S@R devices [0 ~ input]\n"
#else
...
int uart1_cmd (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	char cmd;
	unsigned char addr;
	int len;
	...
	if(argc == 2){
		...
		printf("Send Cmd : 0x%0X to UART1\n", cmd);
		mvUartPutc(1, cmd);
	}
	else{
		if(argc == 4){ // read
			if (strcmp(argv[1],"r") != 0) {
				printf("No Read Command\n");
				return 0;
			}
			...
			for(i=0;i<len;i++){
				data[i] = simple_strtoul(argv[4+i], NULL, 16);
			}
			// Write =========================
			mvUartPutc(1, 0xF6);
			do{
