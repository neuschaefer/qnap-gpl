...
// QNAP patch : Get Model type
int model_name_number;
char model_name_info[][16] = {
	"TS-419P+",	// 0
	"TS-419U+",	// 1
	"TS-419P2+", "TS-419U2+",
	"TS-419P2",	// 4
	...
	"HS-210", // 32
	"Unknown",	// 33
	"Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", // 34 - 40 
	"Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", // 41 - 48
	"Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", // 49 - 56
	"Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown" // 57 - 63
};

#define PHY33 "PHY=3.3v"
#define PHY25 "PHY=2.5v"
#define PHY18 "PHY=1.8v"
#define PHYERR "PHY=Config Error"
int get_model_type(char *ModelName)
{	// Get MPP34,35,36,38,39,44
	int tmp1,tmp2;
	model_name_number = ((!!mvGppValueGet(1,MV_GPP2))<<5) + ((!!mvGppValueGet(1,MV_GPP3))<<4) +
			    ((!!mvGppValueGet(1,MV_GPP4))<<3) + ((!!mvGppValueGet(1,MV_GPP6))<<2) + 
	      		    ((!!mvGppValueGet(1,MV_GPP7))<<1) + (!!mvGppValueGet(1,MV_GPP12));
//	printf("model_name_number = %d, Model=%s\n",model_name_number, model_name_info[model_name_number]);
//	sprintf(ModelName,"%s", model_name_info[model_name_number]);
	tmp1 = ((MV_REG_READ(0x100e0) & 0x8080) == 0x8080)?1:0;	  // 0=>3.3v 1=>1.8v
	tmp2 = (((MV_REG_READ(0x100c8) >> 24) & 0x3) == 0x3)?1:0; // 0=>3.3v 1=>2.5v

	if((tmp1 == 0) && (tmp2 == 0))		// 3.3v
		sprintf(ModelName,"%s ,%s", model_name_info[model_name_number], PHY33);
	else if((tmp1 == 0) && (tmp2 == 1))	// 2.5v
		sprintf(ModelName,"%s ,%s", model_name_info[model_name_number], PHY25);
	else if((tmp1 == 1) && (tmp2 == 0))     // 1.8v
		sprintf(ModelName,"%s ,%s", model_name_info[model_name_number], PHY18);
	else // config fail
		sprintf(ModelName,"%s ,%s", model_name_info[model_name_number], PHYERR);
	return 0;
}
///////////////////////////////
...
void print_dev_id(void){
	...
	get_model_type(modelname);

#if defined(MV_CPU_BE)
	printf("\n ** MARVELL BOARD: %s BE %s ",boardName, modelname);
#else
	printf("\n ** MARVELL BOARD: %s LE %s ",boardName, modelname);
#endif
    return;
}
...
int board_init (void)
{
	...
	mvCtrlEnvInit();
//QNAP patch PCIE reset , Low to High
	mvGppValueSet(0, MV_GPP7, 0);
	udelay(100000);
	mvGppValueSet(0, MV_GPP7, MV_GPP7);
///////////////////////////////////////
	mvBoardDebugLed(3);
...
ethaddr=$(ethaddr) eth1addr=$(eth1addr) ethmtu=$(ethmtu) eth1mtu=$(eth1mtu) $(netbsd_netconfig)");
	...
	else
	{
        ...
        env = getenv("bootargs");
        if(!env)
                setenv("bootargs","console=ttyS0,115200 root=/dev/ram initrd=0xa00000,0x900000 ramdisk=34816");
#if 0 // no use ==================
	...
	else
	{
        ...
        env = getenv("bootcmd");
        if(!env)
// QNAP Patch: Copy Kernel and Rootfs to memory
		setenv("bootcmd","cp.l 0xf8200000 0x800000 0x080000;cp.l 0xf8400000 0xa00000 0x240000;bootm 0x800000");
...
ip=$(ipaddr):$(serverip)$(bootargs_end); bootm 0x2000000;");
	...
	if (mvCtrlModelGet() == MV_6282_DEV_ID)
	{
       ...
       /* Set boodelay to 1 sec, if Monitor extension are disabled */
        if(!enaMonExt()){
                setenv("bootdelay","1");
		setenv("disaMvPnp","no");
	}
...
//QNAP patch : default Model set
	char bootp_vendor_class_name[30];
	memset(bootp_vendor_class_name, 0, sizeof(bootp_vendor_class_name));
	sprintf(bootp_vendor_class_name, "%s%s","F_",model_name_info[model_name_number]);
	env = getenv("bootp_vendor_class");
        if (!env)
		setenv("bootp_vendor_class", bootp_vendor_class_name);
////////////////////////////////
...
//Patch by QNAP:Fix GE1
		if(model_name_number == 0x14 || model_name_number == 0x15 || model_name_number == 0x1c){ 
			// TS-420U & TS-421U & TS-412 (6282 1.6G DDR3 256MB
			// Keep Lan 2 Work
		}else
		if(model_name_number >= 8){ // Model = TS219 or TS119
			mvCtrlPwrMemSet(ETH_GIG_UNIT_ID, 1, MV_FALSE);
			mvCtrlPwrClckSet(ETH_GIG_UNIT_ID, 1, MV_FALSE);
			MV_REG_WRITE(0x10008, 0x55550000); // GE1 Function change to SATA LED
			MV_REG_WRITE(0x1000C, 0x0); // GE1 Function change to GPIO
			MV_REG_WRITE(0x10010, 0x0); // GE1 Function change to GPIO
		}
///////////////////////
	}
	/* Close TDM */
