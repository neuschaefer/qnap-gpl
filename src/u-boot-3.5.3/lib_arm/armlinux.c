...
#include <dataflash.h>
#endif
//Patch by QNAP:Fix detect LCM
extern unsigned long mvGppValueGet(unsigned int group, unsigned long mask);
///////////////////////////////////////
/*cmd_boot.c*/
...
#endif
//Patch by QNAP:Fix detect LCM
	if(model_name_number <= 7){ // Model = TS419x
		if(!mvGppValueGet(1,0x00002000)){
			//LCM	
			char *ptr = strstr(commandline,"console");
			if(ptr != NULL){
				int i;
				for(i=0;i< strlen(commandline);i++,ptr++){
					if(*ptr != ' ' && *ptr != '\n' && *ptr != 0x0)
						*ptr = ' ';
					else break;
				}
