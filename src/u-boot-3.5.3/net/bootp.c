...
#ifdef QNAPNAS
    /* Richard Chen 20081224, QNAP for Recovery Button */
    {
        int vlen = 0;
        char *vclass = getenv("bootp_vendor_class");

        if (vclass) {
            vlen = strlen(vclass);
            *e++ = 60;
            *e++ = vlen;
            memcpy(e, vclass, vlen);
            e += vlen;
            debug ("bootp option attached: 60: %s\n", vclass);
        }
    }
#endif /* QNAPNAS */
...
DhcpHandler(uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	Bootp_t *bp = (Bootp_t *)pkt;
#ifdef QNAPNAS
    /* Richard Chen 20081225, Bootfile Environment */
    char *prefix = NULL;
#endif /* QNAPNAS */
...
#ifdef QNAPNAS
        /* Richard Chen 20081225, QNAP for Recovery Button */
        prefix = getenv("bootp_vendor_class");
        printf ("Bootfile Prefix: %s\n", prefix);
        if( prefix && !strncmp(bp->bp_file, prefix, strlen(prefix))) {
#endif /* QNAPNAS */
