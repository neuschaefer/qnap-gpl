...
//Ricky added 2005/09/14
#define QNAP
#ifdef QNAP

// QNAP Patch for new ToolChain
typedef unsigned long long u64; // from asm-generic/int-ll64.h
typedef u64 sector_t;           // from
typedef struct {                // from
        int counter;
} atomic_t;
////////////// QNAP Patch for new ToolChain
...
#include <math.h>
#include <linux/fs.h>
static void icp_usage(void);

#ifndef BLKQRRPART
#define BLKQRRPART  _IO(0x12,200)	/* re-read partition table but skip bdev->bd_part_count checking.*/
#endif
int opt_Q = 0;
#endif
...
void reread_partition_table(int leave);
...
{
        int ret=0,len,i,j,val;
        char tmp_ch;
        len = strlen(str);
        for(i=0;i<len;i++) {
                tmp_ch = str[i];
// modify by Kent 2008/12/02
// due to marvell arm glibc-2.5 pow(x,y) fail...
//		ret = hex_val(tmp_ch)*pow(16,len-i-1)+ret;
		val=1;
		for (j=0;j<len-i-1;j++)
			val=16*val;
		ret=hex_val(tmp_ch)*val+ret;
        }
        return ret;
}
#endif
...
"-b 2048: (for certain MO disks) use 2048-byte sectors\n");
		...
		case usage2:
		  /* msg in cases where fdisk used to probe */
			message = _(
#ifdef QNAP
"Usage of ICP fdisk commnad line:\n\n"
"fdisk -n|d partition_no [-t type][-c start_cylinder][-q partition_size][-a boot_number][-z swap_partition_no] DISK\n"
"fdisk -h icp       => show this help\n"
"fdisk -p DISK      => show disk information\n"
"------------------------------------------------------------------------------\n\n"
"partition_no : primary ->(1~4) , logical ->(up 4)\n"
"DISK         : is something like /dev/hdb or /dev/sda\n\n"
"-n : add a partition(is more than one partition ,used ',' as delimiter)\n"
"-d : delete a partition(is more than one partition ,used ',' as delimiter)\n"
"-t : partition type. p->primary,e->extented,l->logical;(default is p)\n"
"-c : start cylinder. (default is followed by previous one partition)\n"
"-q : partition size. (something like 50K or 1024M,default is increased to maximun)\n"
"-a : set bootable flag \n"
"-z : set sysid(is more than one partition ,used ',' as delimiter)\n"
"-j : set sysid of partition(used ',' as delimiter)\n"//avery20100429
"\nE.g.: fdisk -n 1 -q 5120M /dev/hdg (add 5G to partition /dev/hdg1)\n"
"      fdisk -n 2 /dev/hdg (add maximun quota to hdg2)\n"
"      fdisk -d all /dev/hdg (delete all partition on hdg)\n"
"      fdisk -n 1,2 -q 65M,0M -z 82,83 /dev/hdg (add two partition 65M and up to max on /dev/hdg)\n"
"      fdisk -j 1,82 /dev/hdg (set sysid as 82 on /dev/hdg1 )\n" //avery20100429
...
void icp_usage(void) {
	...
	message = _(
"Usage of ICP fdisk commnad line:\n\n"
                    "fdisk -n|d partition_no [-t type][-c start_cylinder][-q partition_size][-a boot_no][-z swap_partition_no] DISK\n"
                    "fdisk -h icp       => show this help\n"
                    "fdisk -p DISK      => show disk information\n"
                    "------------------------------------------------------------------------------\n\n"
                    "partition_no : primary ->(1~4) , logical ->(up 4)\n"
...
//Patch by QNAP:partition alignment 4K 
#ifdef QNAP
    sector_offset = 8;
#endif    
//////////////////////////////////////    
...
#ifdef QNAP
static void
change_sysid_ext(int part_no,int sys_id) {
        char *temp;
        int i = part_no, sys, origsys;
        struct partition *p;
...
#ifdef QNAP
static void list_disk_geometry_ext(void)
{
        printf("%d %d %d\n%d %d\n",heads, sectors,cylinders,units_per_sector, sector_size);
}
#endif
...
#ifdef QNAP
static int
add_partition_ext(int n,unsigned long s_cylinder,unsigned long parti_size,int sys){
...
//                                         0, mesg);
			if ( s_cylinder == 0 )
                                ...
                                start = cround(s_cylinder); //transfer to sectors
                        if (display_in_cyl_units) {
...
//                                cround(start), mesg);
                if (parti_size >0 ) {
                        stop = parti_size *2; // kb
                        stop /= units_per_sector;
//                      printf(" %d stop= %ld\n",units_per_sector,stop);
                        stop = stop +cround(start);
//                      printf("stop= %ld\n",stop);
                }
                else
                        stop = limit / units_per_sector;
                // NAS4000, bird change above statememnt on nasware 2.11
                // for solving HD larger than 40G will get wrong partition
                if (display_in_cyl_units) {
...
#ifdef QNAP
static int
add_logical_ext(int n,unsigned long s_cylinder,unsigned long parti_size) {
        ...
        return add_partition_ext(partitions-1,s_cylinder,parti_size,LINUX_NATIVE);
}
#endif
...
#ifdef QNAP
static int
new_partition_ext(int n,unsigned long s_cylinder,unsigned long parti_size,char parti_type){
	...
	if (!free_primary) {
                if (extended_offset)
#ifdef QNAP
			ret = add_logical_ext(n,s_cylinder,parti_size);
#else
                        add_logical();
#endif
                else
		...
		switch (parti_type) {
                case 'p':
                        ret = add_partition_ext(n,s_cylinder,parti_size,LINUX_NATIVE);
                        break;
                case 'l':
                        if (extended_offset)
                                ret = add_logical_ext(n,s_cylinder,parti_size);
                                ...
                                ret = -1;
                                printf("\n\n !!!!Please create extended partition first \n");
                        }
                        break;
                case 'e':
                        if (!extended_offset)
                                ret = add_partition_ext(n,s_cylinder,parti_size,EXTENDED);
                        break;
                default:
			break;
		}
...
void
Qreread_partition_table(int leave) {
	int error = 0;
	int i;
	...
	sync();
	sleep(2);
	if ((i = ioctl(fd, BLKQRRPART, NULL)) != 0) {
...
reread_partition_table(int leave) {
	...
	sync();
	sleep(2);
	if ((i = ioctl(fd, BLKRRPART, NULL)) != 0) {
                /* some kernel versions (1.2.x) seem to have trouble
                   rereading the partition table, but if asked to do it
		   twice, the second time works. - biro@yggdrasil.com */
                sync();
...
main(int argc, char **argv) {
	...
	int opt_n=0,opt_c=0,opt_t=0,opt_q=0,opt_d = 0,opt_a=0,opt_p=0,opt_z=0,opt_m=0,opt_j=0;
	int q_list_cnt = 0;
        unsigned long p_list_size[20];
        int my_sysid_list[20];
        int my_sysid_cnt = 0;
	int my_sysid_num = 0;	//avery20100429
	int my_sysid_parti = 0;	//avery20100429
        int n_list[20];
        int n_list_cnt = 0;
        int d_list[20];
        int d_list_cnt = 0;
	int boot_no=0;
        ...
        int parti_no = 1;                               //partition number (1 ~ n)
        unsigned long s_cylinder = 0;           // s: start cylinder
        char parti_type = 'p';                  // t: partion type, 'p'->primary 'l'->logical
        unsigned long p_size = 0;       // q: partition size (if 0, got maximum size)
        int d_parti_no = 1;                     // delete partition no (1~n) ,-1 => delete all
        char *tmp;
#endif
	 ...
	 */
#ifdef QNAP
	while ((c = getopt(argc, argv, "b:C:H:lsS:uvVn:t:c:q:d:h:a:ApPZ:z:mQj:J:")) != -1) {		//avery20100429
#else
			...
			exit(0);
                ...
                case 'n':
                        ...
                        if ( (tmp=strchr(optarg,',')) != NULL) {
                                char *m_opt;
                                while((m_opt=strchr(optarg,',')) != NULL) {
                                        *m_opt = '\0';
                                        n_list[n_list_cnt] = atoi(optarg);
                                        optarg = m_opt +1;
                                        n_list_cnt++;
                                }
                                n_list[n_list_cnt++] = atoi(optarg);
                                ...
                                parti_no = atoi(optarg);
                        break;
                case 't':
                        opt_t = 1;
                        if ( strcasecmp("P",optarg) == 0)
                                parti_type = 'p';
                        else if (strcasecmp("E",optarg) == 0)
                                parti_type = 'e';
                        else if (strcasecmp("L",optarg) == 0)
                                parti_type = 'l';
                        else
                                icp_usage();
                        break;
                ...
                case 'm':
                        ...
                        break;
		case 'j':
		case 'J':	//avery 20100429
		  	opt_j=1;
                	my_sysid_cnt = 0;
                        if ( (tmp=strchr(optarg,',')) != NULL) {				
                           	char *m_opt;
                             	if((m_opt=strchr(optarg,',')) != NULL) {
                              		*m_opt = '\0';
					my_sysid_parti = atox(optarg);
                                        optarg = m_opt +1;
                                }
				my_sysid_num =  atox(optarg);				
                        }
     			else{
				my_sysid_num =  atox(optarg);
				my_sysid_parti = 1;
                        }
			   printf("my_sysid_num= %d , my_sysid_parti = %d\n",my_sysid_num,my_sysid_parti);			
		  	   break;	
                case 'q':
                        opt_q = 1;
                        q_list_cnt = 0;
                        if ( (tmp=strchr(optarg,',')) != NULL) {
                                ...
                                while((m_opt=strchr(optarg,',')) != NULL) {
                                        *m_opt = '\0';
                                        if ((tmp = strchr(optarg,'K'))!= NULL) {
                                                *tmp='\0';
                                                p_list_size[q_list_cnt] = atol(optarg);
                                        }
                                        else if ((tmp = strchr(optarg,'M'))!= NULL) {
                                                *tmp='\0';
                                                p_list_size[q_list_cnt] = atol(optarg) * 1024;
                                        }
                                        else
                                                icp_usage();
                        ...
                        break;
                case 'd':
                        // d_parti_no not check now
                        opt_d = 1;
                        d_list_cnt=0;
                        if (strcasecmp(optarg,"all") == 0 ){
                                d_parti_no = -1;
			}
                        else if ( (tmp=strchr(optarg,',')) != NULL) {
                                char *m_opt;
                                while((m_opt=strchr(optarg,',')) != NULL) {
                                        *m_opt = '\0';
                                        d_list[d_list_cnt] = atoi(optarg);
                                        optarg = m_opt +1;
                                        d_list_cnt++;
                                }
                                d_list[d_list_cnt++] = atoi(optarg);
                                ...
                                d_parti_no = atoi(optarg);

                        break;
                case 'h':
//                case 'H':
                        if (strcasecmp(optarg,"icp") == 0 )
                                icp_usage();
                        else
                                fatal(usage);
                        exit(0);
                        ...
                        if ( (tmp=strchr(optarg,',')) != NULL) {
                                ...
                                while((m_opt=strchr(optarg,',')) != NULL) {
                                        *m_opt = '\0';
                                        ...
                                        my_sysid_list[my_sysid_cnt] = atox(optarg);
	...
	if (opts) {
        ...
        if (opt_n) {
                ...
                if (argc-optind == 1) {
                        ...
                        if ((tmp_fd = open(disk_device, O_RDONLY)) < 0)
                                fatal(unable_to_open);
                        if (ioctl(tmp_fd, BLKGETSIZE, &size))
                                fatal(ioctl_error);
                        close(tmp_fd);
                }
                ...
                //printf("opt_n = %d ,type = %d\n",opt_n,parti_no);
                //printf("opt_t = %d ,type = %c\n",opt_t,parti_type);
                //printf("opt_c = %d ,start cylinder = %ld\n",opt_c,s_cylinder);
                //printf("opt_q = %d ,partition size = %ld kb\n",opt_q,p_size);
                if (n_list_cnt > 0) {
                        int i;
                        for(i=0;i<q_list_cnt+1;i++)
                                ret = new_partition_ext(n_list[i]-1,0,p_list_size[i],(IS_EXTENDED(my_sysid_list[i])) ? 'e' : parti_type);
                }
                else
                        ret = new_partition_ext(parti_no-1,s_cylinder,p_size,(IS_EXTENDED(my_sysid_list[0])) ? 'e' : parti_type);
                //printf("ret = %d\n",ret);

                if(opt_a == 1) toggle_active(boot_no-1);
                if(opt_z == 1) {
                        if(my_sysid_cnt >1 ) {
                                int i;
                                if (my_sysid_cnt != q_list_cnt+1) {
                                        close(fd);
                                        icp_usage();
                                }
                                for(i=0;i<my_sysid_cnt;i++)
                                        change_sysid_ext(n_list[i]-1,my_sysid_list[i]);
                        }
                        else
                                change_sysid_ext(parti_no-1,my_sysid_list[0]);
                }
                if (ret >=0)
                        write_table();
                close(fd);
        ...
        if (opt_j) {	//avery 20100429
                ...
                change_sysid_ext(my_sysid_parti-1,my_sysid_num);
                write_table();
                close(fd);
        ...
        if (opt_d) {
                // Before using argument, check if it is legal
                nowarn = 1;
                if (argc-optind == 1) {
                        int tmp_fd;
                        long size;
                ...
                else if (d_list_cnt >0) {
                        for(i=0;i<d_list_cnt;i++)
                                delete_partition(d_list[i]-1);
