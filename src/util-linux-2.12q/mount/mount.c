...
#ifdef QNAP
static char *simplify_opts(const char *mnt_opts)
{
	char *new_opts = strdup(mnt_opts);
	char *tmp_opts = strdup(mnt_opts);
	char *ptr, *head = tmp_opts;
	int delalloc = -1, acl = -1, discard = -1, priv = -1;

	*new_opts = 0;
	while (1) {
		if ((ptr = strchr(head, ',')) != NULL)
			*ptr = 0;
		if (strcmp(head, "delalloc") == 0) {
			delalloc = 1;
		}
		else if (strcmp(head, "nodelalloc") == 0) {
			delalloc = 0;
		}
		else if (strcmp(head, "acl") == 0) {
			acl = 1;
		}
		else if (strcmp(head, "noacl") == 0) {
			acl = 0;
		}
		else if (strcmp(head, "discard") == 0) {
			discard = 1;
		}
		else if (strcmp(head, "nodiscard") == 0) {
			discard = 0;
		}
		else if (strcmp(head, "priv") == 0) {
			priv = 1;
		}
		else if (strcmp(head, "nopriv") == 0) {
			priv = 0;
		}
		else {
			if (*new_opts)
				strcat(new_opts, ",");
			strcat(new_opts, head);
		}
		if (!ptr)
			break;
		head = ptr + 1;
...
//		strcat(new_opts, ",");
	if (delalloc == 1) {
		strcat(new_opts, ",delalloc");
	}
	else if (delalloc == 0) {
		strcat(new_opts, ",nodelalloc");
	}
	if (acl == 1) {
		strcat(new_opts, ",acl");
	}
	else if (acl == 0) {
		strcat(new_opts, ",noacl");
	}
	if (discard == 1) {
		strcat(new_opts, ",discard");
	}
	else if (discard == 0) {
		strcat(new_opts, ",nodiscard");
	}
	if (priv == 1) {
		strcat(new_opts, ",priv");
	}
	else if (priv == 0) {
		strcat(new_opts, ",nopriv");
	}
/*	else {
		free(new_opts);
		new_opts = NULL;
...
update_mtab_entry(const char *spec, const char *node, const char *type,
	...
	if(!strcmp(type, "ext4dev"))
		strcpy((char *)type, "ext3");
	mnt.mnt_type = type;
	if(strstr(opts, "extents"))
		strcpy((char *)opts, "rw,data=writeback");
	mnt.mnt_opts = opts;
	mnt.mnt_freq = freq;
	mnt.mnt_passno = pass;
...
#ifdef QNAP
		{
			char *new_opts = simplify_opts(mnt.mnt_opts);
			if (new_opts) mnt.mnt_opts = new_opts;
#endif
