...
getftp (struct url *u, wgint *len, wgint restval, ccon *con)
{
  ...
  wgint rd_size;
	...
	if (opt.gen_temp_file) 
		sprintf(pid_file, "/tmp/qget/%d", getpid());
	else
		sprintf(pid_file, "/dev/null");
...
#ifdef QNAPNAS
#ifndef DSV2
			if ((progress_fpt = fopen(pid_file, "a")) != NULL ){
				fprintf (progress_fpt, "ERROR:The server refuses login.\n");
				fclose(progress_fpt);
			}
#endif
#endif
...
#ifndef DSV2
			if ((progress_fpt = fopen(pid_file, "a")) != NULL ){
				fprintf (progress_fpt, "ERROR:Login incorrect.\n");
				fclose(progress_fpt);
			}
#endif
#endif 
...
#ifndef DSV2
			if ((progress_fpt = fopen(pid_file, "a")) != NULL ){
				fprintf (progress_fpt, "ERROR:No such directory `%s'.\n", escnonprint (u->dir));
				fclose(progress_fpt);
			}
#endif
#endif    
...
#ifndef DSV2
			if ((progress_fpt = fopen(pid_file, "a")) != NULL ){
				fprintf (progress_fpt, "ERROR:No such file `%s'.\n", escnonprint (u->file));
				fclose(progress_fpt);
			}
#endif
#endif   
...
Error in server response, closing control connection.\n"));
...
#ifndef DSV2
			if ((progress_fpt = fopen(pid_file, "a")) != NULL ){
				fprintf (progress_fpt, "ERROR:No such file or directory.\n");
				fclose(progress_fpt);
			}
#endif
#endif   
...
#ifndef DSV2
			if ((progress_fpt = fopen(pid_file, "a")) != NULL ){
				fprintf (progress_fpt, "ERROR:%s. %s\n", con->target, strerror (errno));
				fclose(progress_fpt);
			}
#endif
#endif  
  ...
  if (*len)
	...
	if(*len || expected_bytes){
		size1 = expected_bytes ? expected_bytes : *len;  
		bt_db_update_task_field_by_pid(getpid(), BT_TASK_TBL_FIELD_TOTAL_WANTED, &size1);
	}
	else {
		bt_db_update_task_field_by_pid(getpid(), BT_TASK_TBL_FIELD_TOTAL_WANTED, 0);
	}
#else
	progress_fpt = fopen (pid_file, "a");
	if (progress_fpt)
		fprintf (progress_fpt, "start_time:%ld\n", stime);
		
	if(*len || expected_bytes){
		size1 = expected_bytes ? expected_bytes : *len;          
		if (progress_fpt){
			fprintf (progress_fpt, "saving:%.*f\n", 1, (double) size1 / (1024.0 * 1024.0));
			if (restval)
				fprintf (progress_fpt, "last_size:%.*f\n", 1, (double) restval / (1024.0 * 1024.0));
			else
				fputs ("last_size:0.0\n", progress_fpt);
			fflush (progress_fpt);
			fclose(progress_fpt);
...
#ifdef QNAPNAS
#ifdef DSV2
	int bt_state = BT_TASK_STATE_FINISHED;
#else
	FILE *progress_fpt;
	DIR *qget_tmp_folder;
	char pid_file[16];
...
#ifdef QNAPNAS
#ifdef DSV2
	bt_db_update_task_field_by_pid(getpid(), BT_TASK_TBL_FIELD_FILENAME, locf);
#else
  if ((qget_tmp_folder=opendir("/tmp/qget/"))!=NULL)
		closedir(qget_tmp_folder);
	else
		if (mkdir("/tmp/qget",0777)<0) exit(-1);
     
    if (opt.gen_temp_file) 
		sprintf(pid_file, "/tmp/qget/%d", getpid());
	else
		sprintf(pid_file, "/dev/null");	 
  progress_fpt = fopen (pid_file, "w");

	//stime=time((time_t *)NULL);

 if (progress_fpt){
 		//fprintf (progress_fpt, "start_time:%ld\n", stime);
		fprintf (progress_fpt, "time_left:<unknown>\n");
		fprintf (progress_fpt, "upload_rate:0.0\n");
		fprintf (progress_fpt, "uptotal:0.0\n");
		fprintf (progress_fpt, "downtotal:0.0\n");
		fprintf (progress_fpt, "seed:0\n");
		fprintf (progress_fpt, "down_peer:0\n");
		fprintf (progress_fpt, "download_to:%s\n", locf);
		fflush (progress_fpt);
		fclose(progress_fpt);
	}
#endif
...
#ifdef QNAPNAS
#ifdef DSV2
			bt_db_update_task_field_by_pid(getpid(), BT_TASK_TBL_FIELD_TOTAL_WANTED_DONE, &len);
			bt_db_update_task_field_by_pid(getpid(), BT_TASK_TBL_FIELD_STATE, &bt_state);
			bt_db_update_task_field_by_pid(getpid(), BT_TASK_TBL_FIELD_FINISH_TIME, tms);
#else
			if ((progress_fpt = fopen(pid_file, "a")) != NULL ){
				fprintf (progress_fpt, "saved:%s\n", number_to_static_string (len));
				fprintf (progress_fpt, "percent_done:100\n");
				fclose(progress_fpt);
			}
#endif
#endif
