...
#ifdef QNAPNAS
	{
	  ...
	  logprintf (LOG_VERBOSE, "%2d %s\n", statcode,
		...
		if ((progress_fpt = fopen(pid_file, "a")) != NULL ){
			fprintf (progress_fpt, "ERROR:%2d %s.\n", statcode, message ? escnonprint (message) : "No response");
			fclose(progress_fpt);
		}
#endif
	}
  ...
  if (opt.verbose)
	...
	if( contlen != -1 ){
		...
		if (progress_fpt){
			...
			if (contrange > 0)
				fprintf (progress_fpt, "last_size:%.*f\n", 1, (double) contrange / (1024.0 * 1024.0));
			else
				fputs ("last_size:0.0\n", progress_fpt);
		}
	}
...
#ifdef QNAPNAS
#ifdef DSV2
	bt_db_update_task_field_by_pid(getpid(), BT_TASK_TBL_FIELD_FILENAME, hstat.local_file);
#else
	//stime=time((time_t *)NULL);
	if (opt.gen_temp_file) 
		sprintf(pid_file, "/tmp/qget/%d", getpid());
      ...
      if (hstat.len == hstat.contlen)
...
#ifdef QNAPNAS
#ifdef DSV2
			bt_db_update_task_field_by_pid(getpid(), BT_TASK_TBL_FIELD_TOTAL_WANTED_DONE, &(hstat.len));
      ...
      else if (hstat.res == 0) /* No read error */
        {
          if (hstat.contlen == -1)  /* We don't know how much we were supposed
			...
			{
#ifdef DSV2
			bt_db_update_task_field_by_pid(getpid(), BT_TASK_TBL_FIELD_TOTAL_WANTED_DONE, &total_downloaded_bytes);
#else		
			if ((progress_fpt = fopen(pid_file, "a")) != NULL ){
				fprintf (progress_fpt, "saved:%s\n", number_to_static_string (total_downloaded_bytes));
				fprintf (progress_fpt, "percent_done:100\n");
				fclose(progress_fpt);
			}
#endif
