...
#ifdef QNAPNAS
  { "gentempfile",      &opt.gen_temp_file,     cmd_boolean },
#endif
...
#ifdef _QNAP_
  { "HTTP1.1",     	&opt.http_1_1,     	cmd_boolean },
#endif
...
defaults (void)
{
  ...
  if(strcmp(tmpval, "")){
    setval_internal (command_by_name ("httpproxy"), "http_proxy", tmpval);
    setval_internal (command_by_name ("httpsproxy"), "https_proxy", tmpval);
    setval_internal (command_by_name ("ftpproxy"), "ftp_proxy", tmpval);

    //user name
    _getcfg_str("System", "proxy username", "", tmpval, 1023, "/etc/config/uLinux.conf");
    if(strcmp(tmpval, "")){
      setval_internal (command_by_name ("proxyuser"), "proxy_user", tmpval);
    }
    
    //password
    _getcfg_str("System", "proxy password", "", tmpval, 1023, "/etc/config/uLinux.conf");
    if(strcmp(tmpval, "")){
      setval_internal (command_by_name ("proxypassword"), "proxy_passwd", tmpval);
    }
  }
#endif
}
