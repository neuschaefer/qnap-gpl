...
#ifdef QNAPNAS
#ifdef DSV2
static const char *eta_to_output(int);
#endif
#endif
...
#ifdef QNAPNAS
#ifdef DSV2
	  bt_db_update_task_field_by_pid(getpid(), BT_TASK_TBL_FIELD_PROGRESS, &percentage);
#endif
#endif
      p += 4;
    }
...
#ifdef QNAPNAS
#ifdef DSV2
	  rate = dlquant / dltime / 1024;
	  bt_db_update_task_field_by_pid(getpid(), BT_TASK_TBL_FIELD_DOWNLOAD_RATE, &rate);
#endif
#endif
    }
  else
...
#ifdef QNAPNAS
#ifdef DSV2
		  sprintf(buf, "%s", eta_to_output(eta));
		  bt_db_update_task_field_by_pid(getpid(), BT_TASK_TBL_FIELD_TIME_LEFT, buf);
#endif
#endif
        }
...
eta_to_output (int secs)
{
  ...
  if (secs < 60)
    sprintf (buf, "0:0:%d", secs);
  else if (secs < 60 * 60)
    sprintf (buf, "0:%d:%d", secs / 60, secs % 60);
  else if (secs < 60 * 60 * 90)
    sprintf (buf, "%d:%d:%d", secs / 3600, (secs % 3600) / 60, secs % 3600 % 60);
  else
    sprintf (buf, "99:99:99");
