...
create_image (struct bar_progress *bp, double dl_total_time, bool done)
{
  ...
  */
  ...
  if (bp->total_length > 0)
...
#ifdef QNAPNAS
			double percentage = (100.0 * size / bp->total_length);
#else
...
#ifdef QNAPNAS //percent_done
      if (percentage < 100)
	sprintf (p, "progress_info: %2.1f ", percentage); 
      else
	strcpy (p, "progress_info: 100.0 ");
	      p += strlen(p);
#else
      if (percentage < 100)
    ...
    APPEND_LITERAL ("    ");
#ifndef QNAPNAS  //Ignore process bar
  /* The progress bar: "[====>      ]" or "[++==>      ]". */ 
...
#ifdef QNAPNAS  //downtotal
	/* " 234,567.0" */
  {	
  	wgint size1;
  	size1 = size;
  	sprintf (p, "%.1f ", (double) (long) size1 / 1024.0);
	}
#else
  /* " 234,567,890" */
