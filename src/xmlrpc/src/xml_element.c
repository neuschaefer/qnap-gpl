...
static const char rcsid[] = "#(@) $Id: xml_element.c,v 1.1.1.1 2010/04/21 12:52:13 kent Exp $";
 ...
 * HISTORY
 *   $Log: xml_element.c,v $
 *   Revision 1.1.1.1  2010/04/21 12:52:13  kent
 *   init
 *
 *   Revision 1.22  2008/04/16 13:03:15  robincornelius
 *   tab2space the code, using 4 spaces for a tab
 *
 *   Revision 1.21  2008/04/14 12:05:49  robincornelius
 *   remove libxml2 as the php compatability layer is a licence issue
 *
 *   Revision 1.20  2008/04/02 10:57:15  robincornelius
 *   even more build warnings
 *
 *   Revision 1.19  2008/04/02 10:35:13  robincornelius
 *   Fix a few more build warnings in expat and sample
 *
 *   Revision 1.18  2008/03/26 12:27:36  robincornelius
 *   change build *again* to allow either expat or libxml as a parser
 *
 *   Revision 1.17  2008/03/26 08:59:35  robincornelius
 *   better return value for file_out
 *
 *   Revision 1.16  2008/03/25 09:46:06  robincornelius
 *   to fit in with autotools better, build private expat as default and enable shared expat with configure option
 *
 *   Revision 1.15  2008/03/24 15:24:09  robincornelius
 *   allow optional building with builtin or system wide libexpat
 *
 *   Revision 1.14  2008/03/21 19:03:27  robincornelius
 *   lets not use a private copy of libexpat
 *
 *   Revision 1.13  2008/03/21 17:37:09  robincornelius
 *   fix strict gcc warnings some of which can cause arch specific issues, eg 64 bit failure
 *
 *   Revision 1.12  2008/03/21 09:59:10  robincornelius
 *   fix a gcc error in build
 *
...
static void _xmlrpc_startElement(void *userData, const XML_Char *name, const XML_Char **attrs)
{
   xml_element *c;
   xml_elem_data* mydata = (xml_elem_data*)userData;
   const char** p = (const char**)attrs;

   if(mydata) {
      c = mydata->current;

      mydata->current = xml_elem_new();
      mydata->current->name = (char*)strdup((const char*)name);
      ...
      while(p && *p) {
         ...
         if(attr) {
            attr->key = strdup(*p);
            attr->val = strdup(*(p+1));
            Q_PushTail(&mydata->current->attrs, (const char**)attr);
...
static void _xmlrpc_endElement(void *userData, const XML_Char *name)
{
   xml_elem_data* mydata = (xml_elem_data*)userData;
                        ...
                        int len)
{
   ...
   if(mydata && mydata->current) {
      ...
      if(mydata->needs_enc_conversion && mydata->input_options->encoding) {
         int new_len = 0;
         char* add_text = utf8_decode((const char*)s, len, &new_len, mydata->input_options->encoding);
         if(add_text) {
            len = new_len;
      ...
      simplestring_addn(&mydata->current->text, (const char*)s, len);
   }
}
/******/
...
xml_element* xml_elem_parse_buf(const char* in_buf, int len, XML_ELEM_INPUT_OPTIONS options, XML_ELEM_ERROR error)
{
   ...
   if(in_buf) {
      ...
      /* parse the XML */
      if(XML_Parse(parser, (const XML_Char *) in_buf, len, 1) == 0) {
